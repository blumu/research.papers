...
\setlist[itemize]{itemindent=0.5em}
...
\end{theorem}
\begin{proof}
(i) This is the counterpart of Theorem 2 from \cite{danos-head} for the head linear reduction.

The proof can be adapted from Danos-Regnier's proof of the same result for head-linear reduction~\cite{danos-head}.
\begin{todobox}
Include proof details by adapting Danos-Regnier proof to the definition of \emph{leftmost} linear reduction.
\end{todobox}
...
\begin{todobox}
Hopefully the proof should not involve state machine/stack/environments, ...
\end{todobox}
\end{proof}
...
%%%%%%%%%%%%%%%%%
\subsection{Correctness of ULC normalization}
...
\label{fig:reducing_trivialredex}
    ...
    \item $\rightarrow_M$ can simulate $\rightarrow_N$ and reciprocally.

    This is shown by induction on the traversal rules. One interesting case is when ghost nodes in $M$ get materialized into structural nodes in $N$:
...
Hence the arity threshold of a traversal ending with external variable $x$ is precisely $|x|$, the arity of $x$. A trivial induction on the traversal rules shows, that since the \rulename{Var} is never used, we necessarily have  $\travsetnorm(M) = \pathset(M)$. Further, since all nodes are externals, the projection $\coresymbol$ and $\pview{\_}$ functions both coincides with the identity function thus $\pview{\core{\travsetnorm(M)}} = \travsetnorm(M) = \pathset(M)$.
