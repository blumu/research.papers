...
\newcommand{\extctree}{\tilde{\Tau}} %extended computation tree
...
%%%%%%%%%%%%%%%
\subsection{Overview}
In this paper we: (i) Generalize the notion of traversals to the untyped lambda calculus (ULC), which we call \emph{imaginery travserals}; (ii) Introduce a method to evaluate untyped lambda terms based on enumerating traversals; (iii) Formalize the connection between traversals and the notion of \emph{head linear reduction}.

The key idea behind imaginary traversals lies in an ad-hoc use of eta-expansion. Attempting to traverse an untyped lambda term using the original traversal rules causes the traversal to get stuck when the operator sub-term of an application has insufficiently many operands. In such a situation, there is no operand node in the term tree to traverse. Take the untyped term $(\lambda f.f)(\lambda x.x)$ for example. Traversing it using the rules of STLC gives the sequence of nodes $\lambda f \cdot f \cdot \lambda x \cdot x$ (where pointers are omitted), at which point the traversal is stuck because there is no subterm representing the first argument of the variable occurrence $f$.
...
Generalizing this idea, the traversal rules of STLC induce an implicit `eta-expansion strategy' that imaginary ULC traversals
 implement: at any given point where a traversal get stuck, among all possible subterms to eta-expand, there is only one that exhibits nodes that are necessary for the corresponding STLC traversal to resume.
...
\\[12pt]
    ...
    \end{tikzpicture}
    \end{tabular}
\caption{Examples of computation trees of untyped lambda terms. Ghost nodes are omitted.}
...
We call the tree hereby defined the \defname{extended computation tree} of $M$,
denoted $\extctree(M)$ over the extended set of nodes $\ExtendedNodes = \Nodes + \ghostvar + \ghostlmd$, where $+$ denotes disjoint union on sets. We use the shorthands $\ImNodesLmd$ for $\NodesLmd + \ghostlmd$ and $\ImNodesVar$ for $\NodesVar + \ghostvar$.

\subsection{Justified sequence of nodes}
\label{sec:justseq}

A \defname{justified sequence} is defined a sequence of nodes in $\ExtendedNodes$ where every occurrence $n$ in the sequence, except the first one, has an associated link called ``justification pointer'' pointing to some previous node occurrence $j$ in the sequence, called its ``justifier'', with an associated ``link label'' $l\geq0$; with the constraint that the justifier node $\enables$-enables the source node with the corresponding label; that is $j \enables_l n$ must hold. We represent a justified sequence as a sequence of labelled node occurrences with back-pointing arrows representing justification pointers, \eg, $\Pstr[10pt]{ s = \cdots (j){j} \ldots (n-j,25:l){n} }$ and we say that $n$ `points to' or `is justified by' $n$.
...
\begin{definition}
The set $\travulc(M)$ of \defname{traversals} of an untyped lambda term $M$, abbreviated $\travulc$ when the term is clear from context, is the set of justified sequences of nodes over the computation tree of $M$ extended with ghost nodes, recursively defined by the rules of Table~\ref{tab:trav_rules}.
...
\begin{itemize}[leftmargin=3em]
    ...
    \begin{compactitem}
        ...
        $\beta \in \ImNodesLmd, m \in\ImNodesVar \Union \NodesApp$
...
\item[\rulenamet{Var}] If \Pstr[0.5cm]{t\cdot(m){m}\cdot(beta){\beta}
    \ldots (n-beta,50:i){n}\in\travulc} for $m \in\ImNodesVar \union \NodesApp$, internal nodes $\beta \in \ImNodesLmd$, $n \in \ImNodesVar$
...
\begin{itemize}[leftmargin=3em]
\item[\rulenamet{IVar}] If $t \cdot n \in \travset$ with external node $n \in \ImNodesVar$ then
$t \cdot n \cdot\alpha\in\travulc$
for every $i$th child $\alpha$ of any node $m \in \ImNodesVar$ occurring in $\oview{t\cdot n}$, $i\geq1$, where $\alpha$ points to $m$ with label $i$.
\end{itemize}
...
the enabling relation $\enables$ on extended nodes $\ExtendedNodes$, and $t$ verifies the alternation condition;
...
\item[(iii)] $\pview{t}$ is a (justified) path in the extended computation tree $\extctree(M)$;
...
It is the variable rules \rulenamet{Var} and \rulenamet{IVar} that perform `on-the-fly expansion' when the arity of a variable is too low to statically determine the operand of an application. When the variable arity is high enough, the definition of the rule coincides with that of STLC, where the tree statically dictates the node to traverse next.
We distinguish those two rule sub-cases and refer to them as the \emph{concrete sub-case} where $i \leq |m|$ and $\alpha$ is a structural lambda node in $\NodesLmd$; and the \emph{eta-expanded sub-case} where $i > |m|$ and $\alpha$ is a ghost lambda node $\ghostlmd$.
...
\coresymbol_{\epsilon}(s) \cdot n
    & \mbox{if } n\in\ImNodesVar\inter\ExternalNodes, \\
\coresymbol_{pop_{|n|}(\overline{y})}(s)
    & \mbox{if } n\in(\ImNodesVar\inter\InternalNodes)\union\NodesApp, \\
...
Let us fix a simply-typed term-in-context $\Gamma \entail M :T$, with typed-context $\Gamma$ (a set of typed variables), and simple type $T$. Its \defname{eta-long form}, defined inductively on the type $T$, is obtained by recursively eta-expanding every subterm as many times as possible with respect to the subterm type~\cite{OngLics2006,BlumPhd}.
...
are the justified sequences obtained by applying on its
\emph{eta-long form} the system of rules
of Table~\ref{tab:trav_rules}
with the exclusion of rule \rulenamet{Lam^\ghostvar} and
where rules \rulenamet{Var} and \rulenamet{IVar} are restricted to their concrete sub-case--that is justification label $i$ must be no greater than the arity $|m|$ of variable $m$.
\end{definition}
...
Let $\Gamma \entail M : T$ be a simply-typed term in context.
 ...
 justified sequences obtained by applying on its
 eta-long normal form the system of rules
of Table~\ref{tab:trav_rules}
where \rulenamet{IVar} is restricted to the concrete subcase with the condition $i\leq|m|$.
\end{proposition}
\begin{proof}
...
In this section we show how to evaluate lambda terms with traversals and find their normal form when it exists. The crux of the algorithm lies in the \emph{Path Characterization} (Theorem~\ref{thm:path_charact_ulc}) which states that the set of traversals \emph{core P-views} captures what is strictly required from traversals to reconstruct the normal form.
For STLC, the characterization result was previously shown using the Game Semantics correspondence (Proposition~\ref{prop:path_charact_stlc}); we prove its untyped counterpart using instead a term-rewriting argument.
Such characterization of paths by traversals suggests a normalization method based on enumerating all the maximal traversals. The challenge is that there are
...
A normalization procedure based on enumerating all traversals is not practical because the set of traversals can be infinite. This is expected for non-normalizing terms such as $\Omega = (\lambda x. x x)(\lambda y. y y)$ which has infinitely long traversals of the form $\lambda x \cdot x \cdot \lambda y \cdot y \cdot x \cdot \lambda y \cdot y \cdot \ldots$ But this is also the case for terms having a normal form. Take for example $M = \lambda f . f (\lambda x. x)$, then for all $k\geq0$ the justified sequence $t_k = \lambda f \cdot f \cdot (\lambda x \cdot  x)^k$ (where every occurrence of $\lambda x$ points to $f$ and every $x$ points to the preceding $\lambda x$) is a traversal.
This is due to rule \rulenamet{IVar} which offers two non-deterministic choices when traversing a free variable:
\begin{itemize}
...
This is because we want to model the behaviour of \emph{all} contexts that can possibly interact with $M$. To continue with the above example, for every $k$ there exists a term that applies its argument $k$ times, for instance $F_k = \lambda g . g (g ( \ldots (g z)))$ with $k$ applications of $g$. Therefore there has to be plays in the game denotation of $M$ to accounts for the possibility of such terms being passed as argument $f$. Those plays correspond to those traversals $t_k$, $k\geq 0$.

 Fortunately, traversals accounting for all those contexts are redundant for normalization: due to the absence of side-effects, calling the same argument multiple times always involves the same underlying computation in $M$. We formalize this intuition with the notion of \emph{branching traversals} (Definition~\ref{dfn:branching_traversals})  which prevent such repetitive behaviour while still covering all $\sim$-equivalence classes (Proposition~\ref{prop:branching_traversal_simcomplete}).
...
input-variable rule \rulenamet{IVar} is restricted to be necessarily the last node in the traversal: \ie, $m=n$ and the traversal obtained is $\Pstr[0.5cm]{t \cdot (n){n} \cdot (l-n,25:i){\alpha}} \in \travsetbr$.
...
Continuing with the intuition from Game semantics: Lambda nodes correspond to opponent moves in game semantics; and restriction on the opponent moves limit the set of contexts in which a term can appear. By limiting the possible lambda nodes to traverse after a free variable, the branching restriction eliminates all contexts where some term applied to $M$ refers to the same argument more than once; or where two arguments of the same higher-order function are called in the same function body. This excludes contexts involving $F_k$, $k\geq 2$, or Kierstead's terms $\lambda f .f (\lambda x .f (\lambda y.y))$ and $\lambda f .f (\lambda x .f (\lambda y.x))$ where $f$ occurs more than once, but also $\lambda f. \lambda g . f (\lambda f .(\lambda x . g (\lambda y . x)))$ where the consecutive lambda abstractions $\lambda f . \lambda g . ( \cdots )$ have in the body occurrences of both $f$ and $g$.
...
\label{prop:branching_traversal_simcomplete}
  The set of branching traversals $\travsetbr$ is  $\sim$-complete.
\end{proposition}
\proofatend
%\begin{proof}
...
\cite{OngLics2006} (where \rulenamet{IVar} is renamed \rulenamet{Sig}).
...
The normalization procedure for the simply-typed lambda calculus is given by Algorithm~\ref{algo:stlc_normalization_by_traversals}. It was originally implemented in~\cite{BlumGalop2008, Blum-HogTool}.
...
For typed languages such as the simply-typed lambda calculus, PCF or Idealized Algol, the theory of traversals~\cite{OngLics2006} correspond to Game Semantics in a very concrete way: they are in one to one correspondence with the plays from the game denotation where all internal moves are revealed; whereas \emph{traversal cores} are precisely the plays from the game denotation~\cite{BlumPhd}.
\begin{theorem}[Game Semantics Correspondence for STLC -- Theorem 4.96 in~\cite{BlumPhd}]
\label{thm:gamesem_correspondence_stlc}
Let $\Gamma \entail M :T$ be a simply-typed term-in-context. Then
 $\travstlc$ is in bijection with the \emph{revealed game denotation} $\revsem{\Gamma \entail M :T}$ of the term;
 and its image by $\coresymbol$--the justified sequences that are \emph{core} of some STLC traversal--is in bijection with the innocent game denotation $\sem{\Gamma \entail M :T}$.
\end{theorem}
...
A consequence of this correspondence is that traversals characterize
the paths in the tree representation of the normal form of a simply-typed term in eta-long form:
\begin{proposition}[Normalized Paths Characterization for STLC]
\label{prop:path_charact_stlc}
For every simply-typed term-in-context $\Gamma \entail M : T$
we have $\travstlc(\Gamma \entail M : T)/{\sim}\ = \pathset(\etabetalnf{M})$
where $\etabetalnf{M}$ denotes the eta-long form of the beta-normal form of $M$.
\end{proposition}
\begin{proof}
 By soundness of Game Semantics, beta-eta equivalent terms have the same game denotation, therefore by Theorem~\ref{thm:gamesem_correspondence_stlc}, the set of traversals of the eta-long beta-normal form of $M$ is also given by the set of cores of traversals of the eta-long form of $M$. The Path-View correspondence (Proposition~\ref{prop:ulctrav_welldefined_pathview}) and Proposition~\ref{prop:ulc_and_stlc_trav_coincide} then show that the P-views of traversal cores of the eta-long form of $M$ give the tree paths of the eta-long beta normal form.
\end{proof}
...
We showed in the previous section that for eta-long forms, there is a finite number of branching traversals, which in turn implies termination of the normalization procedure of Algorithm~\ref{algo:stlc_normalization_by_traversals}. For untyped terms, however, branching traversals can be infinite because of \emph{on-the-fly eta-expansion} in rule $\rulename{IVar}$: the justification label $i>0$ in the traversal rule is unbounded. In this section, we show that, for the purpose of term evaluation, $i$ can be bounded by a quantity called the \emph{arity threshold} computable from the traversal, effectively limitting  limits the number of times eta-expansion can be performed by rule \rulenamet{IVar} for the sake of discovering the set of paths of the beta-normal form. In section \ref{sec:correctness_ulc_normalization} we show that this suffice to  sufficient to characterize the normal form of a lambda term when it exists.
...
The \defname{arity threshold} of a traversal $t$ ending with strand
 $\underline{\alpha_k}\,n_k\,\alpha_{k-1}\,n_{k-1}\cdots \alpha_2\,n_2\, \alpha_1\,\underline{n_1}$ for $k>0$ where $\alpha_j \in \ImNodesLmd$, $n_j \in \ImNodesVar\union\NodesApp$ for all $1\leq j\leq k$ is
$$
\arth(t) = \max_{q=1..k} \left( |n_q| + \sum_{j=1..q-1} (|n_j|-|\alpha_j|) \right)\ .
$$
Equivalently, by abbreviating $ \delta_j = |n_j|-|\alpha_j|$, we have
$\arth(t) = \max_{q=1..k} \left( |n_q| + \sum_{j=1..q-1} \delta_j \right)
= \max_{q=1..k} \left( |\alpha_q| + \sum_{j=1..q} \delta_j\right)
$.
% \begin{align*}
...
One can thus calculate the arity threshold with the induction
 $b_1 = |n_1|$, $b_{q+1} = b_q + |n_{q+1}|-|\alpha_q|$ for $1 \leq q \leq k-1$,
...
\label{prop:weaving}
Let $t$ be a branching traversal of $\travsetbr$ ending with a strand
$\underline{\alpha_k}\,n_k\,\alpha_{k-1}\, n_{k-1}\cdots\alpha_2\,n_2\, \alpha_1\,\underline{n_1}$ for some $k\geq1$, and $t_{\max}$ be a maximal extension of $t$ in $\travsetbr$. By rule $\rulename{IVar_\branching}$, $t$ is immediately followed in $t_{\max}$ by an external $\lambda$-node justified by $t^\omega$ with label $i\geq 1$.
...
\end{proposition}
\shortproof
By finite induction on $q\in\{1..k\}$. Base case $q=1$ is taken care of by rule \rulenamet{IVar_\branching} followed by
\rulenamet{Lam^\ghostvar_\branching}.
...
By the induction hypothesis we have  $i_{q} = i - \sum_{j=1..q-1} \delta_j = i - \left( |n_q| + \sum_{j=1..q-1} \delta_j\right) - |n_q|
> i - \max_{r=1..k} \left( |n_r| + \sum_{j=1..r-1} \delta_j \right) - |n_q|
= i - \arth(t) + |n_q|$.
Since by assumption $i>\arth(t)$ this implies that $i_{q} > |n_q|$.
We can therefore use rule \rulenamet{Var_\branching}
...
We then use rule \rulenamet{Lam^\ghostvar_\branching} to traverse the next ghost variable justified by $\alpha_q$ and labelled by $i_{q+1} = i_q + |\alpha_{q}| -|n_{q}|$.
...
\begin{itemize}
\item Base case $q=1$: Because $t^\omega$ is a ghost external variable node, the only rule to apply is \rulenamet{IVar_\branching}, and by assumption the following node is a ghost lambda node justified by $t^\omega$ with label $i_1 = i$. By rule \rulenamet{Lam^\ghostvar_\branching} the next node is a ghost variable justified by $\alpha$ with label $i_2 = i + |\alpha_1| - |t^\omega|$.
...
Since $i_q > |n_q|$, by rule \rulenamet{Var_\branching} the next node is necessarily a ghost lambda node justified by $n_q$ with label $i_q$. With rule \rulenamet{Lam^\ghostvar_\branching} the following node is a ghost variable node $\ghostvar$ justified by $\alpha_q$ and labelled by $i_{q+1} = i_q + |\alpha_{q}| -|n_{q}|$.
\end{itemize}
...
%      \andalso n \in\ExternalNodes\inter\ImNodesVar
%      \andalso n \enables_i \alpha
...
\begin{table}[htbp]
    ...
    \infrule[$\rulefont{Lam^\ghostvar_\normalizing}$]
        ...
        }
        {
            ...
            \andalso(\beta\in\ImNodesLmd, m \in\ImNodesVar\union\NodesApp)
        }
    ...
    \infrule[$\rulefont{Var}_\normalizing$]
        {
            ...
            \ldots (n-beta,50:i){n}}
            \andalso n \in \ImNodesVar \setminus \ExternalNodes
        ...
        }
        {
            ...
            \andalso (m \in \ImNodesVar\union\NodesApp, \alpha\in\ImNodesLmd,\beta\in\ImNodesLmd)
        }

    \emph{\bf DATA - Input-variable rule}
    \infrule[$\rulefont{IVar_\normalizing}$]
         {\istraversal t \cdot n
          \andalso n \in \ImNodesVar\inter\ExternalNodes
          \andalso n \enables_i \alpha
          \andalso 1\leq i \leq\arth(t)
         }
...
$t_\epsilon = \Pstr[0.7cm]{(n0){\lambda }\,(n1){@}\,(n2-n1){\lambda x y s z}\,(n3-n2){x}\,(n4-n1,32){\lambda s z}\,(n5-n4,30){s}\,(n6-n3,33){\lambda }\,(n7-n2,37){s}\,(n8-n1,32){\ghostlmd^3}\,(n9-n0,32){\ghostvar^2} }$.
...
%\item
How many more values of $k$ shall we look at? Intuitively, we should not eta-expand the node beyond the point where further traversals are guaranteed to visit ghost occurrences only. That upper-bound for $k$ is precisely given by the \emph{arity threshold} of $t_\epsilon$. The traversal is itself a strand and the node occurrence arities are in order $0,1,4,2,2,1,0,0,0,0$ therefore:
     %\max \{ 0, 1,\, (1)-2+2,\, (1-2+2)-4+1 \} =
     \begin{align*}
     \arth(t_\epsilon)
     &= \max \{ |s^2|,\ %\\
                |s^1| + (|s^2| - |\lambda|),\  %\\
    ...
    & = \max \{   0 , 1 + (0 - 0) ,  2 + (1 - 2) + (0 - 0)
            ...
            \} = 1.
     \end{align*}
We thus do not have to consider the cases $k>1$ which means that the normal form has shape $\lambda y s \ldots \cdot s (y R_1 \ldots R_{q_2})$.
...
(n19-n0,42){\ghostvar^1}
(n20-n19,25){\ghostlmd^1}
(n21-n18,25){\underline{\ghostvar^1}}
