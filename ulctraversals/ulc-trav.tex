...
\begin{itemize}[leftmargin=3em]
    \item[\rulenamet{Root}]
    $\istraversal r$ where $r$ is the root of the term tree $\ctree(M)$.
    ...
    \begin{compactitem}
        ...
        then $\istraversal \Pstr[0.5cm]{ u \cdot(beta){\beta} \,
...
\caption{Derivation rules of the imaginary traversals $\travulc$ of the untyped $\lambda$-calculus.}
...
By the induction hypothesis we have $|\lambda_l(\lambda\overline{\xi_i})| < k_i$, and since nodes in $v$ are all ghosts we have $|@_i|< k_i$ thus the `$\max$' is negative which shows $|\lambda_l(\lambda\overline{\xi_{i-1}})| < k_{i-1}$.
...
\fi
%
Let $@_j$ and $\lambda\overline{\xi_j}$ for $1\leq j\leq a$, $a\geq 1$
...
So that $\lambda\overline{\xi_1}$ is $x$'s binder and $@_1$ its parent.
Let $i$ denote $x$'s binding index in $\lambda\overline{\xi_1}$.
%
After reduction,
...
%that maps $\lambda\overline{y}$ to $\lambda\overline\eta$.
