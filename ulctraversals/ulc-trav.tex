...
Figure~\ref{fig:comptree_examples} illustrates examples of term trees.
...
The \defname{quadruplet encoding} of a justified sequence of nodes with variables in $\VarSet$ is a sequence of quadruplets in $O_4 = \nat^*\times \mathcal{V}^* \times\nat\times\nat$ defined occurrence-wise where each quadruplets $(n,p,j,i) \in O_4$ is such that the \emph{Node path} $n$ encodes the path to the node in the extensional tree, which uniquely defines the type and label of the occurrence;
...
\label{examp:ghost_materialization}
    Take the term $M = \lambda x. (\lambda y z.z) u$. The structural paths of the tree  $\ctree{M}$ are $\Sigma = \{\epsilon, 0, 00,000, 01,010\}$. The justified sequence
    $\Pstr[10pt]{
    ...
    $\Pstr[10pt]{
        (l){\lambda xg} \cdot (a)@ \cdot (lyz-a,25:0){\lambda y z h k} \cdot (z-lyz,25:2)z \cdot (gl-a,30:2)\lambda \cdot (gv-l,31:2)g }
    $.
\end{example}
...
\begin{example}
The eta-expansion of $J = (\lambda u . u(x\,u)) (\lambda v . v\,y)$, with tree shown in Figure~\ref{fig:comptree_examples}, with respect to traversal $t$ from Example~\ref{ex:ulctrav_sample} is
...
the traversal $t$ of $J$ correspond to the
traversal
$\Pstr[0.7cm]{
 (n0){\lambda}
 (n1){@}
...
We consider sequences of variable names in $\VarSet^*$ as stacks equipped with  push and pop operations:
for any sequence of variables $\overline{x} = x_1 \ldots x_n$, $n,j\geq0$, the operation $pop_j$ removes the first $j$ elements of the sequence: $pop_j (x_1 \ldots x_n) = x_{j+1} \ldots x_n$ if $j<n$, and $pop_j (x_1 \ldots x_n) = \epsilon$ otherwise. And for any other variable names $\overline{y} = y_1 \ldots y_m$, $m\geq0$ we write $\overline{x} \otimes \overline{y}$
for the stack obtained by pushing the variables $\overline{x}$, read from right to left, onto $\overline{y}$, while renaming afresh any variable in $\overline{x}$ that would clash with some name in $\overline{y}$:
$\overline{x} \otimes \overline{y} = \tilde{x}_1 \ldots \tilde{x}_n y_1 \ldots y_n$, where $\tilde{x}_i = x_i'$ with fresh name $x_i'$ if $x_i\in \overline{y}$, or $\tilde{x}_i = x_i$ otherwise, for $1\leq i\leq n$.
...
\coresymbol_{\overline{x} \otimes \overline{y}}(s)
...
(To keep the definition simple we do not bother renaming the variable occurrences when a variable get renamed via $\otimes$, this is because the name of variable occurrences can always be recovered from its justifier and justification label.)
\end{definition}
