...
\begin{itemize}[leftmargin=3em]
    ...
    \item[\rulenamet{Lam}] If $\istraversal t \cdot \alpha$ for $\alpha\in\ExtendedNodesLmd$ then $\istraversal t \cdot \alpha ~ n$ where $n$ is the unique child of $\alpha$ and:
    \begin{compactitem}
        \item[\rulenamet{Lam^@}] If $n$ is an $@$-node then it has no justifier;
        \item[\rulenamet{Lam^{\sf var}}] If $n$ is a structure variable in $\NodesVar$ then it points to the only occurrence in $\pview{t\cdot \alpha}$ of a node $m$ verifying $m \enables_k n$ for some $k\geq 0$, and has link label $k$;

        \item[\rulenamet{Lam^\ghostvar}] If $n$ is a ghost variable $\ghostvar$
        %(\ie,~$\alpha = \ghostlmd$)
         with $t \cdot \alpha = \Pstr[0.5cm]{ u \cdot \beta \,
        (m){m} \cdot v \cdot (gl-m,40:i){\ghostlmd}}$,
        $\beta \in \ExtendedNodesLmd, m \in\ExtendedNodesVar \union \NodesApp$
        then it points to $\beta$ with label $|\beta| + i - |m|$
        that is
        $\istraversal \Pstr[0.5cm]{ u \cdot(beta){\beta} \,
    (m){m} \cdot v \cdot  (gl-m,30:i){\ghostlmd}\, (al-beta,40:{|\beta| + i - |m|}){\ghostvar}}$.
    \end{compactitem}
\end{itemize}
\emph{\bf PROGRAM -- Copy-cat rule}
\begin{itemize}[leftmargin=3em]
\item[\rulenamet{Var}] If $\istraversal \Pstr[0.5cm]{t\cdot(m){m}\, (beta){\beta}
\cdot v \cdot (n-beta,50:i){n}}$ for $m \in\ExtendedNodesVar \union \NodesApp$, $\beta \in \ExtendedNodesLmd\inter\InternalNodes$, $n \in \ExtendedNodesVar\inter\InternalNodes$, $i>0$ then
...
\infrule[$\rulefont{IVar_\essential}$]
     ...
     {\istraversal  \Pstr[0.5cm]{t \cdot (n){n} \cdot (l-n,25:i){\alpha}}}
...
so that the argument index $i$ cannot be greater than the dynamic arity of the traversal. Written as a judgement derivation rule the rule becomes:
 \infrule[$\rulefont{IVar_\normalizing}$]
      ...
      {\Pstr[0.5cm]{\istraversal t \cdot (n){n} \cdot (l-n,25:i){\alpha}}.}
\iflongversion
