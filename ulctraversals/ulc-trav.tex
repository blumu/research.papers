...
%\title{Normalizing Untyped Lambda Terms via Imaginary Traversals [Draft]}
...
% Normalizing Lambda Terms with Imaginary Traversals [Draft]
...
Two sequences $s$ and $u$ over possibly two distinct terms are \defname{equivalent}, written $s \equiv u$ just if they have the same structure: the types of the underlying nodes in the two sequences match each other and have the same justification pointers. Two equivalent sequences are not necessarily equal, in particular they may have different variable names.
...
\todo{Minor: Add an example of core projection involving an external ghost lambda nodes.}
...
\proofatend
%\begin{proof}
We prove the result using a bisimulation argument on the state transition system whose states are the traversals and transitions are given by traversal rules.

Suppose that $M \rightarrow_l N$. Let $x$ denote the leftmost linear variable occurrence in $M$ and let $(\lambda x, A)$ be the generalized redex involving it, so that $N$ is the term obtained after firing the generalized redex $(\lambda x, A)$.

The substitution involved in the leftmost linear reduction affects the computation tree in three possible ways depending on $x$'s operands:
\begin{description}
    \item[1. $x$ has at least one operand and $A$ is an abstraction]
    This case is represented on Figure~\ref{fig:firing_genredex_effect_on_tree_with_operands_and_lambda_argument}: after reduction the label of node $x$ gets replaced by $@$, its existing children are preserved, and $A$ gets prepended as its $0$th child.
...
which maps nodes in $N$ resulting from the substitution, to their corresponding nodes in $M$, and nodes from the substituted subterm to the corresponding nodes of argument $A$ in $M$. By abuse of notation we write $\phi$ to denote its element-wise structure-preserving extension\footnote{Recall that two justified sequences over two distinct terms share the same structure just if their constituting nodes are of the same type, have the same link labels, and same pointer lengths.} over justified sequences of $N$:
$$\phi : \justseqset(N)  \rightarrow \justseqset(M) $$

An easy induction on justified sequences shows that $\phi$ and $\coresymbol$ commute.
...
For any term $M$ we define $\travset^\dagger(M)$ as the subset of traversals of $\travsetnorm(M)$ ending with either a structural node or an external ghost (\ie, either a non-ghost node or a ghost node hereditarily justified by the root). Observe that every traversal in $\travsetnorm$ has a traversal extension in $\travset^\dagger$. Indeed, if $t \in \travsetnorm$ is not in
$\travset^\dagger$ then it must end with an internal ghost node so repeatedly applying rules \rulenamet{Var} and \rulenamet{Lmd} yields a traversal ending with an external node.

Because the core-projection $\coresymbol$ removes internal nodes from a traversal, to prove the result it suffices to show that the desired isomorphism holds for the restricted subset $\travset^\dagger$:
\begin{equation}
...
;
\end{tikzpicture}
\caption{Reduction $M\rightarrow_l N$ where node $x$ has at least one operand, and $A$ is a non-dummy abstraction $\lambda\overline{y} . A'$. Linearly firing  generalized redex $(\lambda x, A)$ for $x$ in $M$ (left) yields $N$ (right) with fresh variables $\overline{y_2}$.}
...
\end{proof}
\todo{TODO: Find alternative proof based on a traversal argument without appealing to termination of leftmost linear reduction for normal forms.}
        ...
        \left[
            x_2 \cdot y_1 \cdot
             \ghostvar^{2^i -1}\cdot y_2
                 \sum_{i\leq j<i} (\ghostvar^{2^j -1} \cdot y_2)
        \right] \ .
\end{equation*}
\end{example}
...
%\todo{Include proof details by adapting Danos-Regnier proof to the definition of \emph{leftmost} linear reduction.}
...
%\todo{Hopefully the proof of (ii) should not involve state machine/stack/environments, ...}
\end{proof}
