...
\emph{Sequences:} For any alphabet $\Sigma$ we write $\Sigma^*$ to denote the set of (possibly infinite) sequences over elements of $\Sigma$.
 ...
 %
 The set of maximal directed paths $\Pi(t)$ of a $\mathcal{L}$-labelled-tree $t$ is the set of sequences over $\mathcal{L}\times \nat$
...
\begin{proposition}
 ...
 $\core{\travsetnorm^{\max}(M)}$ is $\alpha$-consistent.
\end{proposition}
\begin{proof}
...
\paragraph{Soundness of the normalization procedure} It follows from Theorem~\ref{thm:path_charact_ulc} and the fact that a term is uniquely characterized by its set of justified paths $\pathset$ (Property~\ref{prop:tree_path_charact}).  Because the justified paths $\core{\travsetnorm^{\max}(M)}$ are $\alpha$-consistent we can reconstruct from them the beta-normal from using the readout procedure of Proposition~\ref{proposition:termtree_readout_from_justitied_paths}.
