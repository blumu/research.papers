...
Then $T$ induces the labelled-tree $(P,L)$ defined as follows:
\begin{itemize}[nosep]
    \item Directed paths $P \subseteq \nat^*$ are \emph{generated} from justified sequences in $T$: variable occurrences have child index $1$; the child index of lambda-node occurrences is given by their justification label.
    \item Labels are $\mathcal{L} = \ExtendedNodes \times P \times \nat$.
    \item Label function $L : P \rightarrow\mathcal{L}$.
    For $p\neq\epsilon$, let $s = s' \cdot n$ be any sequence in $T$ generating path $p$.
...
The set $\travulc(M)$ of \defname{traversals} of an untyped lambda term $M$, abbreviated $\travulc$ when the term is clear from context, is the set of justified sequences of nodes over $M$ recursively defined by the rules of Table~\ref{tab:trav_rules}.
\end{definition}
...
with the exclusion of rule \rulenamet{Lam^\ghostvar} and the `eta-expanded' sub-cases of \rulenamet{Var} and \rulenamet{IVar}.
\end{definition}
...
Let $\Gamma \entail M : T$ be a simply-typed term in context.
 ...
 $\etalf{M}$ obtained with rules of Table~\ref{tab:trav_rules} with the exclusion of the eta-expanded case in \rulenamet{IVar}.
\end{proposition}
\begin{proof}
...
rule \rulenamet{Lam^\ghostvar} and \rulenamet{Var} essentially calculates the quantity ``$|\alpha|+i-|m|$'' at each iteration, which is the exact same quantity ($r-a+l$) used in step 2. (b) of the PAM transition from \cite{danos-head}.}.
...
The theory of traversals~\cite{OngLics2006} was shown to correspond to Game Semantics in a very concrete way for typed languages like the simply-typed lambda calculus, PCF and Idealized Algol~\cite{BlumPhd}: traversals are in one to one correspondence with the plays from the game denotation of a term where all internal moves are revealed (Theorem~\ref{thm:gamesem_correspondence_stlc}).
We conjecture that a similar correspondence holds for the untyped lambda calculus. More specifically that there is an isomorphism between the set of cores of imaginary traversals from Table~\ref{tab:trav_rules} and the innocent \emph{effectively almost everywhere copycat (EAC)} game denotation of untyped lambda terms~\cite{KerThesis}; as well as an isomorphism between the imaginary traversals
 and the EAC game denotation where internal moves are not hidden during strategy composition. A proof might consist in a similar argument to the STLC case~\cite{BlumPhd} based on EAC strategies instead of innocent strategies. There might exist a connection between the ``on-the-fly'' eta-expansion of imaginary traversals and  the morphism $Fun : U \rightarrow (U \Rightarrow U)$ of Ker's game category where $U$ denotes the maximal game arena.
%%%%%%%%%%%%%%%%%
