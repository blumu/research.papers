...
%%%%%%%
\noindent
\emph{Case $r>1$ (right)} The reduct tree is obtained by removing the @-node and argument $A_1$ and combining the two lambda-nodes into $\lambda\overline{y}$ where we define $\overline{y} = \overline{w} x_2 \ldots x_n$.
%
There is an implicit injection $\Phi$ from nodes of $N$ but $\lambda\overline{y}$ to nodes of $M$.
...
%
%%%%%%%%%%
We show by induction on traversals that $\rightarrow_M$ simulates $\rightarrow_N$ and reciprocally.
We only treat the relevant case here:
A traversal ending with $\lambda\overline{y}$
 of the form $u_1 \cdot \lambda\overline{y} \in \travsetnorm(N)$
 such that $u_1$ is simulated in $M$ by $t_1$,
 is simulated in $M$ by traversal
$t_1 \cdot \lambda\overline{w} \cdot @ \cdot \lambda\overline{x}$.
%
Any further extension of $u_1 \cdot \lambda\overline{y}$ in $N$ is of the form
$u' = u_1 \cdot \lambda\overline{y} \cdot s \cdot z$ where $s$ is the spinal descent in $T$ and $z$ is the \emph{hoc} of $T$.
This gets simulated in $M$ by $t' = t_1 \cdot \lambda\overline{w} \cdot @ \cdot \lambda\overline{x} \cdot \varphi(s) \cdot \Phi(z)$
where $\varphi$ denotes the pointwise-extension of $\Phi$ to justified sequences.
%
If $z$ is internal then since $s$ and $\varphi(s)$ have the same node type and arities, we have $\core{u'} = \core{t'}$.
(i) If $z$ is external and its justifier is not $\lambda\overline{w}$ then
$\Phi(z)$ is also external and therefore $\core{u'} = \core{t'}$.
(ii) If $z$ is external and is justified by $\lambda\overline{w}$ with link label
$j<|\overline{w}|$ then $\Phi(z)$ is necessarily external and $\core{u'} = \core{t'}$.
(iii) If $z$ is external and is justified by $\lambda\overline{w}$ with link label
...
%
We then simulate $u'$ by extending traversal $t'$ in $M$
into $t = t' \cdot (\ghostlmd\, \ghostvar)^k$ for some $k>0$, where
$(\ghostlmd\, \ghostvar)^k$ denotes the argument lookup of $z$ and
where the last $\ghostvar$ points to $\lambda\overline{w}$ with label $j$. We then have $t' \internalextension_M t$  and $\core{u'} = \core{t}$.
%
Reciprocally, take a traversal of $M$ of the form
$\Pstr[10pt]{\ldots
    (l){\lambda\overline{w}}  (a)@  (lx-a,25:0){\lambda\overline{x}} \ldots (gl-a,25:i)\ghostlmd  (gv-l,35:{|\overline{w}|-1+i})\ghostvar }
$ where $i\geq1$.
...
If $i\leq n$ then the the ghost $\ghostvar$ gets materialized into variable occurrence $x_i$ in $N$:
$\Pstr[10pt]{
...
%%%%%%%%%%
    ...
    carries over the list of pending lambdas, the core operation adequately merges nodes $\lambda\overline{w}$ and $\lambda x_1 \ldots x_n$ into
...
\begin{example}
    ...
    is simulated in $N$ by $u = \Pstr[10pt]{ (l){\lambda x z} \, (gv-l,32:2){z} } \in \travsetnorm(N)$.
\end{example}
