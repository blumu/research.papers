...
Traversals for simply-typed languages were previously studied in \cite{BlumPhd}. Given a simply-typed term-in-context $\Gamma \entail M :T$, with typed-context $\Gamma$ (a set of typed variables), and simple type $T$, its set of traversals $\travstlc(\Gamma \entail M :T)$ is a set of justified sequences of nodes from the tree representation of the \emph{eta-long expansion} of $M$ with respect to its type $T$.

The \emph{eta-long normal form} is defined inductively on the type $T$ and is obtained by recursively eta-expanding every subterm as many times as possible with respect to the type of the subterm \cite{Ong2006,BlumPhd}.
