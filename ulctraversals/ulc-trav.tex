...
The accumulating parameter $l = y_1 \ldots y_n$ for some $n\geq0$, is called the sequence of \defname{pending lambdas} (It's technically a sequence of variables but should be interpreted as $\lambda y_1 \ldots y_n$; the $\lambda$ itself is omitted to simplify the above definition.)

We abbreviate $\coresymbol_\epsilon(t)$ as $\core{t}$ and we call it the \defname{core} of $t$.
