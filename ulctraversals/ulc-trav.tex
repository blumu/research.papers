...
We look up the node arities from the tree represented in Figure~\ref{fig:comptree_examples}. The arities of the nodes in the traversal strand are
 $0,1,4,2,2,1,0,0,0,0$ so the dynamic arity is given by the maximum of the intermediate values generated from the summation $(((((0) + -0+0) -0 +1) -2 + 2) -4 +1)$, thus $\dynar(t_\epsilon) = \max \{ 0,0,1,1,-2 \} = 1$.
...
If $M \rightarrow_{ll} N$ then $M$ and $N$ are beta-equivalent.
\end{theorem}
\begin{proof}
...
`\emph{H1: If $M$ has a $\beta$-nf then its leftmost linear reduction sequence terminates.}'
%
...
%($lloc_M(U) \ne \bot$).
    ...
    `\emph{H2: If $lloc_M(T) \neq \bot$ and $hoc(M)$ is in $T$ then there is
     only a finite number of steps in the linear reduction sequence of $M$ that involve a linear substitutions in (reducts of) $T$.}'
%
...
%%%%
    ...
    Suppose that $\emph{lloc}$ is in $T_1$. Then by the induction hypothesis (H2) there is a finite number of steps in the linear reduction sequence of $M$ that involve (reducts of) $T_1$. Let us consider the first reduct $R$ of $M$ in the $\llred$ reduction sequence such that the \emph{lloc} is not under a reduct of $T_1$.  Observe that in the linear reduction sequence of $T$, the subterm $T_2$ remains unmodified as long as the \emph{lloc} remains in (the reduct) of $T_1$.
    %
...
%
There is an implicit injection $\Phi$ from nodes of $N$ to nodes of $M$.
...
a bijection $\varphi$ from $\travset_\normalizing(N)$ to a subset of $\travset_\normalizing(M)$ and show by induction that $\rightarrow_M$ simulates $\rightarrow_N$ via $\varphi$ and reciprocally.
...
Suppose that $u_1 \rightarrow_N u_1 \cdot \lambda\overline{y} = u'$. Then $u' \internalextension_N u_1 \cdot \lambda\overline{y} \cdot s \cdot z = u$ where $s$ is the spinal descent in $T$ and $z$ is the \emph{hoc} of $T$.
...
on $t'_{\leq \lambda\overline{x}}$ adequately generates pending lambdas from variable names $x_2 \ldots x_n$ and discharge them onto node $\lambda\overline{w}$. To proive
that the simulation preserves $\core{\_}$
it suffices to show that occurrences following
$\lambda\overline{x}$ in $t'$ and those following $\lambda\overline{y}$ in $u$ produce the same pending lambdas, and that the last occurrence in the two traversals are either both internal or both external.
%
...
%
We simulate $u'$ by extending $t' \rightarrow^*_M t = t' \cdot (\ghostlmd\, \ghostvar)^k$ for some $k>0$, where
...
Example~\ref{tab:varity2_trav} shows that traversals can easily get very long even for relatively simple terms. This is not surprising given that they were not designed with efficiency nor optimality in mind, but rather derived from the game-semantic analogy.
...
This work got inspired by the Galop workshop slides by Daniil Berezun and Neil D. Jones~\cite{berezunjones_partialevalbytraversals}. It benefited from email communications with the authors and memorable discussions with Neil D. Jones during a visit at Microsoft in May 2017.
