...
From the parity property of traversals, a strand must consist of alternations of lambda nodes and variable/application nodes. We will highlight a strand within a traversal by underlining its first and last occurrence as follows: $ t = \cdots \underline{\alpha_k}\ n_k\ \alpha_{k-1}\ n_{k-1}\ \cdots\ \alpha_2\ n_2\ \alpha_1\ \underline{n_1} \cdots $
...
Let $M$ and $N$ be two terms with variables in $\mathcal{V}$, and $\phi\colon \travsetnorm(N) \rightarrow\justseqset(M)$ be a function
from traversals of $N$ to justified sequences of $M$ such that
$\phi$ maps the initial singleton traversal of $N$ to that of $M$.
%
Let $(X, R_\phi, \rightarrow_X)$ denote the state transition system
over \emph{States} $X = \travsetnorm(M) + \travsetnorm(N)$
with  \emph{Transitions} $t \rightarrow_X t'$
if either $t \rightarrow_M t'$ or $t \rightarrow_N t'$ holds, for $t, t' \in X$, and \emph{Binary relation $R_\phi\subseteq X \times X$} defined for $t, u \in X$ by $ t\,R_\phi\,u$ just if $t\in\travsetnorm(M)$, $u\in\travsetnorm(N)$ and $\phi(u) = t$.

The terms $M$ and $N$ are \defname{$\phi$-bisimilar} if $R_\phi$ is a bisimulation over $X$ with respect to $\rightarrow_X$:
\begin{itemize}[nosep]
    \item[(B1)]
    %for all $t_1, t_2 \in \travsetnorm(M)$, $u_1 \in \travsetnorm(N)$ with
    if $t_1\,R_\phi\,u_1$ and $t_1 \rightarrow_M t_2$ then there is $u_2 \in \travsetnorm(N)$ such that $u_1 \rightarrow_N u_2$ and $t_2\,R_\phi\,u_2$;
    \item[(B2)]
    %for all $t_1 \in \travsetnorm(M)$, $u_1, u_2 \in \travsetnorm(N)$ with
    if $t_1\,R_\phi\,u_1$ and $u_1 \rightarrow_N u_2$ then there is $t_2 \in \travsetnorm(M)$ such that $t_1 \rightarrow_M t_2$ and $t_2\,R_\phi\,u_2$.
\end{itemize}
\end{definition}

The subset $\travsetscn$ of \defname{strand-complete normalizing traversals} of $\travsetnorm$, consists of finite normalizing traversals ending with an external (possibly ghost) variable node.
%
Observe that every finite maximal traversal in $\travsetnorm$ is
necessarily in $\travsetscn$ for if it does not end with an external variable then either rule \rulenamet{App}, \rulenamet{Lmd} or \rulenamet{Var} can extend it depending on the type of the last occurrence, which contradicts maximality.
Consequently the finite maximal traversals of $\travsetnorm$ are precisely the  maximal traversals of $\travsetscn$.
...
So in particular, two almost equal sequences have equal prefix up to the
their last external variable occurrence; and thus in $\travsetscn$, `almost-equality' coincides with `equality`.
%
A function $\phi\colon J\longrightarrow\justseqset(M)$
defined on some $J\subseteq \justseqset(N)$
is \defname{almost the identity} if we have $u \almostequal \phi(u)$ for all $u\in J$.
%
Two subsets $J_1\subseteq \justseqset(M)$, $J_2\subseteq\justseqset(N)$ are \defname{almost equal}, written $J_1\almosteqisomorphic J_2$, if there exists a bijection $\phi \colon J_1\longrightarrow J_2$ such that $j\almostequal\phi(j)$ for all $j\in J$.


\begin{lemma}
\label{lem:bisimulation_isomorphism}
If $M$ and $N$ are $\phi$-bisimilar
and the restriction of $\phi$ to $\coresymbol(\travsetnorm(N))$
 is almost the identity,
 is injective,
 and commutes with core projection $\coresymbol$
then $\coresymbol(\travsetnorm(N)) \almosteqisomorphic \coresymbol(\travsetnorm(M))$, and thus
$\coresymbol(\travsetscn(N)) \structisomorphic \coresymbol(\travsetscn(M))$.
\end{lemma}
\begin{proof}
Consider $\phi$ as a function
 $\coresymbol(\travsetnorm(N))\longrightarrow\coresymbol(\travsetnorm(M))$.
It is \emph{well-defined}: by definition, $\travsetnorm(N)$ is the transitive closure of the singleton traversal by $\rightarrow_N$, so for any traversal $u$ of $\travsetnorm(N)$, repeatedly applying (B2) for each step of its $\rightarrow_N$ derivation shows that $t = \phi(u) \in \travsetnorm(M)$, and the commutativity of $\coresymbol$ and $\phi$ gives $\coresymbol(t)=\phi(\coresymbol(u)) \in \coresymbol(\travsetnorm(M))$.
Similarly, it is \emph{surjective} by (B1) because $\travsetnorm(M)$ is the transitive closure of the singleton traversal by $\rightarrow_M$. Finally it is \emph{injective} by assumption. Hence we have the desired isomorphism.
Finally, for strand-complete traversals, almost equality coincides with equality.
\end{proof}
...
If $M \llred N$ then $\coresymbol(\travsetnorm(M))
\almosteqisomorphic \coresymbol(\travsetnorm(N))$.
\end{proposition}
%\shortproof
%\endshortproof
%\begin{proof}
...
The linear substitution affects the term tree in three possible ways
 depending on the arity of the \emph{lloc} and the structure of the argument. For each one we show that there is an injection $\phi\colon\travsetnorm(N)\longrightarrow \justseqset(M)$ such that $M$ and $N$ are $\phi$-bisimilar:
...
%%%%\begin{itemize}
%%%% \item
The restriction of $\phi$ to $\coresymbol(\travsetnorm(N))$
coincides precisely with the element-wise structure-preserving
extension of $\Phi$ over justified sequences.
This is because $\phi_1$ only alters justification pointers of internal nodes $@_x$ and $\lambda\overline y$, and $\phi_2$ only inserts internal nodes, while justified sequences in $\coresymbol(\travsetnorm(N))$ do not contain any external node.

%%%% \item
The restriction of $\phi$ to $\coresymbol(\travsetnorm(N))$, is injective.
...
Suppose that $\coresymbol(u)\neq\coresymbol(v)$ for $u,v\in\travsetnorm(N)$ then we must have $u\neq v$.
Let $s$ denote the longest common prefix of $u$ and $v$; then the next two nodes $n_1, n_2$ immediately following $s$ in $u$ and $v$
are necessarily traversed by the non-deterministic rule \rulenamet{IVar}.
Hence they must both be external nodes and therefore they are not eliminated after applying $\coresymbol$: the last occurrence of $\coresymbol(s\cdot n_1)$ and $\coresymbol(s\cdot n_2)$ are different. Since $\Phi$ maps distinct external sibling nodes in $N$ to distinct sibling nodes in $M$, we necessarily have
$\phi(\coresymbol(s\cdot n_1))\neq \phi(\coresymbol(s\cdot n_2))$.
%%%%%%%%%

%%%% \item
Since $\Phi$ preserves $\lambda$-node labels, it also preserves arities, hence $\coresymbol$ involves the same pending lambdas when applied to either $u$ or $\phi(u)$, hence $\coresymbol$ and $\phi$ commute.
%%%%\end{itemize}

We now show that $M$ and $N$ are in $\phi$-bisimulation.
For $(B1)$ and $(B2)$, given $u\in \travsetnorm(N)$ and $t\in\travsetnorm(M)$
such that $\phi$ is well defined for $u$ and $t \doteq \phi (u)$
and $m, n$ denote respectively the last node in $t$ and $u$.
We prove both directions by induction on
and case analysis on the last traversal rule used to
...
{\bf Case 2}
%\item[Case 2]
 where $x$ has at least one operand ($|x|>0$) and $A$ is an application ($|\overline{y}|=0$). The root lambda node of $A$ is therefore a dummy lambda: $A = \lambda. R$. Let $r$ denote the root node of $R$, which is either a variable if $R$ is in head-normal form, or an $@$-node otherwise.
...
\label{prop:ulctrav_sound_for_trivialreduction}
    Let $M$ be an untyped term containing only trivial redexes, and $N$ the reduct obtained by firing the \emph{leftmost spinal-innermost} trivial redex, then $\coresymbol(\travsetnorm(M)) \almosteqisomorphic \coresymbol(\travsetnorm(N))$.
\end{proposition}
%\proofatend
\begin{proof}
...
If $M$ is in \emph{qnf} and has beta-normal form $N$ then $\coresymbol(\travsetnorm(M)) \almosteqisomorphic \coresymbol(\travsetnorm(N))$.
\end{corollary}
\begin{proof}
...
the set $\core{\travsetnorm(\_)}$ is an invariant (up to $\almosteqisomorphic$-isomorphism) throughout this trivial reduction sequence.
\end{proof}
...
$\core{\travsetnorm^{\max}(M)} \, \structisomorphic\, \pathset^{\max}(T)$.
\end{theorem}
\begin{proof}
...
shows, that since \rulenamet{Var} is never used we necessarily have  $\travsetnorm(M) = \pathset(M)$. Since all nodes are externals the projection $\coresymbol$ coincides with the identity thus $\core{\travsetnorm(M)} = \travsetnorm(M) = \pathset(M)$.
%
If $M$ is not normal form then since $M$ has a beta-nf, by Theorem~\ref{thm:completeness_leftmostlinearred} its \emph{leftmost linear reduction sequence} terminates and yields a \emph{qnf}. By Proposition~\ref{prop:ulctrav_impl_linear_reduction} the set of core of maximal traversals is preserved by \emph{linear reduction} (up to an isomorphism) thus the set of core of maximal traversals of the \emph{qnf} is isomorphic to the core of maximal traversals of $M$. Corollary~\ref{cor:qnf_and_nf_traveset_invariant} permits us to conclude.
\end{proof}

\begin{proposition}
    \label{prop:core_travsetnorm_alpha_consistent}
    $\core{\travsetscn(M)}$ is $\alpha$-consistent, and therefore so
    is
    $\core{\travsetnorm^{\max}(M)}$.
\end{proposition}
\begin{proof}
A traversal of $\travsetscn$ is uniquely defined by the sequence of non-deterministic choices of the argument index $i$ made at the end of each strand. Further, by \ref{thm:path_charact_ulc}, this sequence of indices also determines the path to the corresponding node in the tree of the normal form.
Hence for any two traversal $t_1,t_2 \in \travsetscn$
ending with strands $\ldots  \lambda\overline{x}_1 \ldots y_1$ and
$\ldots \lambda\overline{x}_1 \ldots y_1$.
If the sequence of argument indices leading to $\lambda\overline{x}_1$ and $\lambda\overline{x}_2$ are not the same then both occurrence refer to different node of the tree of the normal form.
Otherwise, if the argument indices are the same, then all the non-deterministic choices leading to $\lambda\overline{x}_1$ and $\lambda\overline{x}_2$ are the same therefore $t_{\leq \lambda\overline{x}_1} = t_{\leq \lambda\overline{x}_2}$, which by the deterministic rules extends to $t_1 = t_2$.
...
\paragraph{Soundness of the normalization procedure} It follows from Theorem~\ref{thm:path_charact_ulc} and because a term is characterized by its justified paths $\pathset$ (Property~\ref{prop:tree_path_charact}). Because the justified paths $\core{\travsetnorm^{\max}(M)}$ are $\alpha$-consistent we can reconstruct from them the beta-normal from using the readout procedure of Proposition~\ref{proposition:termtree_readout_from_justitied_paths}.
