...
input-variable rule \rulenamet{IVar} is restricted to be the last occurrence of the traversal ($m=n$) so that the traversal produced is $\Pstr[0.5cm]{t \cdot (n){n} \, (l-n,25:i){\alpha}} \in \travsetes$.
...
\defname{concrete and essential} traversals.
That is the set of traversals $\travsetes \inter \travsetcon$ obtained by imposing the above restriction on \rulenamet{IVar} and additionally restricting  it to its concrete sub-case (Definition~\ref{def:concrete_traversals}).
\end{definition}
