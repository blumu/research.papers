...
The ``on-the-fly'' eta-expansion of imaginary traversals relates to the $Fun : U \rightarrow (U \Rightarrow U)$ morphism of Ker's game category where $U$ denotes the maximal arena.
\end{proof}
...
This suggests a normalization method for STLC term based on enumeration of all traversals, with one caveat: the set of STLC traversals can be infinite! This is because the rule traversing free variables has so much non-determinism that it gives rise to arbitrarily long traversals. But since the above bijection holds for the quotiented traversal set, one does not actually need to enumerate all traversals: it's sufficient to enumerate a $\sim$-complete subset. Such a normalization procedure is described in Algorithm \ref{algo:stlc_normalization_by_traversals}.
...
\subsection{Restricting non-determinism}

All the traversal rules are deterministic except the one used to model free variables: The rule \rulenamet{IVar} offers two non-deterministic choices:
\begin{itemize}
\item[(i)] which variable to pick in the O-view,
\item[(ii)] which child lambda node to pick amongst the picked variable's children.
\end{itemize}
...
\paragraph{Quotienting traversals}
The Game Semantics Correspondence explains why there can be infinitely many traversals: The game denotation of a term must allow Opponent's moves accounting for all possible context terms (modelled by the Opponent).

In particular, suppose that $M$ takes a second-order function as argument. Then one can define an infinite sequence of term contexts that would pass as argument to $M$ a function that calls its argument exactly $k$ times for $k>0$ (\eg, $\lambda g . g ( g(\ldots (g z)))$ with $k$ applications of $g$).

It turns out that traversals accounting for all those contexts are redundant in the sense that they all belong to the same $\sim$-equivalence class. Indeed, due to the absence of side-effects, calling the same argument multiple times always involves the same underlying computation in $M$. Proposition \ref{prop:branching_traversal_normalizing} formalizes this intuition by showing that the subset of \emph{branching traversals} prevents such repetitive behaviour while still covering all $\sim$-equivalence classes.
...
We define the set of \defname{normalizing traversals}, noted $\travsetnorm$ as the subset of branching traversals defined by the rules of Def.~\ref{dfn:branching_traversals} where the label $k$ in rule \rulenamet{IVar} is bounded by the arity threshold of the traversal:
