...
The set $\travset(M)$ of \defname{traversals} over an untyped lambda term $\tau(M)$ is a justified sequence of elements in $\Nodes + \ghostlmd + \ghostvar$ with associated pointers. It is defined by induction over the rules of Table \ref{tab:trav_rules}.
...
We use the standard game-semantic operations for justified sequences \cite{Abr02}. In particular we refer to the P-view
of a traversal, denoted $\pview{t}$, as the sub-sequence obtained by reading the traversal backwards as follows: and alternatively (i) reading the preceding two occurrences of  (i) if the node being read is a variable node then keep that node and follow its justification pointer (\ie skip  all the occurrences ``underneath that pointer'') (ii) if the node is a lambda node then keep that node and move to the preceding node. The O-view, denoted $\oview{t}$, is defined as the dual of the P-view.
...
\begin{itemize}[]
    \item \rulenamet{Lam} If $t \cdot \alpha$ is a traversal where $\alpha\in\NodesLmd$ then so is
        ...
        \begin{compactitem}
            ...
            \item a free variable node in $\Nodes_{\sf var}$  then it points
            to the only occurrence of the root in
            $\pview{t \cdot \alpha}$,
            \item a bound variable in $\Nodes_{\sf var}$ then it points to the only occurrence of its binder in $\pview{t\cdot \alpha}$.
        \end{compactitem}
...
\begin{example}[Walk-through example ``${\sf add}\ 1$'']
