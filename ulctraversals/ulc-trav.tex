...
Observe that rule $\rulename{IVar^\eta}$ leaves infinitely many choices for the link label: any value greater than $1$. The following result shows that, for calculating paths in the tree representation of the beta-normal form of the term, it suffices to consider labels that are stricly smaller than the arity threshold.

\begin{property}[Weaving]
\label{prop:weaving}
Let $t \in \travsetbr$ be a branching traversal ending with a variable node hereditarily justified by the root. Let $2k$ be the length of the weave ending at $t^\omega$ for $k\geq1$. Suppose $t_{max} \in \travsetbr$ is a maximal traversal extension of $t$, in which case by rule $\rulename{IVar^\eta}_\branching$, the node immediately after $t$ is a ghost lambda node justified by $t^\omega$ with label $i\geq 1$. Then:
...
Since $i_{q-1} > |n_q|$, by the rule \rulenamet{Var^\eta_\branching} the next node is necessarily a ghost lambda node justified by $n_q$ with label $i_q$. With rule \rulenamet{Lam^\ghostlmd_\branching} the following node is a ghost variable node $\ghostvar$ justified by $\alpha_q$ and labelled by $i_q = i_{q-1} + |\alpha_{q-1}| -|n_{q-1}|$.
\end{itemize}
...
The inductive rules defining $\travsetnorm$ are recapitulated in table \ref{tab:normalizing_trav_rules}. Note that in this table, the rules \rulenamet{IVar^\eta} and \rulenamet{Var^\eta} are respectively merged with \rulenamet{IVar} and \rulenamet{IVar} for conciseness.
...
\infrule[$\rulefont{App}_\normalizing$]
    {t \cdot @ \in \travsetnorm \andalso \alpha\in\NodesLmd \andalso @ \enables_0 \alpha }
...
\infrule[$\rulefont{Lam}_\normalizing$]
     ...
     {t \cdot \alpha \cdot n \in \travsetnorm
      \andalso
      \parbox[t]{5cm}{\footnotesize
            \mbox{where $n$'s pointer}
            \begin{compactitem}[-]
                \item $n=@$: is undefined;
                \item $n\in\Nodes_{\sf fv}$: points to the initial occurrence of $t$,  is labelled by the index of the variable name;
                \item $n \not\in \Nodes_{\sf fv}$: points to the only occurrence of $n$'s binder in $\pview{t\cdot \alpha}$, is labelled by the binding index of the variable name.
            \end{compactitem}
            }
      }
...
\infrule[$\rulefont{Lam^\ghostlmd}_\normalizing$]
    {
        \Pstr[0.5cm]{ t \cdot(alpha){\alpha} \cdot (m){m} \cdot \ldots \cdot (gl-m,40:i){\ghostlmd} \in\travsetnorm}
        \andalso  \alpha \in \NodesLmd^\ghostlmd
        \andalso m \in\NodesVar^\ghostvar
    }
    {
        \Pstr[0.5cm]{ t \cdot(alpha){\alpha} \cdot
        (m){m}
...
where necessarily $n \in \NodesVar^\ghostvar \union \Nodes_{@}$ and $\alpha,\beta \in \NodesLmd^\ghostlmd$.
...
\item What should be the value of $q$? In other words, how many more $k$ do we need to look at?  The answer: we need to keep iterating on $k$ until the point where applying the traversal rules will only produce ghost variables and ghost lambda-nodes! Because there is a finite number of nodes in the computation tree, the variable node arities are bounded. Therefore for high enough argument indices $k$, the condition $i\leq|n|$     in the definition of \rulenamet{Var} will never be met:
    ...
    = \max \{   & 0 , \\
     ...
     = 1
     \end{align*}
     Thus we don't need to look at higher value of $k$ at $t_0$, we thus have:
