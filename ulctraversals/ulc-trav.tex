...
Their traversals work on the traditional term tree where each node represents a token from the lambda calculus syntax,
...
Their `read-out' procedure to reconstruct the normal form (shared with the author in private correspondence) involves recursively applying rewriting rules to the traversal. In contrast, our read-out algorithm reconstructs the normal form from the tree-paths generated from maximal traversals.
...
\begin{todobox}
    ...
    %
    Hyperlazy does not mean efficient: linear substitution ends up repeating same things several times. However, no useless work is done, unlike the traditional substitution. Could we recover optimality by adding some notion of caching to traversals?
...
occurrences of nameless variables bound by either a structural or a ghost lambda node.

We define ghost nodes by simultaneously and recursively
extending the tree's parent-child relationship and the enabling relation $\enables$: for every (ghost) variable or @-node $n$ with parent (ghost) lambda-node $\alpha$, and for all $k>|n|$, we introduce a ghost variable node $\ghostvar$ defined as the unique child of
ghost lambda node $\ghostlmd$, itself defined as the $k$th-child of $n$;
and we extend the enabling relation with $n \enables_k \ghostlmd$
and $\alpha \enables_{|\alpha|+ k} \ghostvar$; where by convention ghost nodes have arity $0$, and the arity of structural nodes remains unchanged (\ie, given by their arity in the structural tree without ghost nodes).
This extended enabling relation induces a binding relationship between ghost variables and their (possibly ghost) lambda-node binder, which lets us forgo the notion of variables names in ghost nodes: we will thus
use respectively $\ghostlmd$ and $\ghostvar$ as a unique label for all ghost lambda and variable nodes.

We call the infinite tree hereby defined the \defname{extensional tree} of $M$,
denoted $\exttree(M)$ over the \defname{extended nodes} $\ExtendedNodes$,
consisting of structural nodes of $\Nodes$ plus all ghost variables $\ghostvar$ and lambda nodes $\ghostlmd$ as defined above.
 We use the shorthands $\ExtendedNodesLmd$ for the subset of lambda nodes (structural in $\NodesLmd$ or ghosts $\ghostlmd$) and $\ExtendedNodesVar$ for variable nodes (structural $\NodesVar$ or ghosts $\ghostvar$).
