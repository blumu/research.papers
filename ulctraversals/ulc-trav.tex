...
The theory of traversals was originally introduced in the context of higher-order recursion schemes~\cite{Ong2006} as tree generators, and was
...
Each traversal explores a branch in the tree representation of the normal form. Its underlying sequence of nodes and pointers encodes the unfolding of the normalization procedure for that branch. There are two kinds of nodes: the \emph{internal} nodes record intermediate steps of the reduction procedure, while \emph{external} nodes represent actual nodes from the tree representation of the normal form. The core projection of a traversal effectively removes all its internal nodes, which yields a path in the abstract syntax tree of the normal form.

This procedure contrasts with term rewriting approaches that gradually transform and refine some representation of the term until reaching a normal form. In particular, it does not involve the ``capture-avoiding'' substitution used in the standard definition of beta reduction to avoid variable capture when reducing redexes. Other reduction methods exist in the literature with the same property such as combinators, Lamping's graphs, or their variant based on the geometry of interaction~\cite{Lamping:1989:AOL:96709.96711, Gonthier:1992:GOL:143165.143172, curry_combinatorylogic, Barendregt84, the-implementation-of-functional-programming-languages}.
Just like those methods, variable names are not essential to the reduction procedure: traversals have explicit justification pointers from which variable bindings are entirely recoverable; variable names in Lamping's graphs are recoverable from their corresponding binding lambda; and combinators removes variables altogether through \emph{abstraction elimination}.
Just like graph-based reduction, the traversal reduction procedure involves a final ``read-back'' phase to reconstruct the resulting normal form: all the explored tree branches are assembled together to form the tree representation of the normal form. Those other reduction methods, on the other hand, require transforming the term into some intermediate structure, such as a combinator expression or an interaction net, whereas traversals operate directly on the abstract syntax tree of the term. On the other hand, unlike Lamping's graphs, traversal-based reduction is not optimal in Levy's sense~\cite{optimal-reduction-levy}.


%The closest related reduction device would be the Pointer Abstract Machine of Danos \etal~\cite{danos-head,danosherbelinregnier1996}
...
The key idea behind imaginary traversals lies in a judicious use of eta-expansion. In the absence of types, eta-long expansion is not feasible as it would yield an infinite term. Indeed, attempting to traverse an untyped lambda term using the original traversal rules can cause the traversal to get stuck. This happens when the operator sub-term of an application has an insufficient number of operands. In such situation, there is no operand node in the term tree that the traversal can jump to.
Take the untyped term $(\lambda f.f)(\lambda x.x)$ for example. Traversing it using the rules of STLC gives the sequence of nodes $\lambda f \cdot f \cdot \lambda x \cdot x$ (where pointers are omitted), at which point the traversal is stuck: the variable rule cannot be used since there is no subterm representing the first argument of the variable occurrence $f$.
...
unblocks the traversal, until it reaches node $z$, at which point we have a maximal traversal per the STLC rules. The final traversal of the eta-expanded term is given by the sequence $t = \lambda z \cdot @ \cdot \lambda f y \cdot f \cdot \lambda x \cdot x \cdot \lambda \cdot y \cdot \lambda \cdot z$, where `@` represents the application node in the AST, and the dummy lambda `$\lambda$' is syntactic sugar which can be ignored for now.

Generalizing this idea, one can infer the implicit `eta-expansion strategy' of untyped traversals from the traversal rules of STLC: at any given point where a traversal get stuck, among all possible subterms to eta-expand, only one creates the additional nodes required to extend the traversal with the STLC traversal rules.
...
which an STLC traversal cannot traverse since $f$ does not have an immediate argument. Presented in a more intuitive way: at $x$ the traversal has already figured out that the subterm $\lambda x.x$ should be substituted for variable $f$, therefore to determine what $x$ should be substituted for, it needs to look for the first argument of $f$. Since $f$ has no direct argument, it must be eta-expanded.

Observe that in this simple example, the nodes visited after unblocking the traversals are all fictitious nodes not present in the original term. In some situations, however, a traversal may jump from a fictitious node back to a  node from the original term. The term $(\lambda u . u\ (y_1~u)) (\lambda v . v~ y_2)$ studied in Example \ref{ex:missingoperand} illustrates this phenomenon.

% The reader may wonder why would , be wondering why would the traversal get stuck at this point since, for comparison, an abstract machine
% %TODO \cite{}
% It might be surprising at first
% For comparison with standard evaluation of lambda terms, observe that an abstract machine such as a Krivine machine, which like

In practice, one does not need to actually eta-expand an untyped term when traversing it; the expansion occurs fictitiously and ``on the fly'' as the traversal unfolds. An untyped traversal, however, bares sufficient traces of those successive fictitious eta-expansions for one to reconstruct the term effectively being traversed.

The free variable rule is responsible for implementing the on-the-fly eta expansion. Just like STLC, it has an unbounded `arity index', chosen non-deterministically, defining which argument of the variable to visit next.
In ULC, however, that number is unbounded: if it is greater than the number of arguments actually present in the term then a fictitious eta-expansion takes place.

Normalizing an untyped term (with a beta-normal form) then boils down to  enumerating its imaginary traversals. Although this set may be infinite, we only need to consider an effectively computable subset with sufficiently enough traversals to cover all paths of the tree representation of the beta-normal form. We achieve this by restricting the non-determinism of the free variable rule: the arity index gets bounded a critical quantity called ``arity threshold''  calculated in linear time from the traversal itself.
...
The read-out phase of the algorithm appeals to a path-correspondence property of traversals to extract tree paths of the normal form the traversals. In the above example, traversal $t$ yields the path $\lambda z \cdot z$. And since the term has a single traversal, its beta-normal form is $\lambda z. z$, up to variable renaming.
 ...
 We have implemented the normalization procedure in HOG, a tool used to study traversals and pushdown automata generated from Higher-Order Grammars and lambda terms~\cite{BlumGalop2008,Blum-HogTool}. The last section provides examples of untyped lambda terms that were normalized using it.
...
\begin{todobox}
This paper is about reducing lambda terms, so I was surprised that the related work section only discussed a very narrow line leading to the results of the paper.
As mentioned above, environment machines and other evaluation methods could be at least mentioned (and saying why this is different).
-	But there are other topics that are much closer: geometry of interaction for example shares some of issues that are discussed here, but in that approach a token traverses the structure of terms (both typed and untyped) and eta-expansion of the term is not needed.
-	There is some interesting comparison to be done here I believe, but was a little surprised that nothing was mentioned.
\end{todobox}


\subsubsection{Game Semantic Theory of Traversals}

The original paper on the theory of traversals concerns higher-order recursion schemes and therefore does not deal with free variables of higher-order rank~\cite{Ong2006}. In~\cite{BlumPhd} we extended the theory to the simply-typed lambda calculus and added traversal rule to model free variables. Both settings assume that the term or grammar is in eta-long form to guarantee that the operand of an application always exists.
...
The present paper achieves this by eta-expanding subterms `on-the-fly' when there is insufficiently many operands in an application to continue the traversal. The term itself does not get altered: a traversal instantiates fictitious ``ghost'' nodes that would exist if the term were to be eta-expanded. We extend traversal rules to handle those fictitious nodes created in these ad-hoc eta-expansions. When restricted to simply-typed terms, the two notions of traversal conveniently coincide (Proposition~\ref{prop:ulc_and_stlc_trav_coincide}).

STLC traversals were shown to correspond to the \emph{interaction game semantic denotation} of a term, where all internal moves from strategy composition are revealed; while the \emph{projection with respect to the tree root} operation, yields a correspondence with the standard Game Semantics~\cite{BlumPhd}.
This induces a method to evaluate simply-typed lambda-terms, which was alluded to in~\cite{BlumPhd,BlumGalop2008,Blum-HogTool,Ong-NormByTrav2015} and implemented in~\cite{BlumGalop2008, BlumPhd}. The present paper states the STLC evaluation procedure in Algorithm~\ref{algo:ulc_normalization_by_traversals} and generalizes it to the untyped lambda calculus.
Projecting traversals is slightly more complex than for STLC as it involves relabelling of the lambda nodes after filtering. A notable improvement is that it produces the traditional beta-normal form rather than the beta-\emph{eta-long} normal form. Another difference is that we prove soundness of the procedure through a connection with the notion of head-linear reduction rather than appealing to Game Semantics.
...
Traversals are essentially a recursive implementation of the head linear reduction strategy on lambda terms. A traversal starts, in a call-by-name-like fashion, with a depth-first search for what Danos-Regnier call the \emph{head occurrence} of the \emph{hoc redex}, and implements linear substitution by ``jumping'' to the node in the tree representing the sub-term to be substituted for that head occurrence. It then continues its descent to look for the next head occurrence, as if that first head occurrence had been replaced by the redex argument. A property of head-linear reduction is that all the terms appearing in the reduction sequence consist of sub-terms of the original term. This property means that there is no need for the traditional notions of environments or closures to reduce beta-redexes. In traversals, such mechanisms are replaced by a system of pointers referencing subterms of the original term.

The present paper formalizes this generalization with the notion of \emph{leftmost linear reduction} strategy that recursively applies head linear reduction; just like the normal reduction from the lambda calculus literature  proceeds by recursively applying head reduction.
...
The next section introduces basic definitions from the traversal theory. Section~\ref{sec:imaginary_traversals} defines the notion of \emph{imaginary traversals} for the untyped lambda calculus. We present in Section~\ref{sec:normalizing_trav} the reduction procedure for untyped lambda terms, and walk through it on examples in Section~\ref{sec:examples}. In Section~\ref{sec:leftmostlinearred}, we present \emph{head linear} and \emph{leftmost linear reduction}. Finally, Section~\ref{sec:correctness_ulc_normalization} shows correctness of the normalization procedure.
...
We consider the set of labels
$\mathcal{L} = \{ @ \} \union \VarSet \union \{ \lambda x_1 \ldots x_n \ | \ x_1 ,\ldots, x_n \in
    \VarSet, n\in\nat \}$.
The \defname{computation tree} $\ctree(M)$ is a $\mathcal{L}$-labelled tree defined inductively on the syntax of $M$:
\begin{align*}
...
\end{figure}
\begin{todobox}
    -	Is there any relation with Bohm trees that could be said at this point also?
\end{todobox}

\begin{todobox}
    -	 I found the paper a bit repetitive in places, for example, we are  told many times about eta-long forms.
...
Note that any lambda term can indeed be written in one of the two forms above. In particular, applicative terms are handled by the case $n=0$ of the form $\lambda . N$ for some term $N$ where `$\lambda$' is referred to as a ``dummy lambda''. One property of this compact tree representation is the alternation between lambda nodes and variable nodes at odd/even levels.
...
We define \defname{hereditarily enabling} as the reflexive transitive closure of the enabling relation, and we say that node $m$ \defname{hereditarily enables} $n$ written $m \enables^* n$.
...
A \defname{justified sequence} is defined a sequence of (extended) nodes from the computation tree where every occurrence $n$ in the sequence, except the first one, has an associated link--the ``justification pointer''--pointing to some previous node occurrence $j$ in the sequence--its ``justifier''--with an associated ``link label'' $l\geq0$, such that the justifier node $\enables$-enables the source node with the corresponding label. That is: $j \enables_l n$. We represent justified sequences as a sequence of labelled node occurrences with back-pointing arrows representing justification pointers:
...
{\bf Name-free encoding}: The \defname{structure} of a justified sequence refers to the sequence obtained by discarding node labels while preserving node types. It is encoded as a sequence of triples $\{\lambda, {\sf Var}, @ \}\times\nat\times\nat$ where the first component indicates the type of each occurrence (lambda bunch, variable, application), the second component indicates the link distance, and the third component is the link label ($0$ for no pointer).
Two sequences $s$ and $u$ over possibly two distinct terms are \defname{equivalent}, written $s \equiv u$ just if they have the same structure: the types of the underlying nodes in the two sequences match pairwise (either two variable/@ nodes or two lambda nodes) and have the same justification pointers. Two equivalent sequences are not necessarily equal since they may have different variable names.
...
The \defname{quadruplet encoding} of a justified sequence of nodes with variables in $\VarSet$ is a sequence of quadruplets in $O_4 = \nat^*\times \mathcal{V}^* \times\nat\times\nat$ defined occurrence-wise where each occurrence is encoded by the following quadruplet:
\begin{enumerate}[nosep]
\item \emph{Node path}: Path to the extended node in the tree that uniquely defines the type and label of the occurrence;
\item \emph{Pending lambdas}: The empty list $\epsilon$;
\item \emph{Link distance}: The number of occurrences separating the encoded occurrence from its justifier;
\item \emph{Link label}: The justification pointer's label.
\end{enumerate}
...
\emph{Decoding a quadruplet sequence} When interpreting a quadruplet sequence as a justified sequence, the variables from the `pending lambda' get appended to the list of variables bound by a lambda node. When typesetting a sequence from $\justseqset(M)$, we represent occurrences by their associated node labels and represent the \emph{pending lambdas} in exponent. For example in the sequence $\cdots @ \cdot x \cdot \lambda\overline{x}^{[y_1 \ldots y_n]}$, $\lambda\overline{x}$ is the label of the tree node associated with that occurrence and $y_1 \ldots y_n$ are the \emph{pending lambdas} for this occurrence. We omit the exponent if the list of pending lambdas is empty.
...
We will use non-canonical encodings to implement $\beta$-reduction on  justified sequences, where the \emph{pending lambda} component helps
 merge two lambda nodes in a combined bunch lambda node, a process by which a ghost variable may materialize into a structural node by getting assigned a variable name.
...
Let $n$ be some occurrence of a justified sequence $s$. The \defname{projection} $s\filter n$ of $t$ with respect to $n$ is the subsequence obtained by keeping only occurrences hereditarily justified by $n$ in $s$. For a subset $A$ of nodes in $\Nodes$ we write $s \filter A$ to denote the subsequence of $s$ consisting of occurrences of nodes in $A$ only.
...
For any term $M$ we define the \defname{set of justified paths} $\pathset(M)$ as the set of justified sequences in $\justseqset(M)$ whose underlying sequences of nodes are the structural paths (\ie, without ghosts) in the labelled-tree $\ctree(M)$ and their justification pointers are induced by the enabling relation as follows: Occurrence of bound variable nodes are justified by their binder node with link label determined by the variable index in the binding node; free variables are justified by the root (the initial node) with label index determined by the free variable index; and lambda nodes are justified by their parent (the immediate predecessor in the sequence) with label index given by the child index. This notion is well defined because a variable binder necessarily occurs in the path from it to the root.
...
(ii) Further $M$ is uniquely determined, up to $\alpha$-conversion, by the \emph{structure} of the maximal justified paths in $\pathset$.
\end{property}
\begin{proof}
...
\begin{itemize}[nosep]
    ...
    If $n$ has no justifier then $L(p) = (n, p, 0)$.
    ...
    \end{itemize}
\end{definition}
It is well-defined because sequences in $\mathcal{P}$ correspond to actual paths in the tree of $M_2$, therefore any two sequences $s_1, s_2$ generating the same path $p$ must end with the same tree node with same justifier.

\begin{algorithm}[!ht]
\begin{algorithmic}
\caption{Reconstructing a term tree from justified paths.}
\label{algo:termtree_readout_from_justitied_paths}
\REQUIRE{Set $\mathcal{P}$ of justified sequences of a term $M$ verifying $\mathcal{P} \equiv \pathset(N)$ for some term $N$.}
...
\begin{itemize}[leftmargin=3em]
    ...
    \item[\rulenamet{App}] If $t \cdot @$ is a traversal, so is \Pstr[0.4cm]{t \cdot (at) @  \cdot (a-at,40:0) \alpha} where $\alpha\in\NodesLmd$ is $@$'s $0$th child $\lambda$-node.
...
\item[\rulenamet{Var}] If \Pstr[0.5cm]{t \cdot (m){m} \cdot (alpha){\alpha}
    \ldots (n-alpha,50:i){n} \in \travulc} for $i>0$, $m \in\ImNodesVar \union \NodesApp$, and internal nodes $\alpha \in \ImNodesLmd$ and $n \in \ImNodesVar$ then:
  $$\Pstr[0.5cm]{ t  \cdot
...
Some immediate properties that can be shown by induction on the traversal rules:
\begin{property}
\label{prop:trav_immediate}
Let $t$ be a traversal. Then $t$ and $t\filter\ExtNodes$ are valid
justified sequences
%(with respect to enabling relation $\enables$)
verifying the alternation condition;
and if the last node in $t$ is external then so are all the nodes in $\oview{t}$. % By induction on \oview{t}
\end{property}

\paragraph{On-the-fly expansion}

The definition of the rules reveal what we mean by `on-the-fly $\eta$-expansion': it occurs in variable rules \rulenamet{Var} and \rulenamet{IVar} when the arity of a variable is too low to statically determine the operand of an application. When the variable arity is high enough, the definition of the rule coincides with that of STLC, where the tree statically dictates the node to traverse next.

Rule \rulenamet{Lam} traverses a lambda node by just visiting its unique child node in the tree. No such child node exists for a ghost lambda, so instead rule \rulenamet{Lam^\ghostlmd} traverses a ghost lambda by visiting the imaginary ghost variable that would be created if we were to eta-expand the sub-term under that lambda: the ghost placeholder $\ghostvar$ represents an occurrence of the $j$th variable bound by lambda node $\alpha$ if the sub-term at node $\alpha$ were eta-expanded $i-|n|$ times; hence $j = |\alpha| + i - |n|$.
...
Let $t$ be a \emph{finite} traversal of $M$. The \defname{eta-expansion of $M$ with respect to $t$}, written $M^t$, is defined inductively as just $M$ if $t$ is empty; and if $t = u \cdot n$ by case analysis on the rule used to traverse $n$, where $m$ denotes $n$'s justifier in $u$, if it exists, and $i$ its justifier's label:
$$
\begin{array}{rlcrll}
...
\begin{example}
The eta-expansion of $(\lambda u . u(y_1\,u)) (\lambda v . v\,y_2)$--see its computation tree in Example~\ref{ex:missingoperand}--with respect to traversal $\Pstr[0.7cm]{(n0){\lambda }\,(n1){@}\,(n2-n1){\lambda u}\,(n3-n2){u}\,(n4-n1){\lambda v}\,(n5-n4){v}\,(n6-n3){\lambda }\,(n7-n0,35){y_1}\,(n8-n7){\lambda }\,(n9-n2,27){u}\,(n10-n1,27){\lambda v}\,(n11-n10){v}\,(n12-n9){{\ghostlmd^{1}}}\,(n13-n8){{\ghostvar^{1}}}\,(n14-n13){{\ghostlmd^{1}}}\,(n15-n12,29){{\ghostvar^{1}}}\,(n16-n11,32){\lambda }\,(n17-n0,25){y_2}\,}$
is given by
$(\lambda u . u(y_1 (\lambda \alpha. u (\lambda \beta.\alpha\,\beta))) (\lambda v . v\,y_2)$.
\end{example}

\paragraph{Path-View correspondence}
We now generalize a known result from the theory of traversals for higher-order grammars~\cite{Ong2006} and simply-typed lambda terms~\cite[Proposition 4.29]{BlumPhd} to the untyped setting. The game-semantic concept of P-view from Definition~\ref{def:views} corresponds to the concept of `tree path' in the following sense:
...
We define the partial function $\coresymbol_{\overline{y}}\colon \justseqset(M) \longrightarrow \justseqset(M)$ by induction on the sequence: $\coresymbol_{\overline{y}}(\epsilon) = \epsilon$, and for any justified sequence $t \cdot n\in\justseqset(M)$ with prefix subsequence $t$ and last occurrence $n$:
\begin{equation*}
...
\end{cases}
\end{equation*}
The above equalities are assumed pointer-preserving, which is well-defined since an external node can only be justified by another external node. Note that if $t$ is a traversal, the second last case is not needed since ghost lambdas are necessarily internal by Property~\ref{prop:trav_immediate}.
...
The suffix parameter $\overline{y}$ in the definition of $\coresymbol$, is used as an accumulator for the list of variables forming the abstractions $\lambda \overline{y}$ that eventually gets prepended to the last external lambda nodes. We call it the \defname{stack of pending lambdas}. Hence, \emph{core projection} can be defined more succinctly as the sequence obtained by removing internal nodes and prepending to each external lambda node's label, the stack of \emph{pending lambdas} between that point to the next external node.
...
Let us fix a simply-typed term-in-context $\Gamma \entail M :T$, with typed-context $\Gamma$ (a set of typed variables), and simple type $T$. Its \defname{eta-long form}, defined inductively on the type $T$, is obtained by recursively eta-expanding every subterm as many times as possible with respect to the subterm type~\cite{Ong2006,BlumPhd}.
...
The rules of imaginary traversals from Table~\ref{tab:trav_rules} closely resemble those of STLC~\cite{BlumPhd}: the traversal rules of STLC are precisely those of imaginary traversals with the exclusion of all sub-cases involving ghost nodes:
\begin{definition}[STLC traversal~\cite{BlumPhd}]
The set of traversals $\travstlc(\Gamma \entail M : T)$
of a simply-typed term in context $\Gamma \entail M : T$
is the set of traversals of its \emph{eta-long form} using the rules of Table~\ref{tab:trav_rules} with the exclusion of \rulenamet{Lam^\ghostlmd} and the `eta-expanded' sub-cases of \rulenamet{Var} and \rulenamet{IVar}.
\end{definition}

This definition captures STLC traversal rules as originally defined~\cite{BlumPhd}, though in practice, imaginary and STLC traversals coincide on eta-long forms in a slightly more general way:
\begin{proposition}
\label{prop:ulc_and_stlc_trav_coincide}
Let $M$ be an untyped term inhabiting a simple type $T$ for some context $\Gamma$ and $\etalf{M}$ be its eta-long normal form with respect to $T$. Then $\travstlc(\Gamma \entail M : T)$ is precisely the
set of traversals of $\etalf{M}$ obtained with rules of Table~\ref{tab:trav_rules} with the exclusion of the eta-expanded case in \rulenamet{IVar}.
\end{proposition}
\begin{proof}
...
Since the eta-expanded sub-case of \rulenamet{IVar} is excluded, no ghost node can possibly be traversed. Other rules coincide with STLC, therefore the equality holds.
\end{proof}

\subsection{Property of ghost nodes}

To understand ghost node, it can be helpful to take an analogy with complex numbers in mathematics which can be used to prove trigonometry identities: they are intermediate concepts used to perform some calculation (e.g.\ using De Moivre's Theorem) but do not appear in the final result. Just like imaginary number $i$ was created from the impossibility of calculating the square root of $-1$, ghost nodes are defined from the impossibility of ``traversing'' a beta-redex of a lambda term due to an insufficient number of operands.
...
Observe that among all the rules defined in Table~\ref{tab:trav_rules}, the rule \rulenamet{Var} is the only rule that can materialize a structural node in $\Nodes$ from a traversal ending with a ghost node. This means that after traversing ghost nodes, the only way to `return' to structural nodes is to visit a ghost variable node $\ghostvar$ with an application of rule \rulenamet{Var} of the following form:
...
(i) $\travstlc(M)$ is in bijection with the \emph{revealed game denotation} $\revsem{\Gamma \entail M :T}$ of the term,
(ii) The image of $\travstlc$ by $\coresymbol$--justified sequences that are \emph{core} of some STLC-traversal--
is in bijection with the innocent game denotation $\sem{\Gamma \entail M :T}$.
% \begin{eqnarray*}
...
Observe that rule \rulenamet{IVar} offers two non-deterministic choices when traversing a free variable:
\begin{itemize}
...
$t$ is obtained without \rulenamet{Lam^\ghostlmd_\branching} and without the `eta-expansion' subcases of the variable rules. The remaining rules correspond precisely to the traversal rules of
...
\begin{algorithmic}
\REQUIRE{A term $M$ admitting an eta-long form \emph{wrt} some simple-type $T$ that it inhabits.}
...
we define its \defname{arity threshold} as
$$
...
Even when restricted to branching traversals, the rule $\rulename{IVar_\branching}$ leaves infinitely many choices of value for the link label. The following property shows that if that value exceeds the arity threshold then only ghost nodes can be traversed from that point on, without ever materializing a structural node.
\begin{proposition}[Weaving]
\label{prop:weaving}
Let $t$ ending with strand
$\underline{\alpha_k}\,n_k\,\alpha_{k-1}\, n_{k-1}\cdots\alpha_2\,n_2\, \alpha_1\,\underline{n_1}$ for some $k\geq1$ be a traversal of $\travsetbr$, and $t_{\max}$ be a maximal extension of $t$ in $\travsetbr$. By rule $\rulename{IVar_\branching}$, $t$ is immediately followed in $t_{\max}$ by an external $\lambda$-node justified by $t^\omega$ with label $i\geq 1$.

%We defined the sequence of labels $(i_j)_{j\geq1}$ as follows:
%$$ i_{q+1} = i_1 + \sum_{j=1..q} (|\alpha_j| - |n_j|) $$

\begin{enumerate}[label=(\roman*)]
\item If $i>\arth(t)$ then $t$ is followed in $t_{\max}$ by a strand of length $2k$ consisting of ghost nodes only:
\begin{eqnarray*}
...
\cdots }
\end{eqnarray*}
for some structural internal lambda node $\lambda\overline{x}$, and consequently $i_r \leq |n_r|$.
\end{enumerate}
\end{proposition}
\proofatend
%\begin{proof}
...
This weaving property suggests that traversing external variable rule by exceeding the arity threshold yields traversals that are not relevant for normalization since they essentially expand external ghost variable nodes into deeper external ghost variable nodes. This observation gives rise to a stricter version of traversals:
\begin{definition}[Normalizing traversals]
\label{def:normalizing_traversals}
The set $\travsetnorm$ of \defname{normalizing traversals}, is the subset of branching traversals defined by the rules of Definition~\ref{dfn:branching_traversals} where the index $i$ in rule \rulenamet{IVar} is bounded by the arity threshold of the traversal.
...
Table~\ref{tab:normalizing_trav_rules} recapitulates the rules thus obtained  using judgement derivations, where the judgement notations `$\istraversal t$' means `$t\in\travsetnorm$'.
\end{definition}
...
\begin{table}
    ...
    \infrule[$\rulefont{Lam^\ghostlmd_\normalizing}$]
        ...
        }
        {
            ...
            \andalso(\alpha\in\ImNodesLmd, m \in\ImNodesVar\union\NodesApp)
        }
...
Because the tree has a finite number of nodes, the nodes arities are bounded. Therefore, for high enough index $k$, the eta-expansion case from rule \rulenamet{Var} will never be met, which means that all subsequent extensions of the traversal will be constructed using repeated applications of $\rulename{Lam^\ghostvar}$ and the eta-expanded variants of $\rulename{Var}$, and will thus only traverse ghost occurrences.
...
We thus apply rule \rulenamet{IVar} for child index $k_2$ ranging from $1$ to $ 2$. For the case $k_2 = 1$, we obtain the following traversal, where we underlined the last occurrence of its prefix $t_1$:
...
\node (t1) at (7cm,-3ex) {
        $t_1 = \Pstr[4ex]{(n0){\lambda }\,(n1){@}\,(n2-n1){\lambda u}\,(n3-n2){u}\,(n4-n1){\lambda v}\,(n5-n4){v}\,(n6-n3){\lambda }\,(n7-n0){y_1}\,(n8-n7){\lambda }\,(n9-n2){u}\,(n10-n1){\lambda v}\,(n11-n10){v}\,(n12-n9){{\ghostlmd^{1}}}\,(n13-n8){{\ghostvar^{1}}}\,(n14-n13){{\ghostlmd^{1}}}\,(n15-n12){{\ghostvar^{1}}}\,(n16-n11){\lambda }\,(n17-n0,35){y_2}\,}$
};
\node (t2) at (7cm,-15ex) {
    $t_2 = \Pstr[4ex]{(n0){\lambda }\,(n1){@}\,(n2-n1){\lambda u}\,(n3-n2){u}\,(n4-n1){\lambda v}\,(n5-n4){v}\,(n6-n3){\lambda }\,(n7-n0,30){y_1}\,(n8-n7){{\ghostlmd^{2}}}\,(n9-n6){{\ghostvar^{1}}}\,(n10-n5){\lambda }\,(n11-n0,30){y_2}\,}$
};
\end{tikzpicture}
...
% (Conjecture~\ref{conj:ulc_corresp}).
...
\begin{enumerate*}[nosep,label=(\roman*)]
\item \defname{spinal descent} if it is a path in the computation tree consisting solely of lambda and $@$-nodes, and where each lambda node (except the first occurrence if it is a lambda node) is the $0$th child of the preceding $@$-node.
...
rule \rulenamet{Lam^\ghostlmd} and \rulenamet{Var} essentially calculates the quantity ``$|\alpha|+i-|m|$'' at each iteration, which is the exact same quantity ($r-a+l$) used in step 2. (b) of the PAM transition from \cite{danos-head}.}.
...
\caption{Maximal traversals of $varity\ 2$ from Example~\ref{example:varity}}
