...
\newcommand\NodesVar{\Nodes_{\sf var}}% set of nodes
\newcommand\NodesLmd{\Nodes_\lambda}% set of nodes
...
\item We write $\Nodes(M)$ to denote the set of nodes of the computation tree of $M$, or just $\Nodes$ if the term is clear from the context. We write $\NodesVar$ for the set of variable nodes, $\NodesLmd$ for lambda nodes, and $\Nodes_@$ for  application nodes.
...
We first define the partial function $\coresymbol \colon \relabjustseqset(M) \longrightarrow \relabjustseqset(M)$ for justified sequences that do not contain any ghost occurrences then extend the notion to all justified sequences.

Let $\overline{y} \in \VarSet^*$ denote a stack of variable names.
\begin{itemize}
\item  Let $t\in\relabjustseqset(M)$ denote a justified sequences containing no ghost occurrence. We define $\coresymbol (t)$ by the following inductive definition, where the equality are implicitly pointer-preserving (so that nodes in $t$ have same justifiers as in $\coresymbol(t)$).
\begin{align*}
\coresymbol_{\overline{y}} \colon \relabjustseqset(M) & \longrightarrow \relabjustseqset(M) \\
...
\todo{This definition does not work anymore since we no longer assume that $t$ is a traversal.}
\begin{align*}
...
Given a traversal $t$, we call $\coresymbol_\epsilon(t)$ the \defname{core} of $t$ which we abbreviate as $\core{t}$.
...
%%%%%%% CASE 3
\item[Case 3] $x$ is unapplied: $|x|=0$. The parent lambda node $\lambda\overline{z}$ of $x$ in $M$ becomes $\lambda\overline{z}\overline{y}$ in $N$.
Traversals of $M$ of the form $t = \ldots \lambda\overline{z}\cdot x \cdot v \cdot \lambda\overline{y} \cdot n \cdot \ldots$ are in $R$-relation with traversals $u = \ldots \lambda\overline{z}\overline{y}\cdot n \ldots$.
...
\todo{explain how bisimulation implies the isomorphism:
first direction implies that $\phi$ is well defined, second direction that it is surjective. Still need to show that it's injective}
