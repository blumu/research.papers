...
In the standard lambda calculus, a redex is necessarily formed by the outermost lambda in operator position of an application: if the operator consists  of consecutive lambda abstractions as in $(\lambda x \lambda y . M) A_1 A_2$ then the outermost $\lambda x$ is the one defining the redex. The notion of redex can be generalized to allow any of the consecutive $\lambda$-abstractions to form a redex so that for instance the abstraction $\lambda y$ and corresponding argument $A_2$ defines a redex:


\begin{definition}[Generalized redex]
The set of generalized redexes of a term $M$, written $gr(M)$, is a set of pairs $(\lambda x, A)$ where $\lambda x$ is some abstraction in $M$ and $A$ is a subterm called the argument of $\lambda x$. The head $\lambda$ list of $M$, written $\lambda_l(M)$ is a list of lambda abstractions of $M$. They are defined by mutual induction as follows:
\begin{align*}
...
\begin{example}
Take $M= (\lambda z . (\lambda x \lambda y . M) N) A_1 A_2$ then
$gr(M) = \{ (\lambda z, N), (\lambda x, A_1), (\lambda y, A_2)\}$.
\end{example}

Note: In order to define head-linear reduction, one needs to consider specific \emph{occurrences} of variables and sub-terms in a given term. In particular, when denoting a generalized redex by a pair $(\lambda x, A)$, $\lambda x$ and $A$ denote specific \emph{occurrences} of $\lambda x$ and subterm $A$.
