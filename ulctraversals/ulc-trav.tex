...
We have $k_q = k -|x| + |\lambda\overline{\xi_q}|$ and by Eqn.~\ref{eqn:k_greater_than_x} we have $k>|x|$, hence $k_q >|\lambda\overline{\xi_q}|$.
...
\end{align*}
By the induction hypothesis we have $|\lambda_l(\lambda\overline{\xi_i})| < k_i$, and by Equation~\ref{eqn:qnfdecomp_kj} we have $ |@_i|< k_i$, therfore the maximum of the two quantities in the last equation is negative which shows
...
\item[(Ghost)] If the node following $x_1$ is a structural node then, like we have shown in case B, the next strand is necessarily a spinal descent of type $S$.

If the node following $x_1$ is a ghost.
Like in B, the weaving property shows that the ghost node after $x_1$ is necessarily followed by a \emph{pending parameter lookup} terminated by structural lambda node $\lambda\overline{y_2}$ pointing to some node in $v \cdot \lambda\overline{y_1} \cdot u_1$.
But since $v$ consists solely of ghost node, the justifier is necessarily in $u_1$. We can thus conclude with the same argument as in B that the next strand is necessarily of type
$G$.
\end{enumerate}
\end{proof}

The diagram on Figure~\ref{fig:qnf_strand_decomposition_statemachine} represents the state machine underpinning the above result. The states represents external nodes of the tree: lambda, ghost lambda or input variable; the bottom two states represent beginning of a new strand in the traversal.
...
for $r\geq0$, where $@_r$ denotes the parent node of the redex argument, and $\lambda\overline{x}$ denotes $x$'s binder, such that $\lambda x$ is the $i$th lambda in the bulk lambdas $\lambda\overline{x}$.
...
\begin{proof}
(i) Let $x$ denote the leftmost linear variable occurrence in $M$
and let $(\lambda x, A)$ be the generialized redex involving it. So that $N$ is the term obtained after firing the generalized redex $(\lambda x, A)$ in $M$.
A property of the linear head reduction, is that, modulo subsitution of the \emph{lloc} variable, the reduct contains the original term: this induces an implicit injection $\phi$ from nodes of $M$ to nodes of $N$ and such that $\phi(x)$ is precisely the root node in $N$ of the subterm subsituted for $x$ in $M$.
In what follows we will thus assume that traversals of $M$ are constituted of nodes from the tree of $N$, so that traversals of $M$ and $N$ are generated from the same set of nodes.

We prove the result using a weak bisimulation arguments between traversals of $M$ and $N$. For any two traversals $t$ and $t'$ we write $t \rightarrow_M t'$ if $t$ can be extended to $t'$ by instantiating one of the traversal rule induced from the tree of $M$ (and respectively $\rightarrow_N$ for $N$). We say that two traversals $t_1$ and $t_2$ are bisimilar just if $\coresymbol(t_1)=\coresymbol(t_2)$.

The result amounts to showing that the two sets
$\travsetnorm(M)$ and $\travsetnorm(N)$ are weakly bisimilar: (i) for all traversals $t, t' \in \travsetnorm(N)$ and $u \in \travsetnorm(M)$, if $t \sim_p u$ and $t'$ immediately extends $t'$ then there exists $u' \in \travsetnorm(M)$ that (not necessarily immediately) extends $u$ such that $u' \sim_p t'$. (ii) reciprocally the same holds after subsituting $M$ for $N$.

By induction and case analysis on the traversals rules.
The empty and singleton traversals are trivially bisimilar. Inductive case: The first direction is immediate for any traversal that does not contain the variable occurrence $x$; the second direction is immediate for traversals that do not contain the root of $A$.

The interesting case occurs when a traversal reaches the \emph{lloc} variable $x$ in $M$ (resp. the root of $A$ in $N$). Let's analyze the effect of firing the leftmost linear substitution on the compuation tree of the term. Three sub-cases shall be considered:
\begin{description}
    \item[$x$ is unapplied] (and thus has null arity $|x|$)
    ...
    \item[$x$ has at least one operand and $A$ is not an abstraction]
    ...
    \item[$x$ has at least one operand and $A$ is an abstraction]
    This case is represented on Figure~\ref{fig:firing_genredex_effect_on_tree_with_operands_and_lambda_argument}, $x$ gets replaced by an $@$-node and $A$ gets added as its first child subterm.
\end{description}

For succinctness we will only treat the third case which is the most complex one.
%%%%%%%%%%%
        ...
        child[solid]{node{$\lambda\overline{\xi}$}
            child[dashed]{node{$@$}
                child{node{$\lambda\ldots$}
                    child[solid]{node{$@$}
                        child[solid]{node{$\lambda\overline{z}$}
                            child[solid,level distance=5ex]{node {$@$}
...
;
\end{tikzpicture}
\caption{Linearly firing the generalized redex $(\lambda x, A)$ for the occurrence $x$ in $M$ (left tree) yields $N$ (right tree). Case with $|x|>0$ and $A$ is an abstraction $\lambda\overline{y} . A'$.}
\label{fig:firing_genredex_effect_on_tree_with_operands_and_lambda_argument}
\end{figure}

First direction: Let $u, u' \in \travsetnorm(N)$, $t\in\travsetnorm(M)$ such that $t \sim_p u$ and $u \rightarrow_N u'$ and suppose that $t$ ends with $x$.
%%%%%%%%%%%%%
Let $\lambda\overline{x}$ denote $x$'s binding lambda node, $k$ denote $x$'s binding index, and $@_a$ denote the parent application node of the subterm $A$.
By Lemma \ref{lemma:genredex_lookup}, $@_a$ is precisely the lowest application node in the spinal descent from $@_a$ to $\lambda\overline{x}$ verifying $|@_a| >  i_a$, where $i_a = k + \sum_{1\leq j\leq a} (|\lambda\overline{\xi_j}| - |@_j|)$. This definition matches precisely with the `pending argument lookup' algorithm that is implicitely implemented by the rules \rulenamet{Lmd} and \rulenamet{Var}: with exactly $i_a$ instantiations of those two rules, the traversal $t$ gets extended by $2(i_a-1)$ ghost nodes followed by the root node of $A$:
$$ t' = t_{<x} \cdot x \cdot v \cdot \lambda\overline{y} $$
where $v$ is a spinal descent in $M$. The nodes in $v$ are all ghost nodes and are all external since they are all justified by nodes in the spinal descent, $x$ is also external since it's involved in a redex, and $\lambda\overline{y}$ is external since its parent is an $@$-node. Consequently by definition of $\coresymbol$ we have
\begin{align*}
\coresymbol(t') &= \coresymbol(t_{<x}) \\
                &= \coresymbol(u_{<x}) \\
                &= \coresymbol(u_{<x} \cdot \lambda\overline{y}) \\
                &= \coresymbol(u)
\end{align*}
Hence $t'\sim_p u$.
%%%%%%%%%

Suppose that $n$ is not hereditarily enabled by $x$
the occurrence $x$.

Let $t$ be a traversal containing $x$ and suppose that $\psi(t) \in \travsetnorm(N)$
