...
We define the set of justified sequences of nodes over $M$, written $\justseqset(M)$, as the justified sequences over $\Nodes + \ghostlmd + \ghostvar$ (the nodes from the computation tree augmented with ghost nodes).

It is sometimes convenient to represent a justification sequence using the \defname{triplet encoding}: a sequence of triplet in $(\Nodes + \ghostlmd + \ghostvar)\times\nat\times\nat$ where the first component gives the node with label, the second component encodes the distance between a node and its justifier in the sequence ($0$ for no pointer), and the third is the link label.

Two justification sequences $s$ and $u$ over $M$ are equal, written $s=u$, if their triplet encoding are the same: their underlying sequences of nodes are identical (same node) and the associated justification pointers are identical (same justifier and same pointer label). Two sequences $s$ and $u$ over two distinct terms $M$ and $N$ are \defname{equal up to renaming}, or $\alpha$-convertible, written $s =_\alpha u$ just if the type of the underlying nodes match in order ($@$-node, variable or lambda node) and they have the same justification pointer (\ie, same link distance and label).

We write $I \cong J$ to denote two subsets $I \subseteq \justseqset(M)$ and $J \subseteq \justseqset(N)$ such that there exists a bijection between $I$ and $J$. We say that $\phi : I\longrightarrow J$ is a \defname{strong bijection} if it is a bijection and further for any $j\in J$, $j =_\alpha \phi(j)$.
...
We define the core P-view function $\Phi$ as the composition of $\pview{\_}\colon \justseqset^\eta \longrightarrow \justseqset^\eta$ with the core projection $\coresymbol\colon\travulc  \longrightarrow \justseqset^\eta$:
\begin{align*}
\Phi \colon \travulc & \longrightarrow \justseqset^\eta \\
...
\end{proposition}
\begin{proof}
Let $t\in\travulc$ and let's first assume that $t$ does not contain any ghost occurrence. We show by strong induction on $t$ that there is a subsequence $u\in\travsetbr$ of $t$ such that $\pview{\core{t}} = \pview{\core{u}}$.
...
The following is a direct consequence of Proposition \ref{prop:branching_traversal_normalizing} and the weaving Property \ref{prop:weaving}.
...
\end{proposition}
\begin{proof}
Let $t\in\travulc$. By Prop.~\ref{prop:branching_traversal_normalizing}
there is $u\in\travsetbr$ such that $\pview{\core{u}} = \pview{\core{t}}$.
If $u$ has no ghost node then $u$ necessarily belongs to $u\in\travsetnorm$
since the restriction in rule \rulenamet{IVar_\normalizing} only applies to the case where a ghost lambda node is traversed (child index $i$ strictly greater than the arity of the node $n$).

If $u$ contains ghost nodes then

%  $\eta_u(u) \in \travulc$ has no ghost.
% Clearly $\eta_u(u)$ also belongs to $\travsetbr$ since $u$ does.
%  And $\core{\eta_u(u)} = \core{u}$ by definition of $\coresymbol$.
Suppose that $u$ is not in $\travsetnorm$ then by Property \ref{prop:weaving}, ,it's necessarily of the form:
$$u = u_1 \alpha n \ldots \ghostlmd \ghostvar$$
(without loss of generality we assume $u$ ends with a ghost variable) where $\ghostlmd$ points to $n \in \NodesVar$ and
$\ghostvar$ points to $\alpha \in \NodesLmd$.

But then $\pview{\core{u}} = \pview{u_1 \alpha } n \ghostlmd \ghostvar$
\todo{not be really sim-complete, since arbitrarily long eta expanded lambda nodes are excluded! Need a new definition of equivalence classes to convey this.}
\end{proof}
...
The traversals before and after reduction are identical up to some node mapping and node merging. One can show that the invariant $\core{\travsetnorm(M)}$ is preserved, modulo some isomorphism, by this mapping.
...
\todo{Rework characterization theorem for new definition of $\coresymbol$ which relabel lambda nodes!}
