...
We define the set of \defname{quadruplet sequences} over variables in $\mathcal{V}$ as $O_4^*$ where $O_4 = \nat^*\times \mathcal{V}^* \times\nat\times\nat$. The first component represents a \emph{node path}; the second is a sequence of variable names representing \emph{pending lambdas}; the third represents a \emph{link distance}, and the fourth is a \emph{link label}.

\begin{definition}[Quadruplet encoding]
\label{def:quadruplet_encoding}
The following occurrence-wise encoding yields a sequence of quadruplets in $O_4^*$ that encodes a given justified sequence of nodes.
\begin{enumerate}[nosep]
    \item Node path: path to the extended node in the computation tree that uniquely defines the type and label of the occurrence;
    \item Pending lambdas: empty list $\epsilon$;
    \item Link distance: number of occurrences separating the encoded occurrence from its justifier;
    \item Link label: label associated with the occurrence's justification pointer.
\end{enumerate}
\end{definition}

One should interpret the `pending lambdas' component as a sequence of variable names to be `mentally' appended to the bulk of variables that are already bound by the lambda node. It is empty when encoding a raw justified sequence, but will be used to define transformations that modify the lambda node labels. This will allow us to interpret a justified sequence as a traversal over two distinct terms (\eg, before and after firing a redex).
...
the respective `pending lambdas` to the variables bound by each lambda node occurrence, they have the same structure and same variable names. Formally, given two terms $M$ and $N$ we define the relation
...
\begin{example}
    Alpha-conversion, does not impact the structure of a sequence, obviously, so for instance we have $\Pstr[10pt]{ (l){\lambda x y} \cdot (n-l,25){x} } \equiv \Pstr[10pt]{ (l){\lambda z y} \cdot (n-l,25){z}}$. The number of variable bindings in a lambda node may also change without changing the structure, as long as justification links are unaffected, so for instance we have $\Pstr[10pt]{ (l){\lambda x y z w t} \cdot (n-l,25){x} } \equiv \Pstr[10pt]{ (l){\lambda x} \cdot (n-l,25){x}}$ since both sequences have structure $(\lambda,0)\cdot({\sf Var},1)$.
\end{example}
...
Given two node occurrences $n$ and $m$ in a justified sequence $s$, we say that $n$ is \defname{visible at} $m$ just if $n$ occurs in the P-view $\pview{s_{\leq m}}$.
...
\end{proposition}
\begin{proof}
Because the term is eta-long, the condition $i>|m|$ in rules \rulenamet{Var} never holds while traversing the term. This is shown by induction on the rules, observing that the arity of an $@$ node is necessarily equal to the arity of its $0$th child lambda node, and the arity of a variable node with binding index $i$ is necessarily equal to the arity of the $i$th child of its binder's parent.
And since the eta-expanded rule \rulenamet{IVar}  is excluded, traversals in $\travulc'(\etalf{M})$ do not contain any ghost node. Other rules coincide with STLC, therefore the equality holds.
\end{proof}
...
Let's now apply rule \rulenamet{IVar} for child index $k_2$ ranging from $1$ to $q_2 = 2$. For the case $k_2 = 1$, we obtain the following traversal, where we underlined the last occurrence of its prefix $t_1$:
...
\node (t1) at (5,0) {$t_1 = \Pstr[4ex]{(n0){\lambda }\,(n1){@}\,(n2-n1){\lambda u}\,(n3-n2){u}\,(n4-n1){\lambda v}\,(n5-n4){v}\,(n6-n3){\lambda }\,(n7-n0){y_1}\,(n8-n7){\lambda }\,(n9-n2){u}\,(n10-n1){\lambda v}\,(n11-n10){v}\,(n12-n9){{\ghostlmd^{1}}}\,(n13-n8){{\ghostvar^{1}}}\,(n14-n13){{\ghostlmd^{1}}}\,(n15-n12){{\ghostvar^{1}}}\,(n16-n11){\lambda }\,(n17-n0,35){y_2}\,}
$}
;
\node (t2) at (5,5) {$t_2 = \Pstr[4ex]{(n0){\lambda }\,(n1){@}\,(n2-n1){\lambda u}\,(n3-n2){u}\,(n4-n1){\lambda v}\,(n5-n4){v}\,(n6-n3){\lambda }\,(n7-n0,30){y_1}\,(n8-n7){{\ghostlmd^{2}}}\,(n9-n6){{\ghostvar^{1}}}\,(n10-n5){\lambda }\,(n11-n0,30){y_2}\,}
$}
    ;
\end{tikzpicture}
%&
...
Traversing $M$ with STLC rules gets all the way to the second occurrence of $v$ in $t_1$, where the traversal gets stuck due to bottom node $u$ lacking an operand. Whereas an imaginary traversal creates that missing operand on-the-fly, and refers to it with a ghost lambda node $\ghostlmd^1$.

Traversal $t_2$ illustrates a different situation where instead of being stuck, the STLC traversals misses a path from the beta normal form: variable $y_1$ does have one argument in $M$, which gets traversed by STLC traversals. But the ULC traversal $t_2$ pushes the exploration further by traversing an imaginary second argument to $y_1$, represented by $\ghostlmd^2$, which eventually materializes into existing node $y_2$ of $M$.
