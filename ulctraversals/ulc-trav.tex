...
For $t, t' \in \travsetnorm(M)$ we write $t \rightarrow_M t'$ just if $t'$ is an immediate extension of $t$ in $\travsetnorm(M)$, that is $t'$ extends $t$ using one rule of Table~\ref{tab:normalizing_trav_rules}.
...
from traversals of $N$ to justified sequences of $M$.
%
We consider the state transition system $(X, R_\phi, \rightarrow_X)$
over \emph{States} $X = \travsetnorm(M) + \travsetnorm(N)$,
with  \emph{Transitions} $t \rightarrow_X t'$
if either $t \rightarrow_M t'$ or $t \rightarrow_N t'$ holds, for $t, t' \in X$;
and \emph{Binary relation $R_\phi\subseteq X \times X$} defined for $t, u \in X$ by $ t\,R_\phi\,u$ just if $t\in\travsetnorm(M)$, $u\in\travsetnorm(N)$ and $\phi(u) \doteq t$.

The terms $M$ and $N$ are \defname{$\phi$-bisimilar} if $R_\phi$ is a bisimulation over $X$ with respect to $\rightarrow_X$:
\begin{itemize}[nosep]
    \item[(B1)] for all $t_1, t_2 \in \travsetnorm(M)$, $u_1 \in \travsetnorm(N)$ with $t_1\,R_\phi\,u_1$ and $t_1 \rightarrow_M t_2$ there is $u_2 \in \travsetnorm(N)$ such that $u_1 \rightarrow_N u_2$ and $t_2\,R_\phi\,u_2$;
    \item[(B2)] for all $t_1 \in \travsetnorm(M)$, $u_1, u_2 \in \travsetnorm(N)$ with $t_1\,R_\phi\,u_1$ and $u_1 \rightarrow_N u_2$ there is $t_2 \in \travsetnorm(M)$ such that $t_1 \rightarrow_M t_2$ and $t_2\,R_\phi\,u_2$.
\end{itemize}
\end{definition}
...
Thus in $\travsetscn$, `almost-equality' coincides with `equality`.
...
for some function $\phi$ whose restriction to $\coresymbol(\travsetscn(N))$ is injective and commute with $\coresymbol$
%(for all $u \in \travsetscn(M)$ $\coresymbol(\phi(u)) = \phi(\coresymbol(u))$),
then $\coresymbol(\travsetscn(N)) \cong \coresymbol(\travsetscn(M))$.
\end{lemma}
\begin{todobox}
    to fix: scn -> normalizing
\end{todobox}
\begin{proof}
Consider $\phi$ as a function from $\coresymbol(\travsetscn(N))$ to $\coresymbol(\travsetscn(M))$.
It is \emph{well-defined}: $\travsetscn(N)$ is by definition the transitive closure of the singleton traversal by $\rightarrow_N$, so for any traversal $u$ of $\travsetscn(N)$, repeatedly applying (B2) for each step of its $\rightarrow_N$ derivation shows that $t = \phi(u) \in \travsetscn(M)$, and the commutativity of $\coresymbol$ and $\phi$ gives $\coresymbol(t)=\phi(\coresymbol(u)) \in \coresymbol(\travsetscn(M))$.
Similarly, it is \emph{surjective} by (B1) because $\travsetscn(M)$ is the transitive closure of the singleton traversal by $\rightarrow_M$. Finally it is \emph{injective} by assumption.
...
\end{corollary}
\begin{proof}
By Proposition~\ref{prop:qnf_nf}(ii) a \emph{qnf} reduces to its beta-nf: there is a reduction sequence $M = M_1 \rightarrow_\beta \ldots \rightarrow_\beta M_q= N$, $q\geq1$  where in each step, $M_{i+1}$ is obtained by reducing the leftmost spinal-innermost standard redex of $M_i$ (of the form $(\lambda x . U) T$ such that $U$ does not contain any redex). By Proposition~\ref{prop:qnf_nf}(i), such redex is trivial. By Proposition~\ref{prop:ulctrav_sound_for_trivialreduction}
the operation $\core{\travsetnorm^{\max}(\_)}$ is an invariant (up to isomorphism) throughout this trivial reduction sequence.
\end{proof}
