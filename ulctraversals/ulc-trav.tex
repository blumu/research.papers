...
\def\readoutlmd{\readout_\lambda} % prea-readout function
\def\readoutvar{\readout_{\sf var}} % readout var function
...
\begin{example}
    Alpha-conversion does not alter the structure of a sequence, so for instance we have $\Pstr[10pt]{ (l){\lambda x y} \, (n-l,25){x} } \equiv \Pstr[10pt]{ (l){\lambda z y} \, (n-l,25){z}}$. The number of variable bindings in a lambda node may also change without impacting its structure, as long as justification link labels are unaffected, so we also have $\Pstr[10pt]{ (l){\lambda x y z w t} \, (n-l,25){x} } \equiv \Pstr[10pt]{ (l){\lambda x} \, (n-l,25){x}}$ since the structure of both sequences is $(\lambda,0)\,({\sf Var},1)$.
\end{example}


The \defname{quadruplet encoding} of a justified sequence of nodes with variables in $\VarSet$ is a sequence of quadruplets in $O_4 = \ExtendedNodes \times \mathcal{V}^* \times\nat\times\nat$ defined occurrence-wise where each quadruplets $(n,p,j,i) \in O_4$ is such that $n$ is a \emph{Node} from the extensional tree that uniquely defines the type and label of the occurrence;
\emph{Pending lambdas} $p$ is the empty list $\epsilon$;
\emph{Link distance} $j$ is the number of occurrences separating the occurrence from its justifier plus $1$, or $0$ for no justifier;
\emph{Link label} $i$ is the label of the justification pointer.

This initial encoding is said to be \defname{canonical} because the \emph{pending lambdas} components are all empty. We will define subsequent transformations that alter the pending lambdas.
Observe that given a fixed term, a canonical justified sequence encoding uniquely determines its structure, by just discarding the labels; and reciprocally, labels are uniquely recoverable from the sequence structure together with the enabling relation associated with that term.
%Pending lambdas will help us interpret justified sequences over two distinct terms (\eg, before and after firing a redex).
%We will use non-canonical encodings to implement linear substitution via justified sequences, where the \emph{pending lambdas} component will provide a mechanism to combine two distinct lambda nodes in a single one, a process by which an anonymous ghost variable may materialize into a structural node with a variable name.

When typesetting a sequence from $\justseqset(M)$, we represent occurrences by their associated node labels and represent the \emph{pending lambdas} in exponent. For example in the sequence $\cdots @ \cdot x \cdot \lambda\overline{x}^{[y_1 \ldots y_n]} \cdots$, $\lambda\overline{x}$ is the label of the tree node associated with that occurrence and $y_1 \ldots y_n$ are the \emph{pending lambdas} for this occurrence. We omit the exponent if the list of pending lambdas is empty. We adopt the quadruplet encoding throughout the rest of the paper and thus assume $\justseqset(M)$ to be a (strict) subset of $O_4^*$.

\emph{Decoding a quadruplet sequence}
We define a readout procedure to decode a sequence of quadruplets as a sequence
of node labels.
%
...
The \defname{readout function} $\readout: O_4^* \rightarrow \mathcal{L}^*$ converts a quadruplet-encoded justified sequence into a sequence of node labels in $\mathcal{L}$ that are not necessarily present in nodes from $\ctree(M)$.
We define it in two stages: function $\readoutlmd \colon O_4^* \rightarrow \mathcal{L}_\lambda^*$ first assigns pre-labels to lambda occurrences, then $\readoutvar\colon \mathcal{L}_\lambda^* \rightarrow \mathcal{L}^*$ assigns labels to variable occurrences based on the label assigned in the first stage to their justifier.
The pre-labels are
$\mathcal{L}_\lambda =
\{ \lambda \overline{x} \, | \, \overline{x} \in \VarSet^* \}
    + \{ @ \} + (\nat \times \nat)
$, where the last two dimensions encode respectively the link distance and link label of a variable occurrence.
The pre-labelling function $\readoutlmd \colon O_4 \rightarrow \mathcal{L}_\lambda$ is first defined for one occurrence $(n,l,j,i)\in O_4$ as: $\readoutlmd[(@,l,j,i)] = @$,
$\readoutlmd[(n,l,j,i)] = (j, i)$ for $n\in\ExtendedNodesVar$,
and $\readoutlmd[(\lambda \overline{x},\overline{p},j,i)]
= \lambda \overline{z}$ where $\overline{z} = \overline{x} \otimes \overline{p}$ for $\lambda\overline{x}\in\ExtendedNodesLmd$.
We then consider its extension $\readoutlmd \colon O_4^* \rightarrow \mathcal{L}_\lambda^*$ defined point-wise on justified sequences.
(Without loss of generality we assume that the term is closed, or equivalently that free variable occurrences point to the initial occurrence where the link label is taken from a pre-defined set of free-variable indices.)

Variable label assignment is defined by $\readoutvar\colon \mathcal{L}_\lambda^* \rightarrow \mathcal{L}^*$ as:
$$\begin{array}{rcll}
    \readoutvar[\epsilon]
         &=&  \epsilon \\
    \readoutvar[s \cdot @]
         &=&  \readoutvar[s] \cdot @ \\
    \readoutvar[s \cdot \lambda\overline{x} ]
         &=&  \readoutvar[s] \cdot \lambda\overline{x} \\
    \readoutvar[s \cdot \lambda\overline{x}\,  u_1 \ldots u_{i-1} (i,j)]
         &=&
          \begin{cases}
            \readoutvar[s \cdot \lambda\overline{x}\,  u_1 \ldots u_{i-1}] \cdot x_j &
              \mbox{if $j\leq |\overline{x}|$.} \\
            \readoutvar[s \cdot \lambda\overline{x}\,  u_1 \ldots u_{i-1}] \cdot \ghostvar & \mbox{if $j> |\overline{x}|$.}
         \end{cases}
\end{array}$$
where we assimilate the ghost and dummy lambda so that $\ghostlmd$ is covered by the case  $\lambda\overline{x}$, $\overline{x}=\epsilon$.
The readout function is the composition of the two stages: $\readout[s] = \readoutvar[\readoutlmd[s]]$ for $s \in O_4^*$.
...
\label{examp:ghost_materialization}
    ...
    $(\lambda x, \epsilon ,0,0)$, $(@,\epsilon,0,0)$, $(\lambda y z, \epsilon, 1,0)$, $(z,\epsilon, 1,2)$, $(\ghostlmd,\epsilon,3,2)$, $(\ghostvar,\epsilon,5,2)$ whereas the quadruplet sequence $ (\lambda x, g ,0,0), (@,\epsilon,0,0), (\lambda y z, x\,y\,y, 1,0), (z,\epsilon, 1,2), (\ghostlmd,\epsilon,3,2), (\ghostvar,\epsilon,5,2)$ encodes the justified sequence
    $\Pstr[10pt]{
        (l){\lambda x^{[g]}}  (a)@  (lyz-a,25:0){\lambda y z^{[x\,y\,y]}}  (z-lyz,30:2)z \, (gl-a,40:2)\ghostlmd \, (gv-l,38:2)\ghostvar }
    $ which reads out via $\readout$ as
    $\lambda x g ~ @ ~ \lambda y z x y' y'' ~ z ~ \lambda ~ g$.
\end{example}
...
%
We borrow the following two definitions from Game Semantics~\cite{Abr02}.
...
The P-view is the sub-sequence obtained by reading a justified sequence backwards as follows: if the last node is a variable node then follow its justification pointer (\ie, skip all the occurrences lying underneath); if it is a lambda node then move to the preceding node. Formally:
...
We consider sequences of variable names in $\VarSet^*$ as stacks equipped with  push and pop operations: for any sequence of variables $\overline{x} = x_1 \ldots x_n$, $n,j\geq0$, the operation $pop_j$ removes the first $j$ elements of the sequence: $pop_j (x_1 \ldots x_n) = x_{j+1} \ldots x_n$ if $j<n$, and $pop_j (x_1 \ldots x_n) = \epsilon$ otherwise;
and for variable names $\overline{y} = y_1 \ldots y_m$, $m\geq0$ we write $\overline{x} \cdot \overline{y}$
for the stack obtained by pushing the variables $\overline{x}$, read from right to left, onto $\overline{y}$: $\overline{x} \cdot \overline{y} = x_1 \ldots x_n y_1 \ldots y_n$.
...
Observe that for simply-typed terms in eta-long form, the dynamic arity necessarily matches with the static arity, therefore \emph{concrete essential} traversals coincide with \emph{normalizing} traversals.
 Hence specializing  Algorithm~\ref{algo:ulc_normalization_by_traversals} to eta-long forms yield Algorithm~\ref{algo:stlc_normalization_by_traversals}.
...
\begin{example}
    ...
    We have $\coresymbol(t) = \Pstr[10pt]{(l){\lambda x^{[z]}} \, (gv-l,32:2)\ghostvar } \equiv \Pstr[10pt]{ (l){\lambda x z} \, (gv-l,32:2){z} } \in \travsetnorm(N)$.
\end{example}
