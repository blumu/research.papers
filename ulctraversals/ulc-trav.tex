...
\begin{itemize}
    \item \emph{States}: $X$ denote the disjoint union $\travset^\dagger(M) + \travset^\dagger(N)$.
...
\coresymbol_\alpha(t') &= \coresymbol_\alpha(t \cdot v \cdot \lambda\overline{y}) & \mbox{(Def.~of $t'$)} \\
                ...
                &\equiv \coresymbol_{\alpha \cdot \overline{y_2}}(u) & \mbox{(alpha conversion)}\\
                &= \coresymbol_\alpha(u \cdot \lambda\overline{y_2}) & \mbox{(def of $\coresymbol$)}\\
                &= \coresymbol_\alpha(u') & \mbox{(def of $u'$)}
\end{align*}
and since $\phi(\lambda\overline{y_2}) = \lambda\overline{y}$ we thus have $t'~R~u'$.
%%%%%%%%%
