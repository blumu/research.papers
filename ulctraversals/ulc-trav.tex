...
\end{proposition}
\begin{proof}
(i) Because the term is eta-long expanded, the condition $i>|m|$ in the rules \rulenamet{Var} and \rulenamet{IVar} never holds while traversing the term. (ii) By definition of $\travstlc$, the remaining traversals rules of STLC and ULC coincide, therefore the equality holds.
\end{proof}
...
For simply-typed terms in eta-long form, the Paths Characterization result is a direct consequence of the Game-Semantic Correspondence Theorem for STLC \cite{BlumPhd}:
...
\paragraph{Soundness}
A beta-normal term is uniquely determined by the set of maximal paths in its tree representation.
By Proposition \ref{prop:path_charact_stlc}, this set corresponds precisely to the set of core P-views,
...
\paragraph{Termination} By Property \ref{prop:etalong_trav_finite} the set of branching traversals is finite for terms in eta-long form, therefore the enumeration in Algorithm \ref{algo:stlc_normalization_by_traversals} terminates.
...
We now generalize the normalization procedure of Algorithm~\ref{algo:stlc_normalization_by_traversals} to ULC: Algorithm~\ref{algo:ulc_normalization_by_traversals} describes a normalization procedure that computes the $\beta$-normal of an untyped lambda term if it exists.
...
\paragraph{Soundness} Correctness of the normalization procedure is a direct consequence of the Characterization theorem below and the fact that normalizing traversals are $\sim$-complete (Prop.~\ref{prop:normalizing_traversals_simcomplete}):
