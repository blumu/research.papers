...
\author{William Blum}
\title{Imaginary Traversals of Untyped Lambda Terms \\[12pt]
...
Because ULC is not strongly-normalizing, maximal traverals are not necessarily finite. There is therefore no notion of `last occurrence' in such traversal, which means that the P-view itself is undefined. We will instead consider P-views of finitary approximations of traversals. In other words, for ULC, the equation above becomes:
$$ \Union_{k\geq0} \pview{\travset^k(M)\filter\theroot} = \pathset(\betanf{M}) . $$
for any untyped lambda term $M$
where $\pathset (M)$, or just $\pathset$ if the term is clear from context, denotes the set of \emph{possibly infinite} maximal paths in the infinitary computation tree representation\footnote{For a formal definition of infinitary computation trees see \cite{Blum17-HomogNotRestriction, Ong2006}} of the normal form of $M$.
...
$$\travset^\normalizing(M) = \{ t_{11}, t_{121}, t_{122} \}$$
