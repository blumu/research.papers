...
Let $M$ be a term in quasi-normal form. Let $t_{max}$ be a maximal traversal of $M$. Then $t_{max}$ consists of a succession of weaves of one of the following two forms:
...
\item The $lloc$ at $\lambda\overline\eta$ is undefined (\ie,  $lloc_M(\lambda\overline\eta) = \bot$),
...
\item $t_{max}$ is finite.

\end{enumerate}

\end{proposition}
\begin{proof}
We prove the result using an inductive argument by showing that any traversal ending with a external structural lambda node with undefined $lloc_M$ can only be extended with weaves of the form (A) or (B), and if a traversal ends with either form then in any maximal extension, it's necessarily followed by an external structural lambda node with undefined $lloc_M$.

\emph{Base case:} By rule \rulenamet{Root}, a traversal starts with the root node $\lambda \overline{\eta}$ which is, by definition, an external lambda node. By definition we have $lloc_M(\lambda \overline{\eta}) = lloc_M(M)$ which is undefined since $M$ is in \emph{qnf}.


\emph{Induction}
Let $t$ be a traversal ending with external structural lambda node $\lambda \overline{\eta}$. By traversal rule \rulenamet{Lam}, the node following $\lambda \overline{\eta}$ is its child node $n$.

Suppose $n$ is an external variable node $x_1$ then we fall into case (A). Further, by Property \ref{prop:weave_spinaldescent}(ii), the node following $x_1$ is necessarily a structural child lambda node of $x_1$. The subterm rooted at $\lambda \overline\eta$ is of the form $\lambda \overline\eta . x_1 A_1 \ldots A_q$ for some $q\geq1$, and since $lloc_M(\lambda \overline\eta . x_1 A_1 \ldots A_q) = \bot$ and $x_i$ is external we necessarily have $lloc_M(A_j)=\bot$ for all $1\leq j\leq q$. Hence the node following $x_1$ also has undefined $lloc_M$.

Suppose $n$ is internal. Because $lloc_M(\lambda \overline\eta) = \bot$, $n$ cannot be a variable node, otherwise, by definition of internal variables, it would be involved in some generalized redex and we would have
$lloc_M(\lambda \overline\eta) = lloc_M(\lambda \overline\eta . y A_1 \ldots A_q) = y$ where $y$ is the internal variable labelling $n$.
Hence $n$ is necessarily an application node $@$.
By repeating the above argument we obtain the non-empty spinal descent $u_1$ leading to some external variable $x_1$.

Let $\alpha$ denote the node following $x_1$. By rule \rulenamet{IVar}, $\alpha$ is either the $i$th a child lambda node of $x_1$ with $i\leq|x_1|$ or a ghost lambda node with link label $i>|x_1|$.
