...
A notable limitation is that the tree traversed is built from the \emph{ eta-long form} of the term, a conversion that involves eta-expanding some subterms so as to ensure that every strict subterm that is not of atomic type has necessarily another term applied to it. We omit the formal definition~\cite{huet75-unification, BlumPhd, OngLics2006} which is not essential.
 Effectively, this means that the STLC computation tree is obtained by eta-expanding all variable and application nodes according to the type of the corresponding subterm: if it is $\tau_1 \rightarrow \ldots (\tau_m \rightarrow o)$ and the node has $n< m$ children then the submterm is eta-expanded $m-n$ times.

 In STLC computation trees, the lambda nodes labels consist of typed-annotated variable names, but those annotations are not used in any way by the STLC traversals rules. We can thus apply the rules of Table~\ref{tab:trav_rules} on a typed-term by just erasing all its types.
With this assumption, the definition of STLC traversal from \cite{BlumPhd} can be reformulated as follows. Without loss of generality we can consider a closed term $M^\tau$ of simple type $\tau$. We write $\etalf{M^\tau}$ to denote its \emph{eta-long form}.
\begin{definition}[STLC traversals]
The traversals $\travstlc(M^\tau)$ of a simply-typed term $M^\tau$
are the justified sequences obtained by applying on $\etalf{M^\tau}$ the system of rules of Table~\ref{tab:trav_rules} without \rulenamet{Lam^\ghostvar} and such that \rulenamet{Var} and \rulenamet{IVar} have the added restriction that justification label $i$ is no greater than variable arity $|m|$.
\end{definition}

The two notions of traversals in fact coincide on eta-long forms in a slightly more general way:
\begin{proposition}[ULC traversals subsume STLC traversals]
\label{prop:ulc_and_stlc_trav_coincide}
$\travstlc(M^\tau)$ coincides with the
 justified sequences obtained by applying on $\etalf{M^\tau}$ the system of rules of Table~\ref{tab:trav_rules} where \rulenamet{IVar} is restricted by the condition $i\leq|m|$.
\end{proposition}
\begin{proof}
...
In this section we show how to evaluate lambda terms with traversals and find their normal form when it exists.
The idea behind the evaluation procedure stems
 from the \emph{Paths Characterization} of the normal form (Theorem~\ref{thm:path_charact_ulc}) which states that the set of P-views of traversal cores captures what is strictly required from traversals to reconstruct the normal form when it exists.
This characterization was previously shown for STLC via
a correspondence with Game Semantics,
 and is recalled in Proposition~\ref{prop:path_charact_stlc};
 in Section~\ref{sec:correctness_ulc_normalization} we prove its untyped counterpart using instead a term-rewriting argument.

The Paths Characterization suggests a normalization method by simply enumerating all the maximal traversals. The challenge is that there are arbitrarily long traversals due to the generous non-determinism of the rule \rulenamet{IVar} that dictates how to traverse a free variable node.
...
For the simply-typed lambda calculus, we consider the subset of \emph{branching traversals} which is complete with respect to core P-views, and finite provided that the term is in eta-long form.
For the untyped lambda calculus, \emph{branching traversals} are not adequate as they can still be infinite even for terms with a normal form. We introduce instead a stricter kind of traversals, called \emph{normalizing traversals} that further limits the non-determinism.
...
The traversals $t_k$, $k\geq 0$, correspond to plays
 ...
 such terms being applied to $M$. Fortunately, traversals accounting for those contexts are redundant for normalization: due to the absence of side-effects, calling the same argument multiple times always involves the same underlying computation in $M$. We formalize this intuition with the notion of \emph{branching traversals}:
...
For every closed simply typed term $M^\tau$ in eta-long normal form
...
\begin{enumerate}[nosep]
  ...
  \item Enumerate maximal \emph{branching} traversals of $\etalf{M^\tau}$ from Definition~\ref{dfn:branching_traversals}:
  \begin{itemize}[nosep]
