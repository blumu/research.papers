...
\label{prop:weaving}
Let $t \in \travsetbr$ be a branching traversal ending with a variable node hereditarily justified by the root and $t_{max} \in \travsetbr$ be a maximal traversal extension of $t$. We consider the weave ending at $t^\omega$:
...
We then have:
\begin{enumerate}[label=(\roman*)]
\item If $i>\arth(t)$ then $t^\omega$ is followed in $t_{max}$ by a weave of length $2k$ consisting only of ghost nodes defined as follows:
\begin{eqnarray*}
...
\end{property}
\begin{proof}
(i) By the alternation property of traversals, the last weave of $t$ consists of a succession of lambda nodes $\alpha_q$ and variable or $@$-nodes $n_q$, with indices going from $k$ down to $1$.
...
\begin{property}
\label{prop:weave_spinaldescent}
...
\begin{enumerate}[label=(\roman*)]
  \item If all nodes in $v$ are internal (\ie, if $u$ is a \emph{weave}) then
  $u$ is a spinal descent of $M$.
  ...
  %%\item If the first node in $v$ is external then every node in $v$ is a structural external node.
\end{enumerate}
\end{property}
\begin{proof}
 (i) It trivially follows from the definition of the traversal rules \rulenamet{App} and \rulenamet{Lam}.

 (ii) If $v$ is empty then the weave ending at $x$
 is just $\underline{\lambda \overline{x}} \cdot \underline{x}$ therefore
 the arity threshold of $t$ is precisely $|x|$ and so rule \rulenamet{IVar} can only be used to visit a structural child of $x$, not ghost nodes.
\end{proof}

\begin{proposition}[Qnf weave decomposition]
Let $M$ be a term in quasi-normal form. Let $t_{max}$ be a maximal traversal of $M$. Then $t$ consists of a succession of weaves of one of the following two forms:

 \begin{align*}
    (A) &\ \underline{\lambda\overline{\eta}} \cdot \underline{x_1} \\
    (B) & \underbrace{\underline{\lambda\overline{\eta}} \cdot u_1 \cdot \underline{x_1}}_{\parbox[c]{2cm}{first weave}} \cdot
        \underbrace{\underline{\ghostlmd} \cdot  v \cdot \lambda\overline{\eta_2} \cdot u_2 \cdot \underline{x_2}}_{\mbox{second weave}}
 \end{align*}

where
    \begin{itemize}
    \item $\lambda\overline{\eta}$ is a structural external lambda node,

    \item $u_1$ is a non-empty \emph{spinal descent} (internal nodes),
    \item $v$ is a \emph{pending parameter lookup} (ghost nodes) of length shorter than $u_1$,
    \item $\lambda\overline{\eta_2}$ is an internal lambda node
        justified by some $@$ node occurring in $u_1$,
    \item $u_2$ is a \emph{spinal descent} (internal nodes),
    \item $x_1,x_2$ are structural external variable nodes.
    \end{itemize}

such that
\begin{enumerate}
\item $lloc_M(\lambda\overline\eta) = \bot$.
\item the lambda node $\lambda\overline\eta$ occurs in the path from the last node (either $x_1$ or $x_2$) to the root of the tree.
\end{enumerate}
Consequently,
\begin{enumerate}[label=(\Alph*)]
\item For all external lambda node $\alpha \in \NodesLmd$ occurring in $t$ we have $lloc_M(\alpha) = \bot$.

\item $t$ does not contain any internal variable node (\ie, all the internal nodes are $@$ nodes);

\item $t_max$ is finite.

\end{enumerate}

\end{proposition}
\begin{proof}
By induction on finite prefixes $t$ of $t_{max}$ ending with an external lambda node. We show that any traversal ending with an external lambda node is either of the form (A) ar (B2e2)e2eee22e2e2e2e2e2e2e22e2e2e2e2e2e2e2e2e2e2e2e2e2ed2e2eas2asdead2e2e2e2e22 e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2e2

starts with a weave of type (A) or (B), and if a traversal ends with such forms then it's also followed by weaves of type (A) or (B).

\emph{Base case:} a traversal starts with the root node which is, by definition, an external lambda node. Let's write it $\lambda \overline{\eta}$.
By traversal rule \rulenamet{Lam}, the node following $\lambda \overline{\eta}$ is its child node. Suppose it's an external variable node then we fall into case (A), and by definition we have $lloc_M(t^\omega) = lloc_M(M)$ which is equal to $\bot$ since $M$ is in \emph{qnf}.

Otherwise,


Let $t$ be a finite prefix of $t_{max}$ and suppose that $t^\omega$ is an external lambda node $\lambda \overline{\eta}$.


Inductive case. Suppose the property is true for some finite prefix $t$ of $t_{max}$ then we show that the next weaves following $t$ are necessarily of type (A) or (B).

First $t$ ends with a weave of type (A), then by Property \ref{prop:weave_spinaldescent}(ii), the node following $n$ is necessarily a structural child lambda node of the last node in $t$

, $\lambda \overline{\eta}$



Then the weave starting at $t^\omega$ is of the form


Suppose $t^\omega$ is an external structural variable node $x_1$.
    Consider the weave immediately preceding $x_1$: $\underline{\lambda\overline{\eta}} \cdot u_1 \cdot \underline{x_1}$ where $u_1$ is a (possibly empty) \emph{spinal descent} (internal nodes).
    \begin{itemize}
    \item If $u_1$ is empty then by Property \ref{prop:weave_spinaldescent} the node following $x_1$ is necessarily a structural external lambda node and the next weave is then defined as per case (a) above.

    \item Otherwise the previous and next weaves are the form:
    \end{itemize}

Suppose


%By induction on the traversal $t$. It's trivially true for the base case $t = r$ where $r$ is the root of the tree. In particular, because $M$ is in \emph{qnf} we have $lloc_M(r) = \bot$.
%Let's assume all the properties hold for $t$ and consider a traversal extension $t_{ext}$ of $t$.
Case (a):


We prove the rest by case analysis on $t^\omega$.

Suppose $t^\omega = \lambda\overline{\eta}$ is the root of $M$.
Then
- (A) $lloc_M(t^\omega) = lloc_M(M)$ which is equal to $\bot$ since
 $M$ is in \emph{qnf}.

- (B) holds trivially

C. Suppose $t$ is a singleton then $t^\omega = \lambda\overline{\eta}$ is the root of $M$ so because $M$ is in \emph{qnf} we have $lloc_M(t^\omega) = \bot$.


D. (a) (i) Suppose $t$ is a singleton then $t^\omega = \lambda\overline{\eta}$ is the root of $M$ so because $M$ is in \emph{qnf} we have $lloc_M(t^\omega) = \bot$.
...
\item If $M$ is in \emph{quasi normal form} and has beta-normal form $N$ then $\coresymbol(\travsetnorm(M)) = \coresymbol(\travsetnorm(N))$.
\item If $M$ is in \emph{quasi normal form} and $t$ is a traversal in $\travsetnorm(N)$ then $t$ is finite.
\end{enumerate}
\end{proposition}
\todo{
- explain where the relabelling done in $\coresymbol$ is used in the proof. Answer: during trivial reduction of the qnf, in (ii).
- explain where we use the weaving Property \ref{prop:weaving}???
}
...
(iii) Suppose $M$ is a \emph{qnf}. We first show that traversals of $\travsetnorm$ do not contain internal variables, and therefore do not make use of the \rulenamet{Var} rule.

Let $t\in\travsetnorm$.
Suppose that $t$ contains an internal variable then let $x_i$, $i\geq0$ denotes the first such occurence in $t$ with binder $\lambda\overline{x}$. Then $t$ is necessarily of the form $t = \ldots @ \lambda\overline{x} \ldots x_i$.
Let $U$ be the subterm rooted at $\lambda\overline{x}$. We show that $lloc_M(U) = \bot$

...

Hence $lloc_M(U) = \bot$. But since $x$ is the first occurence of an internal variable in $t$, all the variables occurring in $t_{<x}$ are external variables, and thus not involved in any generalized redex.
We can then show by induction on the path from the root to $\lambda x$ that necessarily $lloc_M(M) = \bot$. Indeed
For abstraction node we have by definition of $lloc$ we have $lloc_M(\lambda\overline{x}.$

%, therefore traversals are necessarily finite.
...
\end{property}
\begin{proof}
(i) If $M$ has a normal form then by Theorem \ref{thm:soundness_leftmostlinearred} its linear reduction sequence terminates, and by Prop.~\ref{prop:ulctrav_impl_linear_reduction}(i) the set of traversals of $M$ is isomorphic to the traversals of its quasi normal form. Hence if $M$ has an infinite traversal then so does its \emph{qnf}, which contradicts Prop.~\ref{prop:ulctrav_impl_linear_reduction}(iii).
