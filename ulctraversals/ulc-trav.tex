...
\item Let $n$ be a node occurring in $s$ we write $s\filter n$ to denote the subsequence of $s$ obtained by keeping only nodes that are hereditarily justified by $n$ in $s$. We call it the \defname{projection of $s$} with respect to $n$;
 ...
 \end{align*}
\end{itemize}
Observe that if $r_1, \ldots r_n$, $n\geq 1$ are the occurrences of the root in $s$ then  $\core{s}$ is also given by the projection of $s$ with respect to those occurrences: $\core{s} = s\filter r_1 \filter \ldots \filter r_n$.
\end{definition}
...
\end{proposition}
\begin{proof}
(i) Because the term is eta-long expanded, the condition $i>|m|$ in the rules \rulenamet{Var} and \rulenamet{IVar} never holds while traversing the term. (ii) holds because, by definition of $\travstlc$, the remaining traversals rules of STLC and ULC coincide.
\end{proof}
...
In this section we show how traversals can be used to normalize lambda terms. The crux of the normalization procedure is the \emph{Path Characterization} result (Proposition \ref{prop:path_charact_stlc} and Theorem \ref{thm:path_charact_ulc})) which states that the set of traversals \emph{core P-views} capture what it strictly required from traversals in order to reconstruct the normal form of a term.

This characterization suggests a normalization method based on enumerating the entire set of traversals. One caveat is that the set of traversals may be infinite! This is because the non-determinism of the free variable rule can give rise to arbitrarily long traversals.
We show, however, that not all traversals need to be enumerated: it is sufficient to enumerate a subset of traversals that covers all the core P-views.

We introduce the subset of \emph{branching traversals} which verifies this property. For simply typed terms in eta-long form this subset is finite. This yields a normalization procedure for STLC.
Soundness follows from the Game Semantics correspondence of \cite{BlumPhd}.
...
Branching traversals restrict the rule \rulenamet{IVar} so as to traverse only nodes leading to paths in the computation tree that are yet unexplored: when choosing the next lambda node to visit, it restricts choice (J) to the \emph{latest} variable node in the traversal, and choice (L) to child lambda nodes of that variable node.
...
We will use the subscript `${}_\branching$' to refer to the rule system thus obtained. Using a `derivation rule'-based presentation, the input variable rule can be stated follows:
...
For terms in eta-long normal form, the on-the-fly eta-expansion sub-cases of traversal rules are never used, therefore branching traversals are bounded in the sense that at a given point, only finitely many nodes can be visited. This explains the set of branching traversals is finite for terms in eta-long form, which implies that the enumeration of algorithm \ref{algo:stlc_normalization_by_traversals} terminates. In the next section we will explore how to further restrict the non-determinism of rule \rulenamet{IVar} to ensure that the enumeration also terminates for untyped terms with a normal form.


\subsection{Normalizing STLC by traversals}

The following characterization is a direct consequence of the Game-Semantic Correspondence Theorem for STLC \cite{BlumPhd}:
...
% next sentence is a clone! TODO: rewrite
This characterization suggests a normalization method for STLC term based on enumerating the entire set of traversals. There is just one caveat: the set of STLC traversals may be infinite! This is because the non-determinism of the free variable rule can give rise to arbitrarily long traversals. Observe, however, that the bijection in the above proposition holds for the \emph{quotiented} traversal set. Thus enumerating all traversals is not necessary: it suffices instead to enumerate a $\sim$-complete subset such as the subset of \emph{branching traversals}. Algorithm \ref{algo:stlc_normalization_by_traversals} describes such a normalization procedure.
...
\begin{enumerate}
  ...
  \item Project each traversal with respect to the tree root to get the \emph{traversal core}. By the correspondence theorem this yields a play in $\sem{M}$;
...
\paragraph{Soundness}
Soundness of Algorithm \ref{algo:stlc_normalization_by_traversals} is shown by Proposition \ref{prop:path_charact_stlc} and from the fact that a beta-normal term is uniquely determined by the set of maximal paths in its tree representation, and by Proposition \ref{prop:branching_traversal_normalizing} which shows that branching traversals are $\sim$-complete.
...
Algorithm~\ref{algo:ulc_normalization_by_traversals} describes the normalization procedure that computes the $\beta$-normal of an untyped lambda term if it exists (without eta-long expansion).
...
\paragraph{Soundness} The following theorem shows soundness of the procedure:
...
\end{theorem}
Observe that Conjecture \ref{conj:ulc_corresp} implies the above by the same game-semantic argument used to show characterization for STLC. We will instead prove the characterization using a term-rewriting argument based on the \emph{leftmost linear reduction} strategy. We sketch the argument below and refer the reader to section \ref{sec:correctness} for the proof.
...
\paragraph{Termination} If $M$ has a normal form then its set of tree paths is finite, therefore by the above theorem so is $\travsetnorm(M)/{\sim}$.
...
This process yields the set of traversals $\{t_\epsilon, t_{11}, t_{12}, t_{121}, t_{122} \}$. Table \ref{tab:varity2_trav_tesilon} represents $t_\epsilon$ and
Table \ref{tab:varity2_trav} representsthe three maximal traversals $t_{11}, t_{121}, t_{122}$.

\begin{table}
\label{tab:varity2_trav_tesilon}
\resizebox{1.1\hsize}{!}{$
