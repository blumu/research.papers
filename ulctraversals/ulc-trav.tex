...
We first prove the left-right inclusion by constructing inductively a mapping $\psi: \travsetnorm(M) \rightarrow \travsetnorm(N)$ such that for all $t \in \travsetnorm(M)$ there exists $u\in\travsetnorm(N)$ such that $\pi(\psi(t)) =\pi(u)$.

By induction on the traversal. It trivially holds for the empty, singleton and two-element traversals.
Let $t = t' \cdot n$ be a traversal in $\travsetnorm(M)$ where $t' = t'' \cdot m$ and suppose that there exists $u' \in \travsetnorm(N)$ such that $\pi(u') = \pi(\psi(t'))$.
