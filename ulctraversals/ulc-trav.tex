...
\item[\rulenamet{Var}] If \Pstr[0.5cm]{t\cdot(m){m}\cdot(beta){\beta}
    \ldots (n-beta,50:i){n}\in\travulc} for $m \in\ExtendedNodesVar \union \NodesApp$, $\beta \in \ExtendedNodesLmd\inter\InternalNodes$, $n \in \ExtendedNodesVar\inter\InternalNodes$, $i>0$ then we have
  $\Pstr[0.3cm]{ t  \cdot
...
\emph{Essential traversals} implement this idea by restricting the rule \rulenamet{IVar} to only traverse nodes leading to paths in the tree that the traversal has so far not explored: when picking the lambda node to visit next, it forces choice `(J)' to be the \emph{latest} variable node in the traversal. \emph{Concrete} essential traversals further restrict choice `(L)' to be a structural child lambda node of that variable node.
...
where the notion of compositionality is important,
allowing for infinitely many eta-expansions
in imaginary traversal is reasonable:
each eta-expansion represents how the term would interact with its arguments if more arguments were to be applied to it.
With the goal of finding a normal form, however, eta-expanding subterms ad-infinitum is excessive: traversing ghost nodes only helps if it eventually leads to some structural node of the final normal term tree.
...
\fi
We can thus calculate the dynamic arity by reading the strand backwards from the last variable occurrence and alternatively adding and subtracting the node arities until reaching an external variable. The maximal value observed throughout that summation gives the dynamic arity.
...
starts with a strand of type $S$ and transitions to
 strands of type $S$ and $G$ according to the following table:
\begin{center}
...
We preserve the original variable names from $R$, which does not cause capture of variables since the variable binding is uniquely determined by the enabling relation of the term tree.
We write $\lambda\overline{y}'$ to distinguish the root node of $R'$ from the root $\lambda\overline{y}$ of $R$ (although they are both labelled with variable names $\overline{y}$).
%
...
%
The case case $\rulename{Root}_\normalizing$ is immediate, using rule $\rulename{Root}_\normalizing$ itself for the simulation.
%
...
%
    ...
    Otherwise, we have $n = \lambda\overline{z}$ and $u$'s extension $u \cdot @_x$ can be simulated in $M$ using rule $\rulename{Lam}_\normalizing^{\sf var}$, extending $t$ as traversal $t \cdot x$.
    ...
    we can simulate $\rightarrow_M$
     with  $\rulename{Lam}^@_\normalizing$, otherwise with
     $\rulename{Lam}^{\sf var}_\normalizing$.
    ...
    To show that we can use $\rulename{Lam}^{\sf var}_\normalizing$ for the simulation we must show that any $\enables$-enabler of $m$ that is visible at $u$ is mapped to an enabler of $n$ that is visible in $\phi(u)$.
    %
    ...
    and node arities, therefore if the last variable occurring in $t$ is not $x$ then the simulation B1 and B2 can be done using $\rulename{Lam^\ghostvar_\normalizing}$ itself.
    %
...
%%%%%%%%%%  short version
            \begin{tikzpicture}[baseline=(root.base),level distance=4ex,inner ysep=0.5mm,sibling distance=8mm]
                ...
                +(1.5cm,0)
                node[midway, above, text width=1.5cm] {trivial reduction};
...
%%%%%%%%%%
\noindent
{\bf Case $r>1$} The reduct tree is a subtree of the original tree modulo minor relabelling.
...
%%%%%%%
\noindent
{\bf Case $r>1$} The reduct tree is obtained by removing the @-node and argument $A_1$ and combining the two lambdas into $\lambda\overline{y}$,
where $\overline{y} = \overline\eta \overline\eta x_2 \ldots x_n$.
%
...
We extend $\Phi$ element-wise to $\phi$ on justified sequences
...
%
%%%%%%%%%%
An induction on traversals shows that $\rightarrow_M$ can simulate $\rightarrow_N$ and reciprocally. One interesting case is when ghost nodes in $M$ get materialized into structural nodes in $N$:
...
$
%
A traversal $u = u_1 \cdot \lambda\overline{y} \in \travsetnorm(N)$
ending with $\lambda\overline{y}$ for some traversal $u_1$
gets simulated in $M$ by a traversal of the form
$t = t_1 \cdot \lambda\overline{\eta} \cdot @ \cdot \lambda\overline{x}$
where $t_1$ simulates $u_1$.
%%%%%%%%%%
...
By Proposition~\ref{prop:qnf_nf}(i), each such redex is trivial. By Proposition~\ref{prop:ulctrav_sound_for_trivialreduction}
the set $\core{\travsetscn(\_)}$ is an invariant throughout this trivial reduction sequence. And since $M_q$ is $\alpha$-equivalent to $N$ we have the desired structure-preserving isomorphism.
\end{proof}
...
%
If $M$ is not normal form then since $M$ has a beta-nf, by Theorem~\ref{thm:completeness_leftmostlinearred} its \emph{leftmost linear reduction sequence} terminates and yields a \emph{qnf}. By Proposition~\ref{prop:ulctrav_impl_linear_reduction} the set of traversal cores is preserved by \emph{linear reduction} thus the set of cores of  traversals of the \emph{qnf} is isomorphic to the traversal cores of $M$. Corollary~\ref{cor:qnf_and_nf_traveset_invariant} permits us to conclude.
\end{proof}
