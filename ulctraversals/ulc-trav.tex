...
The \defname{eta-long traversals} of a simply-typed term $M^\tau$ is defined as $\travset_{\concrete}(\etalf{M^\tau})$, the concrete traversals of its eta-long form.
\end{definition}

\begin{proposition}
\label{prop:ulc_and_stlc_trav_coincide}
The eta-long traversals of $M^\tau$ are also obtained from
$\etalf{M^\tau}$ by restricting only the imaginary traversal rule \rulenamet{IVar} to its concrete subcase.
\end{proposition}
\begin{proof}
