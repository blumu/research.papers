...
\begin{todobox}
I was expecting some diagrams to be given close to the definition of computation tree - a few are given much later in the paper (far from the definition). I picture might help the reader to fix to your notation.
\end{todobox}
...
Two justified sequences are considered \defname{equal} if they have the same structure and same variable names after appending each pending lambda list to the bound variables in the corresponding lambdas. Formally, given two terms $M$ and $N$ we define the relation
...
\item Let's eta-expand using rule \rulenamet{IVar} for child index $k_2$ ranging from $1$ to $q_2 = 2$. For the case $k_2 = 1$, we obtain the traversal:
...
$$
Using imaginary traversals, that operands gets created on-the-fly and is represented by ghost lambda node $\ghostlmd^1$.
