...
\begin{description}[nosep]
  ...
  \item[On-the-fly $\eta$-expansion] In the untyped setting, eta-long expansion is an infinite process, so instead of expanding the term prior to traversing it, imaginary traversals perform eta-expansion  `on the fly'. This occurs in rule \rulenamet{Var} when the arity of a variable in operand position is too low to statically determine the operand of an application (case $i>|n|$). When the variable arity is high enough ($i\geq|n|$), the definition of the rule coincides with STLC and the static tree representation of the term dictates the next node to visit.
...
Let $\Gamma \entail M : T$ be a simply-typed term in context. The set of traversals $\travstlc(\Gamma \entail M : T)$ from \cite{BlumPhd} is  the set of traversals of the \emph{eta-long form} of $M$ generated from the rules of Table~\ref{tab:trav_rules} with the exclusion of \rulenamet{Lam^\ghostlmd} and the `eta-expanded` sub-cases of \rulenamet{Var} and \rulenamet{IVar}.
\end{definition}
...
where $\travulc'$ denote the set of traversals obtained with rules of Table~\ref{tab:trav_rules} with the exclusion of the eta-expanded case in \rulenamet{IVar}.
\end{proposition}
\begin{proof}
...
Since the eta-expanded sub-case of \rulenamet{IVar} is excluded, traversals in $\travulc'(\etalf{M})$ do not contain any ghost node. Other rules coincide with STLC, therefore the equality holds.
\end{proof}
