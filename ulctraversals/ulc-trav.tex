...
A well-known result is that $\rightarrow^*_{h}$ yields the head-normal form. The \defname{normal-order reduction strategy}, also called leftmost-outermost reduction strategy, performs head reduction until reaching the head-normal form and then recursively applies head reduction on each operand of the head variable. A standard result is that this reduction strategy yields the normal form if it exists.
...
We define the \defname{enabling relation} $\enables$ on tree nodes $\Nodes$ as the relation mapping (i) each $\lambda$-node to all the variable nodes that it binds; (ii) the root of the tree to all the free variable nodes; (iii) variable and application node to each one of their child nodes.
...
The \defname{name-preserving encoding} of a justified sequence of nodes with variables in $\VarSet$ is a sequence of elements in
...
%
% NOTE: the actual node structure (i.e., its parent, children) is not recorded in the above two justified sequence encodings.
% But in practice, the sequences we will consider (traversals) will verify the visibility condition (every var/@-node points in the P-view), which suffices to uniquely determine the node $n\in\Nodes$ at any
...
%($lloc_M(U) \ne \bot$).
    ...
    `\emph{H2: If $lloc_M(T) \neq \bot$ and $hoc(M)$ is in $T$ then there is
     only a finite number of steps in the $\llred$-sequence of $M$ that involve linear substitutions in (reducts of) $T$.}'
%
...
%
    ...
    Let $\gamma$ denote the sequence of substitutions performed on $M$ to reduce the prime redexes, so that $\gamma(M)$ is the head normal form of $M$.  Then $\gamma(T_2)$ will become the (unique) argument of $x$ in the head normal form. And because $M$ has a $\beta$-nf, the term $\gamma(T_2)$ must also have a $\beta$-nf.
    %
        ...
        \begin{enumerate}[noitemsep,label=(\alph*)]
            ...
            \begin{compactitem}
                \item Suppose $T_1$ is a variable $x$ then it must be the \emph{hoc} and we have $T = x~T_2$.
                    ...
                    Let $\gamma$ denote the sequence of substitutions performed on $M$ to reduce the prime redexes, so that $\gamma(M)$ is the head normal form of $M$.
...
% left/right becomes top/bottom due to rotation
\newgeometry{left=0cm, right=1cm, bottom=0.4cm, top=0.4cm}
\begin{landscape}
\thispagestyle{empty}
\begin{table}
\resizebox{1\hsize}{!}{
