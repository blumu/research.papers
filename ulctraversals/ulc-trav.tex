...
Observe that given a fixed term, the canonical encoding of a justified sequence uniquely determines its structure, by just discarding the labels; and reciprocally, labels are uniquely recoverable from the sequence structure together with the enabling relation associated with that term.
...
When typesetting a sequence from $\justseqset(M)$, we represent occurrences by their associated node labels and represent the \emph{pending lambdas} in exponent. For example in the sequence $\cdots @ \cdot x \cdot \lambda\overline{x}^{[\overline{y}]} \cdots$, $\lambda\overline{x}$ is the label of the tree node associated with that occurrence and $\overline{y}$ are the \emph{pending lambdas} for this occurrence. We omit the exponent if the list of pending lambdas is empty. We adopt the quadruplet encoding throughout the rest of the paper and thus assume $\justseqset(M)$ to be a (strict) subset of $O_4^*$.
...
The \defname{readout function} $\readout: O_4^* \rightarrow \mathcal{L}^*$ converts a quadruplet-encoded justified sequence into a sequence of node labels in $\mathcal{L}$ (not necessarily present in nodes from $\ctree(M)$).
