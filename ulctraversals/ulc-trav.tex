...
\subsection{Imaginary traversals generalize STLC traversals}
...
For any simply-typed term in context $\Gamma \vdash M : T$, let $M^\eta$ denote the \emph{eta-long normal form}:
\begin{itemize}
\item[(i)] Traversals in $\travulc(M^\eta)$ (where $M^\eta$ is considered as an untyped term) do not contain any ghost node.
\item[(ii)] $\travstlc(\Gamma \vdash M : T) = \travulc(M^\eta)$.
\end{itemize}
\end{proposition}
\begin{proof}
...
\begin{eqnarray*}
 \travstlc(\Gamma \entail M : T) & \cong & \revsem{\Gamma \entail M :T} \\
 \travstlc^\coresymbol(\Gamma \entail M : T) & \cong & \sem{\Gamma \entail M :T} \enspace .
\end{eqnarray*}
where $\revsem{\Gamma \entail M : T}$ denotes the \emph{revealed game denotation} of $M$, $\sem{\Gamma \entail M : T}$ denotes the innocent game denotation of $M$,
and $ \travstlc^\coresymbol$ denotes the image of $\travstlc$ by the traversal core function $\coresymbol$ (\ie, set of justified sequences that are \emph{core} of some traversal of $M$).
\end{theorem}
...
We first consider the STLC case and derive an normalization procedure for it. We then spend the rest of the section showing how to extend this normalization procedure to ULC to calculate $\beta$-normal forms of untyped lambda terms when they exist.

\subsection{Normalizing STLC terms}

The Correspondence Theorem for STLC \cite{BlumPhd} gives the following characterization:
...
\begin{enumerate}
  \item Calculate $\etalnf{M}$, the eta-long expansion of $M$;
  \item Enumerate maximal \emph{branching} traversals of $M$ from Def.~\ref{dfn:branching_traversals} over the computation tree of $\etalnf{M}$;
...
The Game Semantics Correspondence explains why there can be infinitely many traversals: The game denotation of a term must allow Opponent's moves accounting for all possible context terms (modelled by the Opponent). Those traversals accounts for all the possible denotations of the function parameter $f$ of $M$: for each $k$, there exists a term that applies its argument $k$ times: $F_k = \lambda g . g (g ( \ldots (g z)))$ with $k$ applications of $g$, and there got to be plays in the game denotation of the term $M$ to accounts for those possible values of argument $f$.
...
\todo{Following two paragraphs are repeats, maybe move to intro section}
Game-semantic intuition: rule \rulenamet{IVar} from Table \ref{tab:trav_rules} accounts for all possible moves from the opponent. For instance, to model contexts terms like $\lambda f.f(\lambda x.f(\lambda y.y))$, the game denotation of a term $M$ must contain plays with two occurrences of the same O-move representing the two separate occurrences of $f$.
