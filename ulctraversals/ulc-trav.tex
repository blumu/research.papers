...
\emph{Types} We use the term-in-context notation
$\Gamma \entail M : T$ to denote that $M$ is a simply-typed term $M$ with type $T$ and context $\Gamma$ per the standard typing rules of the simply-typed lambda calculus~\cite{Barendregt84}, where
$M$ ranges over typed-terms generated by the grammar $E := x\, |\, (E\, E)\, |\, \lambda x\colon\tau. E $, $x$ ranges over variables $\VarSet$, $\tau$ is a simple type generated from the grammar $\tau := o\, | \tau \rightarrow \tau$, $o$ ranges over some fixed set of atomic types and $\Gamma$ is a sequence of typed-variables of the form $x:\tau$.
...
Traversals for simply-typed languages, such as STLC and PCF, were previously studied in~\cite{BlumPhd}.
 A notable limitation is that they are defined for eta-long forms only. Given a simply-typed term-in-context $\Gamma \entail M :T$, the \defname{eta-long form} of $M$ with respect to $T$, written $\etalf{M}$, is a simply-typed term eta-convertible to $M$ constructed inductively from $M$ by eta-expanding some of its subterms so as to make sure that every strict subterm that is not of base type necessarily has another term applied to it. We omit the formal definition~\cite{huet75-unification, BlumPhd, OngLics2006} which is not essential.

 We can apply the rules of Table~\ref{tab:trav_rules} on a typed-term $M$ by just erasing all its types. With this assumption, the STLC traversal from \cite{BlumPhd} can be reformulated as:
\begin{definition}[STLC traversal~\cite{BlumPhd}]
The traversals $\travstlc(M)$ of a simply-typed term $M$ of type $T$
are the justified sequences obtained by applying on $\etalf{M}$ the system of rules of Table~\ref{tab:trav_rules} with the exclusion of \rulenamet{Lam^\ghostvar}, and where \rulenamet{Var} and \rulenamet{IVar} are restricted to their concrete sub-case--that is justification label $i$ must be no greater than the arity $|m|$ of variable $m$.
\end{definition}
Note that in STLC, the lambda nodes of the computation tree are labelled by typed-variables instead of just variable names, but once the computation tree is produced from the eta-long form, the traversal rules do not make use of those types.

%Without loss of generality we will assume that $M$ is a closed term (no free variables).

This definition matches STLC traversals as originally defined~\cite{BlumPhd}, though in practice, imaginary and STLC traversals coincide on eta-long forms in a slightly more general way:
\begin{proposition}[ULC traversals subsume STLC traversals]
\label{prop:ulc_and_stlc_trav_coincide}
Let $\Gamma \entail M : T$ be a simply-typed term in context.
 Then $\travstlc(M)$ coincides with the
 justified sequences obtained by applying on $\etalf{M}$ the system of rules
of Table~\ref{tab:trav_rules} where \rulenamet{IVar} is restricted to the concrete subcase with the condition $i\leq|m|$.
\end{proposition}
\begin{proof}
...
\begin{algorithmic}
\REQUIRE{A simply typed term $M$ of type $T$.}
\ENSURE{The tree of the eta-long beta-normal form of $M$.}
\begin{enumerate}[nosep]
  \item Calculate the eta-long form $\etalf{M}$ of $M$ with respect to $T$;
...
Let $\Gamma \entail M :T$ be a simply-typed term-in-context. Then
 $\travstlc(M)$ is in bijection with the \emph{revealed game denotation} $\revsem{M}$ of the term;
 and its image by $\coresymbol$--the justified sequences that are \emph{core} of some STLC traversal--is in bijection with the innocent game denotation $\sem{M}$.
\end{theorem}
...
we have $\travstlc(M)/{\sim}\ = \pathset(N)$
where $N$ denotes the eta-long form of the beta-normal form of $M$ with respect to type $T$ and typed free variables $\Gamma$.
\end{proposition}
\begin{proof}
...
We showed in the previous section that for eta-long forms, there is a finite
number of branching traversals, which in turn implies termination of the
normalization procedure of Algorithm~\ref{algo:stlc_normalization_by_traversals}.
For untyped terms, however, branching traversals can be infinite because of \emph{on-the-fly eta-expansion} in rule $\rulename{IVar}$: the justification label $i>0$ in the traversal rule is unbounded. In this section, we show that,
...
\end{proposition}
\shortproof
(i) By finite induction on $q\in\{1..k\}$. Base case $q=1$ is taken care of by rule \rulenamet{IVar_\branching} followed by
