...
\todo{
Proof:
- explain where the relabelling done in $\coresymbol$ is used in the proof! (during trivial reduction of the qnf)
...
- Show where we use Theorem \ref{thm:path_charact_ulc}:
  Apply the bijection from the characterization theorem \ref{thm:path_charact_ulc} to get the corresponding path.
}
...
\paragraph{Soundness} of the normalization procedure then follows from the above theorem and the fact that a term is uniquely characterized by its set of paths $\pathset$ (Property \ref{prop:tree_path_charact}). The last step of the algorithm (assignment of labels to the tree nodes) is sound because by Property \ref{prop:tree_path_charact} justification pointers uniquely determine variables names.
