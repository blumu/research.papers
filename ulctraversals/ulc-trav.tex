...
A subsequence of a traversal is called a \emph{spinal descent} if it is a path in the computation tree consisting solely of lambda and $@$-nodes, and where each lambda node (except the first one if its the first node in the sequence) is the $0$th child of the preceding $@$-node.

\begin{proposition}
Let $M$ be a term in quasi-normal form. Let $t$ be a traversal of $M$. Consider any subsequence $u$ of $t$ of the form
$u = \underline{\lambda \overline{x}} \cdot v \cdot \underline{x}$
for some subsequence $v$ of $t$, variable node $x$ and lambda node $\lambda \overline{x}$.

\begin{enumerate}[label=(\roman*)]
  \item If all nodes in $v$ are internal (\ie, if $u$ is a \emph{weave}) then
  $u$ is a spinal descent of $M$.

  Further if $v$ is empty then the node following $u$ in $t$ is necessarily a structural child lambda node of $x$ (not a ghost node).

  \item If $v$ contains at least one external node then all nodes in $v$ are external.
  Further all nodes in $v$ are structural (no ghost node).

\end{enumerate}
\end{proposition}
\begin{proof}
(i) This trivially follows from the definition of the traversal rules \rulenamet{App} and \rulenamet{Lam}. If $u$ is empty then the arity threshold of $t$ is precisely $|x|$ therefore rule \rulenamet{IVar} can only be used to visit a structural child of $x$.
