...
\infrule[$\rulefont{IVar^\eta}_\affine$]
     {t = t_1 \cdot m \cdot t_2 \in \travsetaffine
     \andalso t^\omega\in \NodeHjByRoot \inter \NodesVar^\ghostvar
     \andalso (m,k,\ghostlmd) \in E(t)\setminus J(t)
     \andalso k > |m|
     }
...
\begin{definition}
We define the set of \defname{branching traversals} $\travsetbr$ as the subset of $\travset$ defined by the induction rules of Table \ref{tab:trav_rules}
with $\travsetbr$ substituted for $\travset$ in every rule, and where the input-variable rules are restricted as follows:
...
\infrule[$\rulefont{IVar^\eta}_\branching$]
     {t \cdot n \in \travsetbr
     \andalso n \in\NodeHjByRoot \inter \NodesVar^\ghostvar
     \andalso n \enables_k \ghostlmd
     \andalso k> |n|}
     {\Pstr[0.5cm]{t \cdot (n){n} \cdot (l-n,25:k){\ghostlmd}} \in \travsetbr}
\end{definition}
...
In this section, we show that, for the purpose of computing beta normal forms, the value $k$ in rule $\rulename{IVar^\eta}$ can be bounded by a computable quantity called the \defname{arity threshold} of the traversal at that point.
...
Intutively, after a sufficiently large number of eta-expansion, we are guaranteed to keep on traversing ghost nodes that never materialize back to structural nodes. This section formalizes this intuition by introducing the \defname{arity threshold} as the maximum number of time we want to eta-expand a give subterm using the branching traversal rule $\rulename{IVar^\eta}_\branching$ to compute the set $\pathset$. We will show later that such limit is sufficient in order to retrieve the normal form of a lambda term when it exists.

\begin{definition}[Traversal arity threshold]
\label{dfn:arity-threshold}
Let $t$ be traversal on which rule $\rulename{IVar^\eta}$ can be applied. Then $t$ ends with a variable node hereditarily justified by the root, let's call it $n_1$. We now consider the longest sequence of nodes preceeding $n_1$ in $t$ consisting only of nodes that are not hereditarily justified by the root, we have:
...
$\alpha_k$ is the only nodes, among the $2k$ nodes between $\alpha_k$ and $n_1$, that is hereditarily justified by the root.
...
Rule $\rulename{IVar^\eta}$ leaves infinitely many choice for the link label: any value greater than $1$. The following result shows that, for calculating paths in the tree representation of the beta-normal form of the term, it suffices to consider labels that are stricly smaller than the arity threshold.

%Consider a traversal on which we are about to apply rule $\rulename{IVar^\eta}$.
%Let's see what happens if we chose a value greater than the arity threshold of the traversal.



\begin{property}
Let $t \in \travsetbr$ be a branching traversal ending with occurrence $t^\omega$ of a variable node hereditarily justified by the root. Let's consider the longest sequence of nodes preceeding $t^\omega$ that are not hereditarily justified by the root:

$$ t = \cdots \underline{\alpha_k}\ n_k\ \alpha_{k-1}\ n_{k-1}\ \cdots\ \alpha_2\ n_2\ \alpha_1\ \underline{n_1} $$

where $n_1 = t^\omega$ and for some $k>0$, for all $1\leq j\leq k$, $\alpha_j \in \NodesLmd^\ghostlmd$,$n_j\in\NodesVar^\ghostvar$, and
all the $2(k-1)$ nodes occurring between $\alpha_k$ and $n_1$ (excluded) are hereditarily justified by some application node.

Let $t_{max}$ be a maxmimal traversal extending $t$. By rule $\rulename{IVar^\eta}_\branching$ the node immediately succeeding $t$ is a ghost lambda node $\ghostlmd$
justified by $t^\omega$ with label $i\geq 1$.


If $i>\arth(t)$ then $t^\omega$ is followed by $2k$ ghost nodes - alternating between lambda and variable ghost nodes - all hereditarily justified by an application node except the last ghost variable occurrence which is hereditarily justified by the root:
...
% (ii) Informally: Since all the added nodes are ghosts, their arity is 0 and therefore any further extension would only possibly lead to more ghost nodes!

% Let $\theta_2$ denote the second node justified by $\alpha$. How can the traversal be extended after that? Using the IVar\eta rule.

\end{property}
\begin{proof}
%
%$$ t_{max} = \Pstr[0.3cm]{ \cdots \alpha_k\ n_k\ \alpha_{k-1}\ %n_{k-1}\ \cdots\ \alpha_2\ n_2\ \alpha_1\ (m){t^\omega}\cdot  %(l-m,30:i)\ghostlmd \cdots } $$
...
\begin{itemize}
\item Base case $q=1$: Because $t^\omega$ is a ghost variable node hereditarily justified by the root, the only next rule that can be applied is \rulenamet{IVar^\eta}, and by assumption the following node is a ghost lambda node justified by $m$ with label $i_1 = i$. Then by rule \rulenamet{Lam^\ghostlmd} the next node is a ghost variable justified by $\alpha$ with label $i_2 = i + |\alpha_1| - |t^\omega|$.
...
\item What should be the value of $q$? In other words, how many more $k$ do we need to look at?  The answer: we need to keep iterating on $k$ until the point where applying the traversal rules will only produce ghost variables and ghost lambda-nodes! Because there is a finite number of nodes in the computation tree, the variable node arities are bounded. Therefore for high enough argument indices $k$, the condition $i\leq|n|$     in the definition of \rulenamet{Var} will never be met:
        after applying rule \rulenamet{IVar^\eta} on $t_0$, all subsequent extensions of the traversal will be done using either rule $\rulename{Var^\eta}$ or rule $\rulename{Lam^\eta}$.
