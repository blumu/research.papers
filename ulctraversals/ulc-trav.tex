...
Take the term $J = (\lambda u . u(x\,u)) (\lambda v . v\,y)$, whose computation tree is represented in Example~\ref{ex:missingoperand}. We have
$\Pstr[0.7cm][1pt]{(n0){\lambda}
(n1){@}
(n2-n1){\lambda u^0}
(n3-n2){u^1}
(n4-n1){\lambda v^1}
...
traversal ending with a ghost node. Once a traversal starts to visit ghost nodes, the only way for it to `return'  to structural nodes is via a ghost variable using an instantiation of \rulenamet{Var} producing a traversal of the form
$
...
\end{definition}
As mentioned earlier, the condition  $i-|m|>0$ only holds for the imaginary case of rules \rulenamet{Var} and \rulenamet{IVar}.
...
\begin{example}
The eta-expansion of $(\lambda u . u(x\,u)) (\lambda v . v\,y)$, with computation tree shown in Example~\ref{ex:missingoperand}, with respect to traversal $t$ from Example~\ref{ex:ulctrav_sample} is
$(\lambda u . u(x (\lambda \alpha. u (\lambda \beta.\alpha\,\beta))) (\lambda v . v\,y)$.
\end{example}
...
\begin{example}
Continuing with Example~\ref{ex:ulctrav_sample}: the following traversal is a valid traversal of $M^t$ where the occurrences corresponding to ghosts in $t$ are underlined:
$$\Pstr[0.7cm]{
 (n0){\lambda}
 (n1){@}
 (n2-n1){\lambda u}
...
\label{prop:essential_traversal_simcomplete}
  The essential traversals $\travsetes$ are $\travset$-complete.
\end{proposition}
\shortproof
See full details in \cite{longversion-2018arXiv}.
...
For typed languages such as the STLC, PCF or Idealized Algol, the theory of traversals~\cite{OngLics2006}, for eta-long forms, concretely relates to Game Semantics: they are in one to one correspondence with the plays of the game denotation in which all internal moves are revealed; and \emph{traversal cores} give the actual plays~\cite{BlumPhd}.
...
For any (closed) simply-typed term $M^\tau$ in \emph{eta-long form},
 $\travsetcon(M^\tau)$ is in bijection with the \emph{revealed game denotation} $\revsem{M^\tau}$; and its image by $\coresymbol$--the traversal cores--is in bijection with the innocent game denotation $\sem{M^\tau}$.
\end{theorem}
...
\begin{algorithmic}
\REQUIRE{A closed simply typed term $M^\tau$ in eta-long form of type $\tau$.}
\ENSURE{The tree of the beta-normal form of $M^\tau$.}
...
$\core{t_\epsilon} = \Pstr[0.7cm]{(n0){\lambda^{[y\,s\,z]}} \cdot (n9-n0){{\ghostvar^2}} } \equiv
\Pstr[0.7cm]{(n0){\lambda y s z} \cdot (n9-n0){s} }$
and therefore $\pview{\core{t_\epsilon}} = \lambda y s z \cdot z$ is a path in the normal form of $M$.
...
t_{11} = \Pstr[0.7cm]{
    ...
    (n20-n19,30){\underline{\ghostlmd^1}}
    %%%%%%%%
    (n21-n18,35){\ghostvar^1}
    (n22-n17,43:1)\lambda % points to non-ghost node
    (n23-n2,42) s
...
t_{12} = \Pstr[0.7cm]{
    ...
    %%%%%%%%
    (n21-n18){\ghostvar^2}
    (n22-n17,46:2)\lambda % points to non-ghost node
    (n23-n2,43) z
...
computation tree is represented in Figure~\ref{fig:comptree_examples},
...
\begin{example}
    Take the fixed-point combinator $\Omega$ whose computation tree is represented in Figure~\ref{fig:comptree_examples}. Let us add subscripts to the variable occurrences to distinguish individual occurrences of the same bound variable so that  $\Omega = (\lambda x. x_1 x_2) (\lambda y. y_1 y_2)$
...
\caption{The maximal traversals $t_{11}$, $t_{121}$ and $t_{122}$ of $varity\,2$ from Example~\ref{example:varity} yield the three maximal paths of the normal form $\lambda x y s z . s (x\, s (y\, s\, z))$. Strands boundaries are underlined.}
\label{tab:varity2_trav}
\end{table}
\end{landscape}
\restoregeometry
