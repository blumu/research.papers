...
\begin{itemize}[leftmargin=3em]
    ...
    (m){m} \cdot v \cdot  (gl-m,30:i){\ghostlmd}\, (al-beta,40:{|\beta| + i - |m|}){\ghostvar} \in\travulc}$.
    \end{compactitem}
\end{itemize}
    ...
    \infrule[$\rulefont{Var}$]
        {
            ...
            \ldots (n-beta,50:i){n}}
            \andalso n \in \ExtendedNodesVar \inter \InternalNodes
...
The \defname{lloc of a tree node} $n$, written
 $lloc_M(n)$, denotes $lloc_M(N)$ where $N$ is the subterm of $M$ rooted at $n$ and $lloc$ denotes the leftmost linear occurrence from Definition~\ref{def:leftmostlinearreduction}.
\end{definition}
...
for some subsequence $v$, external structural variable node $x$ and external structural $\lambda$-node $\lambda \overline{x}$.
If $v$ is empty then the node following $u$ in $t$, if it exists, is necessarily a structural child $\lambda$-node of $x$.
...
%%%%%%%%%%
\noindent
\emph{Case $r>1$} The reduct tree is a subtree of the original tree modulo minor relabelling.
...
%%%%%%%
\noindent
\emph{Case $r>1$} The reduct tree is obtained by removing the @-node and argument $A_1$ and combining the two lambdas into $\lambda\overline{y}$,
...
$ of $M$ for some $i\geq1$.
If $i>n$ then the corresponding traversal in $N$
is $u = \Pstr[10pt]{
...
$u = \Pstr[10pt]{
    (l){\lambda\overline y} \ldots  (gv-l,32:{|\eta|+i-1}){x_i} }
$.
%
A traversal $u = u_1 \cdot \lambda\overline{y} \in \travsetnorm(N)$
ending with $\lambda\overline{y}$ for some $u_1$
...
%
If $M$ is not a normal form then since $M$ has a beta-nf, by Theorem~\ref{thm:completeness_leftmostlinearred} its \emph{leftmost linear reduction sequence} terminates and yields a \emph{qnf}. By Proposition~\ref{prop:ulctrav_impl_linear_reduction} the traversal cores are preserved by \emph{linear reduction} thus the cores of traversals of the \emph{qnf} are isomorphic to the traversal cores of $M$. Corollary~\ref{cor:qnf_and_nf_traveset_invariant} permits us to conclude.
\end{proof}
