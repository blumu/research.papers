...
\label{examp:ghost_materialization}
    ...
    $(\epsilon, \epsilon ,0,0), (0,\epsilon,0,0), (00, \epsilon, 1,0), (000,\epsilon, 1,2), (02,\epsilon,3,2), (020,\epsilon,5,2) $ whereas the quadruplet sequence $ (\epsilon, g ,0,0), (0,\epsilon,0,0), (00, h~k, 1,0), (000,\epsilon, 1,2), (02,\epsilon,3,2), (020,\epsilon,5,2)$ encodes the justified sequence
    $\Pstr[10pt]{
        (l){\lambda x^{[g]}}  (a)@  (lyz-a,25:0){\lambda y z^{[h k]}}  (z-lyz,30:2)z \, (gl-a,40:2)\ghostlmd \, (gv-l,38:2)\ghostvar }
    $ which is equivalent to
    $\Pstr[10pt]{
        (l){\lambda xg} (a)@ (lyz-a,25:0){\lambda y z h k} \, (z-lyz,25:2)z \, (gl-a,35:2)\lambda \, (gv-l,40:2)g }
    $.
\end{example}
...
\coresymbol_{\epsilon}(s) \cdot \lambda \overline{x}\overline{y}
    & \mbox{if } n\in\ExtendedNodesLmd\inter\ExternalNodes, n \mbox{ labelled } \lambda\overline{x},\\
\coresymbol_{\overline{x} \otimes \overline{y}}(s)
    & \mbox{if } n\in\ExtendedNodesLmd\inter\InternalNodes, n \mbox{ labelled } \lambda\overline{x} .
...
(To keep the definition simple, (i) ghost lambda is identified with the dummy lambda, so that ghost lambda case is handle by the last two equalities; (ii) we do not bother renaming the variable occurrences when a variable get renamed via $\otimes$, this is because the name of variable occurrences can always be recovered from its justifier and justification label.)
\end{definition}

In the above definition, the subscript parameter $\overline{y}$ is called \defname{stack of pending lambdas}. As the sequence is read backwards, it accumulates variable names which then get discharched onto the next external lambda node read from the sequence. Hence the \emph{core projection} is the sub-sequence obtained by removing internal nodes and prepending to each external lambda node's label, the stack of \emph{pending lambdas} between that point to the next external node.
...
\begin{example}
    ...
    We have $\Pstr[10pt]{(l){\lambda x^{[z]}} \, (gv-l,32:2)\ghostvar }$, which is equivalent to traversal $\Pstr[10pt]{ (l){\lambda x z} \, (gv-l,32:2){z} }$ in $N$.
\end{example}
