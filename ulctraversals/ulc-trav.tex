...
i_{q+1} &=& i + \sum_{j=1..q} (|\alpha_j| - |n_j|), \quad \mbox{ for $0\leq q\leq k$}
\end{eqnarray*}
...
\begin{align*}
i \leq \arth(t)
        &\iff i \leq \max_{q=1..k-1} \left( |n_q| + \sum_{j=1..q-1} (|n_j| - |\alpha_j|) \right)\quad\hbox{(Definition of $\arth(t)$)} \\
        &\implies i \leq |n_r| + \sum_{j=1..r-1} (|n_j| - |\alpha_j|) \quad \mbox{for some $1 \leq r \leq k-1$} \\
        &\iff i + \sum_{j=1..r-1} (|\alpha_j| - |n_j|) \leq |n_r| \quad \mbox{for some $1 \leq r \leq k-1$} \\
...
Let $M$ be a term in quasi-normal form. Let $t_{max}$ be a maximal traversal of $M$. Then $t_{max}$ consists of a succession of strands of the following forms:
\begin{center}
\begin{tabular}{lll}
If the previous strand is & Then next strand is  & Case \\
\hline \hline \\
$\epsilon$ & $\underline{\lambda\overline{\eta_1}} \cdot u_1 \cdot \underline{x_1}$ & A\\ \\
\hline \\
\multirow{3}{*}{$\underline{\lambda\overline{\eta_1}} \cdot u_1 \cdot \underline{x_1} $}
 & $\underline{\lambda\overline{\eta_2}} \cdot u_2 \cdot \underline{x_2}$ & B1 \\ \\
 & $\underline{\ghostlmd} \cdot  v \cdot \lambda\overline{y_2} \cdot u_2 \cdot \underline{x_2}$ & B2 \\ \\
\hline \\
\multirow{3}{*}{$\underline{\ghostlmd} \cdot  v \cdot \lambda\overline{y_1} \cdot u_1 \cdot \underline{x_1}$}
& $\underline{\lambda\overline{\eta_2}} \cdot u_2 \cdot \underline{x_2}$ & C1
\\ \\
  & $\underline{\ghostlmd} \cdot v \cdot \lambda\overline{y_2} \cdot u_2 \cdot \underline{x_2}$ & C2
\end{tabular}
\end{center}
where
    \begin{itemize}
...
\end{proposition}
\begin{proof}
(A) The first strand is obtained by applying the rule \rulenamet{Root} followed by repeated applications of \rulenamet{App} and \rulenamet{Lam}, which yields $u_1$, a path to $\lambda\overline{\eta_1}$ in the tree of $M$. To show that $u_1$ is a spinal descent we need to prove that it does not contain any internal node (only @-node). This must be the case otherwise, since internal variables are by definition involved in some generalized redex, we would have $lloc_M(\underline{\lambda\eta_1}) = z$ where $z$ is the first internal variable occurring in $u_1$, which contradicts $lloc_M(\lambda \overline{\eta}) = lloc_M(M) = \bot$ since $M$ is in \emph{qnf}.

(B1) If the node following $x_1$ is a structural node $\lambda\overline{\eta_2}$ then by rule \rulenamet{IVar}, it's a child lambda node of $x_1$.
Because $u_1$ is a spinal descent, the subterm rooted at $\lambda \overline\eta_1$ is of the form $\lambda \overline\eta_1 . x_1 A_1 \ldots A_q$ for some $q\geq1$, therefore we have $lloc_M(\lambda \overline\eta_1 . x_1 A_1 \ldots A_q) = \bot$. Since $x_1$ is external, by Property \ref{prop:qnf_longapply} we have $lloc_M(A_j)=\bot$ for all $1\leq j\leq q$, so in particular $lloc_M(\lambda\overline{\eta_2}) = \bot$. The same logic used in (A) lets us conclude that $u_2$ is a spinal descent to external node $x_2$.


(B2) If the node following $x_1$ is a ghost node then by Property \ref{prop:strand_spinaldescent} $u_1$ is necessarily non-empty: we have $u_1 =
@_1 \cdot\lambda\overline{\xi_1} \cdots @_q\cdot \lambda\overline{\xi_q}$ for some $q\geq 1$. By the weaving property \ref{prop:weaving}, the node follwing $x_1$ is itself followed by a \emph{pending parameter lookup} $v$ that is necessarily shorter than $u_1$, and is itself followed by a structural lambda node $\lambda\overline{y_2}$ justified by some application node $@_r$ in $u_1$ for some $1\leq r \leq q$:
\begin{align*}
 t &= \Pstr[10pt]{
...
\ghostvar_{k_{r}}
\end{align*}
where, for all $i$ ranging from $r$ to $q$, ghost nodes
$\ghostvar_{k_i}$ and $\ghostlmd_{k_i}$ point respectively to $\lambda\overline{\xi_i}$ and $@_i$ with link label $k_i\geq1$ defined by:
\begin{align}
  k_i &= k + |\lambda\overline{\xi}_q| - |x| + \sum_{i\leq j < q} (|\lambda\overline{\xi}_{j}| - |@_{j+1}|)
%\\
...
\\
  ...
  \sum_{i< j\leq q} (|\lambda\overline{\xi}_j| - |@_j|) \label{eqn:ki_k}
\end{align}
We introduce the abbreviation $\gamma_i = \sum_{i\leq j\leq q} (|\lambda\overline{\xi_j}| - |@_j|)$ so that:
\begin{align*}
\gamma_{i} &= \gamma_{i+1} + |\lambda\overline{\xi_j}| - |@_j| \\
k_i &= k - |x| + |\lambda\overline{\xi}_{i}| + \gamma_{i+1}
\end{align*}

Let's consider the subterm at $@_r$, represented on Figure\ref{fig:strand_decomposition_proof_induction}. It is of the form
$$(\lambda\overline{\xi_r}. A_0) A_1 \ldots A_{k_{r}-1}\ (\lambda\overline{y_2}. \ldots) \ldots A_{|@_r|}$$

We show by a finite induction on $i$ ranging from $q$ down to $r+1$, that
\begin{enumerate}[label=(\alph*)]
\item $lloc_M(\lambda\overline{\xi_i}) =\bot$;
\item the lambda-list at $@_i$ is of length $\gamma_i$.
\end{enumerate}

\emph{Base case:} We have $i=q$ (and $v=\epsilon$). Because $x$ is an external variable we necessarily have $lloc_M(\lambda\overline{\xi_q}) = \bot$. Further the subterm at $\lambda\overline{\xi_q}$ is head normal therefore its head lambda-list is precisely of length $|\lambda\overline{\xi_q}|$ (the number of variables in the abstraction), and consequently the head lambda-list at $@_q$ is given by $|\lambda\overline{\xi_q}|-|@_q| = \gamma_q$.

\emph{Induction case:} Suppose $i<q$ and assume the result is true for $i+1$.
By definition of head-lambda list, the length of the head lambda-list at $\lambda\overline{\xi_i}$ is precisely given by the number of variables $|\lambda\overline{\xi_i}|$ in the abstraction plus the length of the head lambda-list at $@_{i+1}$:
\begin{align*}
|\lambda_l(\lambda\overline{\xi_i})| &=
  |\lambda\overline{\xi_i}| + |\lambda_l(@_{i+1})| \\
  &= |\lambda\overline{\xi_i}| + \gamma_{i+1}  \qquad\mbox{by I.H.}\\
  &=  \gamma_{i} + |@_i| \qquad\mbox{by def.~of $\gamma$}
\end{align*}
Since by definition $\gamma_i$
The length of the head lambda-list at $@_i$ is by definition given by

The which is itself given
by $k$
is precisely of length $|\lambda\overline{\xi_a}|$ (the number of variables in the abstraction), and consequently the head lambda-list at $@_q$ is given by $|\lambda\overline{\xi_q}|-|@_q|$.

\begin{figure}[htbp]
\centering
% \begin{subfigure}[b]{0.4\textwidth}
% \begin{tikzpicture}[baseline=(root.base),level distance=6ex,inner ysep=0.5mm,sibling distance=10mm]
% \node (root)
% {$\cdots$}
% child {node{$\lambda\overline\eta$}
% child {node{$...$}
% child{node{$@_q$}
...
%             child{node{$\ldots$}
%             }
%              edge from parent node[right]{$k_q$}
%         }
%         child{node{$\cdots$}}
%     } }
% };
% \end{tikzpicture}
% \caption{Base case $q=r$}
% \label{fig:strand_decomposition_proof_basecase}
% \end{subfigure}
%
%\begin{subfigure}[b]{0.4\textwidth}
\begin{tikzpicture}[baseline=(root.base),level distance=7ex,inner ysep=0.5mm,sibling distance=15mm]
    \node (root)
    {$\cdots$}
    child {node{$\lambda\overline\eta$}
        child {node{$@_1$}
            child{node{$\ldots$}
                child{node{$@_{r}$}
                    ...
                    child{node{\ldots}}
                    child{node{} edge from parent node[left]{$|@_r|$}}
                }
                edge from parent node[above left]{$0$}
            }
            child{node{\ldots}}
...
%\end{subfigure}
\caption{Sub-tree at $\lambda\overline\eta$ for a strand of type B2}
\end{figure}

\emph{Induction case:}

Let's first show that $lloc_M(\lambda\overline{y_2})=\bot$.
By assumption we have that $lloc_M(\lambda\overline{\eta}) =\bot$ therefore since $@_r$ occurs in the spine of $M$, by definition of $lloc_M$ we necessarily have $lloc_M(@_r) =\bot$, (and thus also $lloc_M(\lambda\overline{\xi_r}) = \bot$). Let's calculate the length of the lambda list at $\lambda\overline{\xi_r}$:
\begin{align}
    |\lambda_l(\lambda\overline{\xi_r} . A_0)| &=
    |\lambda\overline{\xi_r}| + |\lambda_l(A_0)| \\
&= |\lambda\overline{\xi_r}| + |\lambda_l(@_{r+1})| \qquad\mbox{(since $A_0$ is the subterm rooted at $@_{r+1}$)}\\
&= |\lambda\overline{\xi_r}| + \sum_{r+1 \leq j\leq q} (|\lambda\overline{\xi_j}| - |@_j|) \qquad\mbox{(by induction hypothesis)}\\
&= |@_r| + \sum_{r \leq j\leq q} (|\lambda\overline{\xi_j}| - |@_j|) \\
&= k_r - k + |x| \qquad \mbox{(by Eq.\ref{eqn:ki_k} with $i=r$).}
\end{align}
But because $\lambda\overline{y_2}$ is a structural node by Property~\ref{prop:ghost_justifier_arity} we have $k>|x|$ and therefore $|\lambda_l(\lambda\overline{\xi_r} . A_0)| < k_r$. This means that in the sub-term $(\lambda\overline{\xi_r} . A_0) A_1 \ldots A_{k_r-1}$, there are more operands than the length of the lambda list of the operator, which by definition of the lambda list, implies that $\lambda_l((\lambda\overline{\xi_r} . A_0) A_1 \ldots A_{k_r-1})$ is empty. Hence we necessarily have $lloc(\lambda\overline{y_2}) = \bot$, otherwise by definition of $lloc$ we would have
 $lloc((\lambda\overline{\xi_r} . A_0) A_1 \ldots A_{k_r-1} (\lambda\overline{y_2}. \ldots)) = lloc(\lambda\overline{y_2}) \ne \bot$, which would subsequently imply $lloc(\lambda\overline{\eta_1})\ne\bot$, contradicting the assumption.
...
(C1) \& (C2) %are shown using the same argument as (B1) and (B2).
\end{proof}
...
observe that in case (A), (B1), because $u_1$ and $u_2$ are spinal descents, the last node in the `next` strand ($x_1$ or $x_2$) must be a descendant (in the computation tree representation of $M$) of the first node from the `previous' strand ($\lambda\overline{\eta_1}$).
In case (B2), since both $u_1$ and $u_2$ are spinal descents, there are also paths in the tree, therefore since $\lambda\overline{y_2}$ points to $u_1$ we also have that $x_2$ is a descendant of $\lambda\overline{\eta_1}$. In case (C1) and (C2), since $u_1$ and $u_2$ are spinal descents, $x_2$ is a descendant of $\lambda\overline{y_2}$, itself a descendant of $\lambda\overline{y_1}$.
...
Let $M$ be an untyped term.
\begin{enumerate}[label=(\roman*)]
\item If $M \rightarrow_l N$ then $\travsetnorm(M) \cong \travsetnorm(N)$;
\item If $M$ is in \emph{qnf} and has beta-normal form $N$ then $\coresymbol(\travsetnorm(M)) = \coresymbol(\travsetnorm(N))$.
\end{enumerate}
\end{proposition}
\todo{
- finish proof
- explain where the relabelling done in $\coresymbol$ is used in the proof. Answer: during trivial reduction of the qnf, in (ii).
}

\begin{proof}[Proof sketch]
(i) Proof sketch: The effect of leftmost linear reduction is essentially to inline the \emph{lloc} variable by the argument of the generalized redex involving it. The sets of traversals $\travsetnorm$ before and after reduction are identical up to some mapping between nodes of the term and its reduct.
