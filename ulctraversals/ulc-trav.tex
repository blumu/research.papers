...
This induces a method to evaluate simply-typed lambda-terms~\cite{BlumPhd,BlumGalop2008,Blum-HogTool,Ong-NormByTrav2015}, previously implemented in~\cite{Blum-HogTool}, which we state in Algorithm~\ref{algo:ulc_normalization_by_traversals}. This paper generalizes the evaluation procedure to the untyped lambda calculus,
based on an extended notion of `projection with respect to the root'
that involves relabelling lambda nodes. A notable improvement is that it produces the traditional beta-normal form rather than the beta-\emph{eta-long} normal form. Another difference is that we prove soundness of the procedure through a connection with \emph{head-linear reduction} rather than appealing to Game Semantics.

\subsubsection{Head linear reduction}

\begin{todobox}
Danos \etal showed the connection between game models of simply-typed lambda terms and \emph{head-linear reduction} for eta-long form
%and proposed \emph{Pointer Abstract Machine (PAM)} as an abstract machine implementation of this reduction~\cite{danosherbelinregnier1996}.
\end{todobox}
Danos \etal established the first connection between Game Semantics and the concept of \emph{head linear reduction} for simply-typed lambda terms in eta-long form~\cite{danosherbelinregnier1996}. It was was further studied in the context of untyped lambda terms in \cite{danos-head}. Despite this connection, their work does not explicitly describe how to use head linear reduction to fully evaluate an untyped lambda term. The \emph{pointer abstract machine} (PAM) introduced in the same papers, for instance, yields the ``\emph{quasi-head normal form (qhn)}'' of the term, not the normal form itself. The resulting \emph{qhn} still needs to be first reduced to head normal form (using head reduction) and then normalized using standard normal reduction, which must terminate by the $\lambda$-calculus theory.

Traversals are essentially a recursive implementation of the head linear reduction strategy on lambda terms. A traversal starts, in a call-by-name-like fashion, with a depth-first search for \emph{head occurrence} of the \emph{hoc redex} as defined by Danos-Regnier, and implements linear substitution by ``jumping'' to the node in the tree representing the sub-term to be substituted for that head occurrence. It then continues its descent to look for the next head occurrence, as if that first head occurrence had been replaced by the redex argument. A property of head-linear reduction is that all the terms appearing in the reduction sequence consist of sub-terms of the original term. This property means that there is no need for the traditional notions of environments or closures to reduce beta-redexes. Traversals implement such a reduction using a system of pointers that reference subterms of the original term.

Imaginary traversals effectively implement the \emph{leftmost linear reduction} strategy, a notion formalized in the present paper, that recursively applies head linear reduction to find the normal form; just like how the \emph{normal reduction} of the lambda calculus proceeds by recursively applying head reduction. In particular, they implement what Danos \etal call ``the gymnastics to find the argument of a subterm''~\cite{danos-head} via the mechanism of `weaving' (Proposition~\ref{prop:weaving}),
...
\caption{Maximal traversals $t_{11}$, $t_{121}$ and $t_{122}$ of $varity\,2$ from Example~\ref{example:varity}}
