...
\cdots
\ghostlmd_{k_{r-1}} \cdot \ghostvar_{k_{r-1}}
\end{align*}
...
\begin{align}
  k_i &= k + |\lambda\overline{\xi}_q| - |x| + \sum_{i\leq j \leq q-1} (|\lambda\overline{\xi}_{j}| - |@_{j+1}|)
%\\
...
\\
  ...
  \sum_{i+1\leq j\leq q} (|\lambda\overline{\xi}_j| - |@_j|)
\end{align}

Further, since all the nodes in $v$ are ghost nodes and $\lambda\overline{y_2}$ is not, by Property~\ref{prop:ghost_justifier_arity} we have:
\begin{align}
k > |x| \\
...
We show by induction on $q-r$ that $lloc_M(\lambda\overline{\xi_r}) =\bot$ and that the lambda-list at $@_r$ is of size $\sum_{r+1 \leq j\leq q} |@_j| - |\lambda\overline{\xi_j}|$.
\emph{Base case:} If $q=r$ (in which case $v=\epsilon$) then $\lambda\overline{y_2}$ and  $\lambda\overline{\xi_q}$ share the same parent $@_q$, and $\lambda\overline{y_2}$ is the $k_q$th child of $@_q$.
...
\emph{Induction case:} Suppose it's true for all $r<j\leq q$, then because
...
\begin{align}
  |\lambda_l(\lambda\overline{\xi_r} . A_0)| &= \sum_{r+1 \leq j\leq q} |@_j| - |\lambda\overline{\xi_j}| \\
  &= k - |x| + |\lambda\overline{\xi_r}| - k_r \\
  &> |\lambda\overline{\xi_r}| - k_r \quad (by Eq.\ref{}) \\
  &> 
\end{align}
