...
In the following sections we show how to generalize the normalization procedure of Algorithm~\ref{algo:stlc_normalization_by_traversals} to calculate the $\beta$-normal of an untyped lambda terms if it exists (without eta-long expansion).
...
\end{theorem}
Observe that Conjecture \ref{conj:ulc_corresp} implies this result by the same game-semantic argument used to show characterization for STLC. We prefer instead  an alternative route and prove characterization using a term-rewriting argument based on the \emph{leftmost linear reduction} strategy.
\begin{proof}[Proof sketch]
%$$\pview{\core{\travsetnorm(M)}} = \pathset(N) .$$
It's trivial if $M$ is in beta-normal form by the Path-View correspondence for ULC and since $\core{\travsetnorm(M)} = \travsetnorm(M)$.
For any other untyped term $M$ with a normal form, we consider its \emph{leftmost linear reduction sequence}.
Since $M$ is in beta-nf, the reduction sequence terminates by Prop.~\ref{prop:llred_terminates}
and yields the \emph{quasi normal form} (qnf).
\todo{Add prop:llred_terminates}
...
The set of traversals before adn after reduction are therefore very close and it's easy to show that the
invariant $\core{\travsetnorm(M)}$ is preserved.
...
\begin{remark}
The infinitary counterpart for lambda terms with no normal form (not shown in this paper) can be stated as:
