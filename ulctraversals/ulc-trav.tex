...
$\ghostvar_{k_i}$ and $\ghostlmd_{k_i}$ points respectively to $\lambda\overline{\xi_i}$ and $@_i$ with link label $k_i\geq1$ uniquely determined from $k$ and the arity of the nodes in $u_1^+$,
Further, since all the nodes in $v$ are ghost nodes we have:
\begin{align}
k_j > |@_j| - |\lambda\overline{\xi_j}| & \quad\mbox{for all $r+1\leq j \leq q$ } \label{eqn:qnfdecomp_kj}\\
k_r \leq |@_r| - |\lambda\overline{\xi_r}|
\end{align}
\\

We first show by induction on $q-r$ that $lloc_M(\lambda\overline{\xi_r}) =\bot$ and that the lambda-list at $@_r$ is of size $\sum_{r+1 \leq j\leq q} |@_j| - |\lambda\overline{\xi_j}|$.
...
By assumption we have that $lloc_M(\lambda\overline{\eta}) =\bot$ therefore since $@_r$ occurs in the spine of $M$, by definition of $lloc_M$ we necessarily have $lloc_M(@_r) =\bot$. We have just shown by the above induction that $lloc_M(\lambda\overline{\xi_r}. A_0) = \bot$, that is the $0$th branch of $@_r$ has undefined lloc. Hence by Property \ref{prop:qnf_longapply}, we necessarily have $lloc_M(\lambda\overline{y_2}) = \bot$.
