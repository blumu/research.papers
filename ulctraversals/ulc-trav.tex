...
% head variable occurence (from Danos-Reigner def)
...
$\lambda \overline{x} . U$ if $\overline{x}$ denotes the variable list $x_1 \ldots x_n$.
...
The set of nodes $\Nodes$ of the computation tree all correspond to some tokens in the syntax representation of the lambda term. We refer to them as \defname{structural nodes}.

We now complement them, with two additional infinite sets of fictitious nodes:
(i) the set of ``ghost variable nodes'', denoted $\ghostvar$, (ii) the set of ``ghost lambda nodes'', denoted $\ghostlmd$. Together, nodes in $\ghostlmd$ and $\ghostvar$ are called the \defname{ghost nodes}. We write $\ExtendedNodes$ for the extended set of nodes $\Nodes + \ghostvar + \ghostlmd$; $\ImNodesLmd$ as a shorthand for $\NodesLmd + \ghostlmd$ and $\ImNodesVar$ as a shorthand for $\NodesVar + \ghostvar$.
...
Note that the enabling relation is \emph{statically} induced by the structure of the tree whereas the justification relation is defined on node \emph{occurrences} in one specific justified sequence.

We call \defname{set of justified sequences over $M$} the set of all possible
 justified sequences over the extended set of nodes $\ExtendedNodes$ of the computation tree.

 We now define an embedding for justified sequences that will allow us to easily define transformation involving relabelling of lambda nodes.

\begin{definition}[Quadruplet encoding]
    \label{def:quadruplet_encoding}
    Let $\mathcal{V}^*$ denote the set of sequences of variable names taken from $\mathcal{V}$ and $\mathcal{P}$ denotes the set of paths to (extended) nodes of the computation tree. We define $\justseqset(M)$ as $O_4^*$, the set of sequences of elements in $O_4 =  \mathcal{P}\times \mathcal{V}^* \times\nat\times\nat$.

    A justified sequence of nodes over $M$ is encoded in $\justseqset(M)$ by mapping each occurrence to $O_4$ as follows:
    \begin{enumerate}[nosep]
        \item (Node) The first component is the path to an extended node of the computation tree that uniquely define the type and label of the node occurrence.
...
The `pending lambdas' component represents a list of variable names to be appended to the variables bound by the lambda node. It is redundant when encoding raw justified sequences  of $M$, but it allows us to define transformation on justified sequences involving alterations to lambda abstractions.
...
%It will only be used for nodes of lambda type; for other nodes it will always be the empty list.
 When typesetting a sequence from $\justseqset(M)$, we represent occurrences by their associated node labels, and we write the `pending lambdas' in exponent:
...
We will now show how traversals can be used to evaluate lambda terms and find the normal form when it exists. The crux of it lies in the \emph{Path Characterization} result (Proposition~\ref{prop:path_charact_stlc} and Theorem~\ref{thm:path_charact_ulc})) which states that the set of traversals \emph{core P-views} captures what is strictly required from traversals in order to reconstruct the normal form of a term.
...
This characterization result suggests a normalization method based on enumerating the entire set of traversals. The caveat is that the set of traversals may be infinite, even for terms with a beta-normal from! This is because the non-determinism of the free variable rule can give rise to arbitrarily long traversals.
...
We define the core P-view function
$\pview{\core{\_}} \colon \travulc \longrightarrow \justseqset $
as the composition of $\pview{\_}\colon \justseqset \longrightarrow \justseqset$ with the core projection $\coresymbol\colon\travulc  \longrightarrow \justseqset$ that is $t \longmapsto \pview{\core{t}}$.
...
\FORALL{node $n \in P$ (by depth-first enumeration)}
    ...
    \ELSIF{$n \in\ghostlmd$}
        ...
        \COMMENT{Will prefix names of variable bound by $n$}
...
The computation tree of $M$ is:   %\begin{wrapfigure}[4]{l}{0pt}
...
\\
     t_2 = \Pstr[0.7cm]{(n0){\lambda }\ (n1){@}\ (n2-n1){\lambda u}\ (n3-n2){u}\ (n4-n1){\lambda v}\ (n5-n4){v}\ (n6-n3){\lambda }\ (n7-n0){y_1}\ (n8-n7){{\ghostlmd^{2}}}\ (n9-n6){{\ghostvar^{1}}}\ (n10-n5){\lambda }\ (n11-n0,30){y_2}\ }
\end{align*}
\end{tabular}


With STLC traversals, one can traverse $t_1$ all the way to variable $v$ at which point one gets stuck due to the lack of operand applied to the last occurrence of $u$ (the occurrence  at the bottom of the left branch in the tree). Using imaginary traversals, that operands gets created on-the-fly through eta-expansion and is represented by ghost lambda node $\ghostlmd^1$. The two core P-views give the two maximal paths in the beta-normal form:
...
%%%%%%
thus the beta-normal form of $M$ is $y_1 (\lambda  z.z\ y_2) y_2$.
\end{example}
...
This process yields the set of traversals $\{t_\epsilon, t_{11}, t_{12}, t_{121}, t_{122} \}$.  The traversal $t_\epsilon$ is represented below:
\resizebox{1\hsize}{!}{$
...
Keeping only the maximal traversals gives the set $\travsetnorm_{\max}(M) = \{ t_{11}, t_{121}, t_{122} \}$ represented in Table~\ref{tab:varity2_trav}.
...
\begin{align*}
\betanf{varity\ 2} = \lambda x_1 x_2 x_3 x_4 . x_3 (x_1 x_3 (x_2 x_3 x_4)) \equiv_\alpha \lambda x y s z . s (x s (y s z))
\end{align*}
\end{example}
...
The \defname{head-linear reduction} $\hlred$ is defined as the reduction that linearly fires the head linear redex, if it exists. It can be shown that the reflexive transitive closure $\rightarrow^*_{hl}$ yields the quasi head-normal form.
...
Observe that if $M$ is in \emph{qnf}, its redexes are not all necessarily trivial, but the non-trival redexes get eliminated by the trivial reductions:
\begin{example}
...
\end{property}
\begin{proof}
    (i) and (ii) follow from the fact that performing substitution in an operand of an application does not affect its lambda-list, while substituting a variable in operator position in the application, appends the lambda-list of the substituted term to the lambda-list of the application.
...
\begin{definition}[Strand types]
    \label{def:strandtypes} We distinguish two types of strands based on their shape:
    \begin{itemize}[nosep]
...
Let $M$ be an untyped term.
$$M \llred N \implies \coresymbol(\travsetnorm(M)) \cong \coresymbol(\travsetnorm(N)) \ .
$$
\end{proposition}
%\begin{proof}
\proofatend
...
\end{corollary}
\begin{proof}
By Proposition~\ref{prop:qnf_nf}(ii) a \emph{qnf} trivially reduces to its beta-nf: there is a beta reduction sequence $M = M_1 \rightarrow_\beta M_2 \ldots \rightarrow_\beta M_q= N$, $q\geq1$  where for each reduction step, $M_{i+1}$ is obtained by reducing the leftmost spinal-innermost standard redex of $M_i$ (of the form $(\lambda x . U) T$ such that $U$ does not contain any redex). By Proposition~\ref{prop:qnf_nf}(i), each such redex is trivial: $x$ does not occur freely in $U$ and therefore no substitution is performed to reduce the redex.
