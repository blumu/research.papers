...
In the typed setting, the traversal theory bears a strong connection with Game Semantics: The game denotation of a term is in bijection with the set of \emph{core projections} of its traversals (Theorem~\ref{thm:gamesem_correspondence_stlc}), where the core projection of a traversal is the subsequence obtained by keeping only occurrences that are
...
The read-out phase of the algorithm appeals to a path-correspondence property of traversals to extract tree paths of the normal form the traversals. In the above example, traversal $t$ yields the path $\lambda z \cdot z$. And since the term has a single traversal, its beta-normal form is $\lambda z. z$, up to variable renaming.
 ...
 We have implemented the normalization procedure in HOG, a tool used to study traversals and pushdown automata generated from Higher-Order Grammars and lambda terms~\cite{Blum-HogTool}. The last section provides examples of untyped lambda terms that were normalized using it.
...
In~\cite{BlumPhd} we extended the theory of traversals to the simply-typed lambda calculus and formalized its correspondence with  Game semantics. The thesis introduced a new traversal rule to model free variables, a rule that is not needed in the original presentation of traversals since higher-order recursion schemes are necessarily closed terms of ground type~\cite{Ong2006}.
It introduces a projection operation, called \emph{projection with respect to the tree root}, which returns the ``core'' of a traversal.
Traversals are shown to correspond to the \emph{interaction game semantic denotation} of a term, a variant of the standard game semantics where all internal moves from strategy composition are revealed. The core projection operation yields the correspondence with the standard Game Semantics. This correspondence induces a method to reduce beta-redexes in eta-long simply-typed lambda-terms that was studied in~\cite{BlumPhd,BlumGalop2008,Blum-HogTool,Ong-NormByTrav2015} and implemented in HOG, a software tool to study the traversal theory of higher-order grammars and lambda terms~\cite{BlumGalop2008, BlumPhd}. We recall these results in Section~\ref{sec:traversal_correspondence_stlc}.


 The present paper generalizes this work to the untyped case. Adapting the traversal theory to the untyped setting requires new ingredients detailed in the rest of the paper. In particular:
\begin{itemize}[nosep]
 \item Traversals perform eta-expansion `on-the-fly' at each point where the traditional STLC traversal would get stuck; \ie, when there is insufficiently many operands in an application to continue traversing the term.
 ...
 \item The rules traversing lambda and variable nodes handles the fictitious nodes created by the ad-hoc eta-expansions.
...
The absence of an $\eta$-long transform of the term is a notable difference with the traversals for recursion schemes~\cite{Ong2006} or the simply-typed lambda calculus~\cite{BlumPhd}. This simplifies the presentation as the traversed tree is an abstract syntax representation directly obtained from the the original, unmodified, lambda term. Similarly, the evaluation procedure from Algorithm~\ref{algo:ulc_normalization_by_traversals} produces the traditional beta-normal form rather than its beta-\emph{eta-long} normal form.
...
Finally, in the untyped case we prove soundness through a connection with the notion of head-linear reduction rather than appealing to Game Semantics.
...
\begin{todobox}
    ...
    \begin{description}[nosep]
      ...
      \item[No $\eta$-long form] The original STLC traversals were defined for terms in eta-long form only. This guarantees that the operand of an application always exists. In the untyped setting, eta-long expansion would be an infinite process, so instead imaginary traversals must work on the unmodified tree representation of the term.

      \item[On-the-fly $\eta$-expansion]
      Eta-expansion occurs `on the fly` in variable rules \rulenamet{Var} and \rulenamet{IVar} when the arity of a variable is too low to statically determine the operand of an application. When the variable arity is high enough, the definition of the rule coincides with STLC; in which case the tree statically dictates the node to traverse next.

      \item[Relabelling in traversal core] In the typed setting, the core is  obtained by just filtering nodes with respect to the tree root. In the untyped settings, lambda nodes need to be relabelled after filtering.

      \item[Traversing ghost lambdas]   Rule \rulenamet{Lam} traverses a lambda node by visiting its unique child node in the tree. No such child node exists for a ghost lambda, instead rule \rulenamet{Lam^\ghostlmd} traverses a ghost lambda by visiting the imaginary ghost variable that would be created if we were to eta-expand the sub-term under that lambda.

       The ghost placeholder $\ghostvar$ represents an occurrence of the $j$th variable bound by lambda node $\alpha$ if the sub-term at node $\alpha$ were eta-expanded $i-|n|$ times: hence $j = |\alpha| + i - |n|$.
...
Let $n\in \Nodes$ be the root node of some subterm $N$ of $M$. We write ${\sf ETA}(M, n)$ for the term obtained by substituting
in-place the subterm $N$ of $M$ with $\lambda\theta. N \theta$ for some fresh variable $\theta$.
...
Calculating the arity threshold thus boils down to alternatively adding and subtracting the node arities starting from the last variable occurrence in the traversal strand and reading backwards until reaching an external variable. The maximal value observed throughout that summation is the arity threshold.
\end{remark}
...
This weaving property suggests that instantiations of the external variable rule that exceeds the arity threshold are not relevant for normalization since they essentially expand external ghost variable nodes into deeper external ghost variable nodes. This observation gives rise to a stricter version of traversals:
...
%\begin{itemize}[nosep]
%\item
We start at the root and start traversing the tree using deterministic rules until hitting the first non-deterministic choice. This yields traversal
...
The normal form must therefore have the shape $\lambda y s \ldots \cdot y N_1 \ldots \ldots N_q$ for some fresh variable $y$ and $s$ and $q\geq0$.

%\item
To determine the $N_k$ arguments from the normal form, we enumerate all the possible instantiation of rule $\rulename{IVar}$ for all non-deterministic choices of value $k\geq 1$, and then recusrively extend each traversal until the next non-deterministic choice.
...
That upper-bound for $k$ is precisely given by the \emph{arity threshold} of the traversal $t_\epsilon$ from Definition~\ref{dfn:arity-threshold}:
    \begin{align*}
     \arth(t_\epsilon)
     &= \max \{ |s^2| , %\\
                |s^1| + (|s^2| - |\lambda|) , %\\
    ...
    & = \max \{   0 , 1 + (0 - 0) ,  2 + (1 - 2) + (0 - 0)
            ...
            \} = 1
     \end{align*}
     Thus, we do not have to consider higher values of $k$ at $t_\epsilon$, and  we have $\betanf{M} = \lambda y s \ldots \cdot s (y R_1 \ldots R_{q_2})$.
...
%\item
And with $k_2 = 2$, \rulenamet{IVar} extends $t_1$ to:
...
%\end{itemize}
Assembling the two maximal tree paths together yields $M$'s beta normal form $\lambda y s z \cdot s\ (y\ s\ z)$.
\end{example}
