...
\subsection{Correspondence with Game semantics for STLC}

This section recalls previous known results in the more restricted context of the simply-typed lambda calculus.

In \cite{BlumPhd} we formalized the correspondence between the theory of traversals and Game Semantics. Given a simply-typed term-in-context $\Gamma \entail M :T$, with typed-context $\Gamma$ (a set of typed variables), and simple type $T$, its traversals $\travset(\Gamma \entail M :T)$ is a set of sequences of nodes from some tree representation of $M$ where each node occurrence in a given sequence can have an associated pointer to some previous occurrence in the sequence. It is defined by induction over the traversal rules of \cite{Ong2006} augmented with a new rule to represent free variables. We then showed that there is a bijection between the set of traversals of $M$ and the revealed interaction game denotation of $M$, further there is a traversal transformation (the `core projection') that yields a bijection with the innocent game denotation of $M$. Formally:
...
\begin{eqnarray*}
 \travset(\Gamma \entail M : T)^\star & \cong & \revsem{\Gamma \entail M :T} \\
 \travset(\Gamma \entail M : T)^{\filter \theroot} & \cong & \sem{\Gamma \entail M :T} \enspace .
\end{eqnarray*}
...
$\travset(M)^\star$ denotes the set of traversals where application nodes are removed, and $\travset(M)^{\filter \theroot}$ denotes the set of \emph{core projections} of $M$.
\end{theorem}
The (fairly technical) proof relies on several key concepts \cite{BlumPhd}:
\begin{enumerate}
...
\paragraph{Normalizing terms with traversals}

The \defname{projection with respect to the root} of a traversal $t$, also called the \defname{core of the traversal}, consists of the subsequence of nodes that are hereditarily justified by the initial occurrence of the root. By the correspondence theorem the traversal cores are precisely plays from the game denotation of the term. In the traversal world, the game-semantic concept of ``Proponent view'', or just P-view, correspond to just paths in the tree representation of the term (\cite{BlumPhd}, Proposition 4.29). Because P-view are effectively computable from the traversal, this yields a method to calculate the beta-normal form of any STLC term $M$:

\begin{algorithm}
\caption{Normalization by traversals}
\label{algo:stlc_normalization_by_traversals}
\begin{enumerate}
  \item Enumerate maximal traversals of $M$ (The set of traversals may be infinite but in practice only a finite number of traversals need to be enumerated to normalize the term.);
...
For the sake of computing the beta-normal form of a term, however, it's only necessary to traverse finitely many finite traversals (provided that the normal form exists). Effectively, this means limiting the rule \rulenamet{IVar} to traverse only nodes leading to paths in the computation tree that are yet unexplored. In practice, it means that when choosing the next lambda node to visit, one can restrict the next move to child lambda nodes of the latest nodes in the traversal. This intuition is formalized by the notion of \emph{branching traversal} in Definition \ref{dfn:branching_traversals} and Proposition \ref{prop:branching_traversal_normalizing}.
\end{remark}
...
We extend the definition of nodes from STLC by introducing two infinite classes of ``imaginary'' nodes called \defname{ghost nodes}:
...
There are three important differences between the rules of Table \ref{tab:trav_rules} and the traversal rules of the simply-typed lambda calculus introduced in \cite{BlumPhd}:
\begin{enumerate}
...
\subsubsection{Ghost nodes intuition}
...
This equality proves soundness of the normalization procedure of Algorithm \ref{algo:stlc_normalization_by_traversals}.

Observe that the set of traversals can be infinite because the game denotation of a term must account for all possible moves of the Opponent for all possible denotation of the context term (take, for instance, context terms where some parameter is repeatedly called exactly $k$ times where $k$ is unbound). Fortunately, such traversals are redundant when it comes to normalizing the term: one can limit the set of traversals to the subset of \emph{branch traversals} where such behaviour is not allowed for the Opponent while still preserving the above equality.
This is formalized in Proposition \ref{prop:branching_traversal_normalizing}.
\\

In this section we aim to establish a similar normalization procedure for the untyped lambda calculus by proving the counterpart of Equation \ref{eqn:pviewcore_paths} for
the traversal definition of Table \ref{tab:trav_rules} and for untyped lambda-terms with a normal form.

\subsubsection{The View-Path correspondence for ULC}
...
\end{align*}
where for $1\leq q< k-1$ we define $b_q = \sum_{j=1..q} (|n_j| - |\alpha_j|) + |\alpha_q|$.

The $b_q$'s verify the following induction:
 \begin{align*}
 b_1 &= |n_1| \\
 b_{q+1} &= b_q + |n_{q+1}| - |\alpha_q|
 \end{align*}
...
The notion of redex from the standard lambda calculus is generalized by the notion of prime redex:

In order to define head-linear reduction, one needs to consider specific \emph{occurrences} of variables and sub-terms in a given term.

We will denote any redex $(\lambda x. M) A$ occurring in a larger term, by the pair $(\lambda x, A)$

where $\lambda x$ and $A$ denote respectively the specific \emph{occurrences} of $\lambda x$ and $A$ in the term forming the redex.

We say that a variable occurrence is \defname{involved} in a redex $(\lambda x. M) A$ if it is a free occurrence of $x$ in $M$. A variable occurrence can therefore be involved in at most one redex. We define the \defname{linear substitution} of $x$ for $A$, written $M\{x\mapsto A\}$ as the term obtained by performing capture-avoiding substitution of that single occurrence of $x$ by $A$. (Compare this to the standard substitution that is performed on \emph{every} occurrence of $x$ in $M$.) When performing such substitution we say that we \defname{linearly fire} the redex $(\lambda x, A)$ for that occurrence of $x$.

We define the \defname{head variable occurrence} of a term as the left-most variable occurring in the term (\ie, the first variable found by depth-first traversal of the term tree.) If the head variable occurrence of a term is involved in a redex we call that redex the \defname{head-linear redex}, and we say that we linearly fire the head-linear redex if we perform linear substitution for the head variable occurrence. A term that does not have a head-linear redex is said to be in \defname{quasi head-normal form}.
...
\todo{define prime redex}

We have the following result:
\begin{theorem}[Head-linear reduction and head normal form\cite{danos-head}]
...
\subsubsection{Linear reduction strategy}

Danos-Regnier's head-linear reduction is a linear version of the head reduction. It is not complete for normalization: it only leads to the quasi head-normal form, not the normal form. We will now define the linear counterpart of the normal-order reduction strategy.

Informally, we define the \defname{linear reduction strategy} as the strategy that performs head-linear reduction if possible, and otherwise, if the term is in quasi-head normal form, continuously (and recursively) applies the head-linear reduction on each argument of the head variable occurrence.
Formally:
\begin{definition}
Given a term $M$, we define its \defname{head active variable occurrence}, denoted $haoc(M)$ by induction on $M$:
\begin{align*}
haoc(x) &= x \\
haoc(U V) &=
\end{align*}
 ...
 The \defname{linear reduction}, written $\rightarrow_l$, is defined recursively as follows:
$M \rightarrow_l M'$ if either (i)  $M \rightarrow_{hl} M'$
or (ii) $M$ is in quasi-head normal form with  = \lambda x_1 \ldots x_n .

\end{definition}
linearly fires the left-most variable occurrence that is involved in some redex. (Compare this with the head-linear reduction that first locates the left-most variable occurrence and, if it is involved in a redex, linearly fires it.)


\begin{example}
Take $M = (\lambda x. x x N) z$. We have $M \rightarrow_{hl} (\lambda x. z x N) z$ which is in quasi head normal form because the head variable $w$ is not involved in any redex.
And since the left-most occurrence $x$ is involved in the redex $(\lambda x, z)$ we have $(\lambda x. z x N) z \rightarrow_l (\lambda x. z z N) z$.
\end{example}




By Theorem \ref{thm:danosreigner_headlinred}, the head-linear reduction strategy can be used to compute the head normal form of a term (by first calculating its quasi head normal form). We define

So intuitively, one can get to the normal form by repeatedly applying the strategy for each operand of the head variable in the head normal form. This gives the notion of \defname{linear reduction}

\begin{definition}[Traversal reduction]
We define the traversal reduction, written $\rightarrow_t$, as follows:
$T \rightarrow_t T'$ if and only if:
\begin{enumerate}
\item $T \rightarrow_{hl} T'$;
\item or $T$ is in quasi-head normal form and $T \rightarrow_h T'$;
\item or $T$ is in head normal form $T = \lambda x_1 \ldots \lambda x_n . z A_1 \ldots A_m$ for some lambda terms $A_1, \ldots A_m$ where  $n\geq 0, m >1$; and for some $1\leq j\leq m$ we have $T' = \lambda x_1 \ldots \lambda x_n . z A_1 \ldots A'_j\ldots A_m$  and  $A_j \rightarrow_t A_j'$.
\end{enumerate}
\end{definition}
