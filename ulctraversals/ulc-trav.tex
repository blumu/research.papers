...
if either $t \rightarrow_M t'$ or $t \rightarrow_N t'$ holds, for $t, t' \in X$, and \emph{Binary relation $R_\phi\subseteq X \times X$} defined for $t, u \in X$ by $ t\,R_\phi\,u$ just if $t\in\travsetnorm(M)$, $u\in\travsetnorm(N)$, $t = \phi(u)$ and $\core{t} \almostequal \core{u}$.
...
\begin{todobox}
    update proof to to show equality not just a bijection

\end{todobox}
$\travsetnorm(N)$ is by definition the transitive closure by $\rightarrow_N$ of the singleton traversal, so for any $u$ of $\travsetscn(N) \subset \travsetnorm(N)$, repeatedly applying (B2) for each step of its $\rightarrow_N$ derivation shows that $t = \phi(u) \in \travsetnorm(M)$.
But since $\phi(u) \almostequal u$ and $u$ is strand-complete we have $ \phi(u) = u$. Hence $u$ ...traversals, $\phi$ coincide with this identity
%
If $u$ is strand-complete then it is finite so $\coresymbol(t)$ is well-defined and commutativity of $\coresymbol$ and $\phi$ gives $\coresymbol(t)=\phi(\coresymbol(u))$ which simultaneously shows that $t$ is strand complete and that $\coresymbol(t)$ is in $\coresymbol(\travsetscn(M))$.
%
Similarly, it is \emph{surjective} by (B1) because $\travsetnorm(M)$ is the transitive closure by $\rightarrow_M$ of the singleton traversal. Finally it is \emph{injective} by assumption. Hence $\phi$ defines a bijection.

    % $\phi$ as a function
%  $\coresymbol(\travsetscn(N))\longrightarrow\coresymbol(\travsetscn(M))$
% is \emph{well-defined}: $\travsetnorm(N)$ is by definition the transitive closure by $\rightarrow_N$ of the singleton traversal, so for any $u$ of $\travsetnorm(N)$, repeatedly applying (B2) for each step of its $\rightarrow_N$ derivation shows that $t = \phi(u) \in \travsetnorm(M)$.
% %
% If $u$ is strand-complete then it is finite so $\coresymbol(t)$ is well-defined and commutativity of $\coresymbol$ and $\phi$ gives $\coresymbol(t)=\phi(\coresymbol(u))$ which simultaneously shows that $t$ is strand complete and that $\coresymbol(t)$ is in $\coresymbol(\travsetscn(M))$.
% %
% Similarly, it is \emph{surjective} by (B1) because $\travsetnorm(M)$ is the transitive closure by $\rightarrow_M$ of the singleton traversal. Finally it is \emph{injective} by assumption. Hence $\phi$ defines a bijection.
...
shows, that since \rulenamet{Var} is never used we necessarily have
