...
\title{Traversal-based Evaluation of Untyped Lambda Terms (preprint)}
...
We write $\justseqset(M)$ to denote the set of justified sequences over $M$ based on the quadruplet encoding $O_4$. When typesetting a justified sequence, we represent its occurrences by their associated node labels. Further, for lambda nodes, the `pending lambdas' are represented in exponent:
