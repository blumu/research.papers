...
The traversal rules of STLC implicitly define the next subterm to eta-expand: among all the possible subterms that could be eta-expanded, only one can create the fictitious nodes required to unblock the traversal using the unmodified traversal rules of STLC. In the above example, the first appropriate eta-expansion is that of $\lambda f .f$; eta-expanding the term itself first would yield $\lambda z. (\lambda f .f)(\lambda x.x) z$, which an STLC traversal cannot traverse due to the fact that $f$ still does not have an immediate argument.
Actually eta-expanding the untyped term is not necessary to traverse the term; the expansion occurs fictitiously ``on the fly'' as the traversal unfolds. A traversal, however, bares traces of those successive eta-expansions; which makes it possible to reconstruct the eta-expanded term effectively being traversed.
