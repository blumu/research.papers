...
Let $M$ be a term in quasi-normal form. Let $t$ be a non-empty traversal of $M$.
...
\item Let $t_{max}$ be a maximal extension of $t$.

    \begin{enumerate}[label=(\alph*)]
    \item Suppose $t^\omega$ is an external lambda node $\lambda \overline{\eta}$. Then the weave starting at $t^\omega$ is of the form
    $$\underline{\lambda\overline{\eta}} \cdot u_1 \cdot \underline{x_1}$$ where
        $u_1$ is a (possibly empty) \emph{spinal descent} (internal nodes),
        and $x_1$ is a structural external variable nodes.
        % \item Or $u_1$ is non-empty and the weave is of the form
        % $$ \underline{\lambda\overline{\eta}} \cdot u_1 \cdot \underline{x_1} \cdot w \cdot \lambda\overline{\eta_2} \cdot u_2 \cdot \underline{x_2} $$
        % where
        % \begin{itemize}
        % \item $u_1$ is a \emph{spinal descent} (internal nodes),
        % \item $w$ is a \emph{pending parameter lookup} (ghost nodes),
        % \item $\lambda\overline{\eta_2}$ is an internal lambda node
        %     justified by some $@$ node occurring in $u$,
        % \item $u_2$ is a \emph{spinal descent} (internal nodes),
        % \item $x_1,x_2$ are structural external variable nodes.
        % \end{itemize}

    \item Suppose $t^\omega$ is an external structural variable node $x_1$.
    ...
    \item Otherwise the previous and next weaves are the form:
        $$ \underbrace{\underline{\lambda\overline{\eta}} \cdot u_1 \cdot \underline{x_1}}_{\parbox[c]{2cm}{previous weave}} \cdot
        \underbrace{\underline{\ghostlmd} \cdot  v \cdot \lambda\overline{\eta_2} \cdot u_2 \cdot \underline{x_2}}_{\mbox{next weave}} $$
        where
        \begin{itemize}
        \item $u_1$ is a non-empty \emph{spinal descent} (internal nodes),
        \item $v$ is a \emph{pending parameter lookup} (ghost nodes) of length shorted than $u_1$,
        \item $\lambda\overline{\eta_2}$ is an internal lambda node
...
\end{proposition}
\begin{proof}
We will show (A), (B) and (C) together.
For (A) and (B), since (C) shows that the traversal consists of a repeation of weaves of type (a) and (b), it suffices to show that (A) and (B) hold for every weave in the traversal.
...
Case (a): Suppose $\lambda\overline{\eta}$ is the first node in the traversal then it's the root of the tree and therefore by definition $lloc_M(t^\omega) = lloc_M(M)$ which is equal to $\bot$ since
 $M$ is in \emph{qnf}. Otherwise,
