...
\newcommand{\child}{{\small \sf Child}}
...
\infrule[$\rulefont{Lam}^@_\normalizing$]
     {t \cdot \alpha \in \travsetnorm \andalso \alpha\in\NodesLmd, \child(\alpha) \in \Nodes_@ }
     {t \cdot \alpha \cdot \child(\alpha) \in \travsetnorm \andalso \mbox{$\child(\alpha)$ has no pointer}}

\infrule[$\rulefont{Lam}^{\sf var}_\normalizing$]
     {t = t_1 \cdot \beta \cdot t_2 \cdot \alpha \in \travsetnorm
     \quad \alpha\in\NodesLmd, \child(\alpha) \in \NodesVar
     \quad \beta \enables_i \child(\alpha), i \geq 1
     \quad \beta \in\pview{t} }
     {\Pstr[0.5cm]{t_1 \cdot (beta)\beta \cdot t_2 \cdot \alpha \cdot (n-beta,30:i){\child(\alpha)
     } \in \travsetnorm}}
where $\child(\alpha)$ denotes the unique child node in the computation tree of lambda-node $\alpha$, $t_1, t_2$ denotes two subsequences of the traversal, and $\pview{t}$ denotes the P-view of the traversal (the sequence obtained by reading the sequence backwards, alternating between reading the preceding node versus following the justification pointer). (It can be shown a posteriori that a variable's binder necessarily occurs exactly once in the P-view.)
...
TODO: give intuition on why this notion of traversals probably coincide with the game model of the untyped lambda calculus by Andrew Ker.
