...
\label{examp:ghost_materialization}
    Let $M = \lambda x. (\lambda y z.z) u$. The structural paths of the tree of $M$ are $\Sigma = \{\epsilon, 0, 00,000, 01,010\}$. The justified sequence
    $\Pstr[10pt]{
...
Take the term $J = (\lambda u . u(x\,u)) (\lambda v . v\,y)$, whose tree is represented in Example~\ref{ex:missingoperand}. We have
...
\begin{example}
The eta-expansion of $(\lambda u . u(x\,u)) (\lambda v . v\,y)$, with tree shown in Example~\ref{ex:missingoperand}, with respect to traversal $t$ from Example~\ref{ex:ulctrav_sample} is
