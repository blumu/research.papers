...
\item Let $n$ be a node occurring in $s$ we write $s\filter n$ to denote the subsequence of $s$ obtained by keeping only nodes that are hereditarily justified by $n$ in $s$. We call it the \defname{projection of $s$} with respect to $n$;
 ...
 \item We define the \defname{core P-view} as $\pview{\core{s}}$, the P-view of the traversal core.
\end{itemize}
\end{definition}
...
\begin{equation}
\pview{\core{\travstlc(M)}} = \pathset(\betanf{M}) \label{eqn:pviewcore_paths}
\end{equation}
where $\core{\travstlc(M)}$ denotes the set of cores of traversals of $M$ and  $\pathset (M)$ denotes the set of maximal paths in the computation tree representation of $M$.
...
$$ \Union_{k\geq0} \pview{\core{\travset^k(M)}} = \pathset(\betanf{M}) . $$
...
For every traversal $t\in\travulc$ there exists an affine traversal $u \in \travsetaffine$ such that
