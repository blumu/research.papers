...
\ghostvar_{k_{r}}
\end{align*}
where for all $r\leq i \leq q$ the ghost nodes
$\ghostvar_{k_i}$ and $\ghostlmd_{k_i}$ points respectively to $\lambda\overline{\xi_i}$ and $@_i$ with link label $k_i\geq1$ uniquely determined from $k$ and the arity of the nodes in $u_1^+$ as follows:
%\begin{align*}
%k_q &= k + |\lambda\overline{\xi}_q| - |x| \\
%k_j &= k_q + \sum_{i=j..q-1} (|\lambda\overline{\xi}_{i}| - |@_{i-1}|) \quad \mbox{
%for $j$ ranging in $r..q-1$.}
%\end{align*}
...
Further, since all the nodes in $v$ are ghost nodes, by Property~\ref{prop:ghost_justifier_arity} their justification label is greater than the justifier's arity; the opposite holds for $\lambda\overline{y_2}$ which is not a ghost node. Therefore:
\begin{align}
k > |x| \\
k_i > |@_i| - |\lambda\overline{\xi_i}| & \quad\mbox{for all $r+1\leq i \leq q$ } \label{eqn:qnfdecomp_kj}\\
k_r \leq |@_r| %%- |\lambda\overline{\xi_r}|
\end{align}
\\

We show by induction on $q-r$, the distance between $\lambda\overline{\xi_r}$ and $\lambda\overline{\xi_q}$,
 that $lloc_M(\lambda\overline{\xi_r}) =\bot$ and that the lambda-list at $@_r$ is of size $\sum_{r\leq j\leq q} (|@_j| - |\lambda\overline{\xi_j}|)$.
 
 
\emph{Base case:} We have $q=r$ (in which case $v=\epsilon$) and therefore $\lambda\overline{\xi_q}$ and $\lambda\overline{y_2}$ share the same parent $@_q$, they are respectively its $0$th and $k_q$th child as represented in the tree below:
...
\emph{Induction case:} Suppose the I.H.~is true for all $r<j\leq q$.
\begin{tikzpicture}[baseline=(root.base),level distance=6ex,inner ysep=0.5mm,sibling distance=10mm]
\node (root)
{$\cdots$}
child {node{$\lambda\overline\eta$}
...
\end{tikzpicture}
 ...
 then because
of Eqn.~\ref{eqn:qnfdecomp_kj} we have $k_{r+1} > |@_{r+1}| - |\lambda\overline{\xi_{r+1}}|$
\todo{...}
\\

We can now show that $lloc_M(\lambda\overline{y_2})=\bot$.
The subterm at $@_r$ is of the form $$(\lambda\overline{\xi_r}. A_0) A_1 \ldots A_{k_{r}-1}\ \lambda\overline{y_2}\ldots A_{|@_r|}$$
...
Further the lambda-list at $\lambda\overline{\xi_r}$ has size
\begin{align}
  |\lambda_l(\lambda\overline{\xi_r} . A_0)| &= \sum_{r \leq j\leq q} (|@_j| - |\lambda\overline{\xi_j}|) \\
  &= k - |x| + |\lambda\overline{\xi_r}| - k_r \quad \mbox{(by Eq.\ref{eqn:ki_k})} \\
  &> |\lambda\overline{\xi_r}| - k_r  \\
  &>
\end{align}
Hence by Property \ref{prop:qnf_longapply}, we necessarily have $lloc_M(\lambda\overline{y_2}) = \bot$.
...
\end{property}
\begin{proof}
(i) If $M$ has a normal form then by Theorem \ref{thm:soundness_leftmostlinearred} its linear reduction sequence terminates, and by Prop.~\ref{prop:ulctrav_impl_linear_reduction}(i) the set of traversals of $M$ is isomorphic to the traversals of its quasi normal form. Hence if $M$ has an infinite traversal then so does its \emph{qnf}, which contradicts Proposition~\ref{prop:ulctrav_impl_linear_reductionprop:qnf_traversals_are_finite}.
