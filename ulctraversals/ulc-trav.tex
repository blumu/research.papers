...
Let $\rightarrow_{hl}$ denote the head-linear reduction of Danos-Regnier \cite{danos-head},
$\rightarrow_{h}$ denote the standard head reduction (the one that fire the head redex of a term)and, $\rightarrow^*_{hl}$, $\rightarrow^*_h$ denotes their respective transitive closures. We have the following result:

\todo{define quasi-head normal form and prime redex}
...
A head-linear reduction strategy therefore yields the head normal form of a term. To get to the normal form one can repeatedly apply the strategy for each operand of the head variable in the head normal form:
