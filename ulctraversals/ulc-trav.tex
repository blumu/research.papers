...
\newcommand{\core}[1]{\coresymbol(#1)} % core of a traverasal
...
Our ``on-the-fly'' eta-expansion relates to the $Fun : U \rightarrow (U \Rightarrow U)$ morphism of Ker's game category where $U$ denotes the maximal arena.
\end{proof}

\section{Normalizing with Traversals}

We introduce an equivalence relation on traversals:
\begin{definition}[Quotienting]
We define the \defname{core P-view} function $\Phi$ by
\begin{align*}
\Phi : \travulc & \Longrightarrow \justseqset \\
t & \longmapsto \pview{\core{t}}
\end{align*}

We defined $\sim$ as the equivalence relation over $\travulc$ induce by $\Phi$. That is, $t \sim u$ just if $\Phi(t) = \Phi(u)$. We write $\travulc/{\sim}$ for the set of equivalence classes of $\travulc$.
\end{definition}

\subsection{Normalizing STLC terms}

The Correspondence Theorem \ref{thm:ulc_corresp} for STLC gives the following path characterization:

\begin{theorem}[Normalized Paths Characterization for STLC \cite{BlumPhd}]
\label{thm:path_charact_stlc}
For every simply-typed term-in-context $\Gamma\vdash M :T$, let $\etabetalnf{M}$ denote the eta-long form of the beta-normal form of $M$. We have:
\begin{equation}
\Phi : \travstlc(\Gamma\vdash M :T)/{\sim}\ \cong\ \pathset(\etabetalnf{M}) \label{eqn:pviewcore_paths}
\end{equation}
\end{theorem}
\begin{proof}
A simply-typed term in beta-normal form is uniquely determined by the maximal paths of its tree representation. By Theorem \ref{thm:ulc_corresp}, the set of traversals of the beta-normal form of $M$ is precisely the set of cores of traversals of $M$. By the Path-P-view correspondence (Prop.~\ref{prop:pathview_ulc}) and Prop.~ \ref{prop:ulc_and_stlc_trav_coincide}, the set of tree paths of eta-long beta  normal form is given by the set of P-views of traversal cores of $M$. Hence the P-view of cores of traversals of $M$ yields all the paths in the tree of the eta-long beta-normal form of $M$.
\end{proof}

The above bijection suggests a normalization procedure for STLC (modulo eta-expansion): enumerate all traversals and take their core P-view; this gives the paths in the eta long beta-normal form, which yields the normal eta-long beta form itself. There is just one caveat: the set of STLC traversals can be infinite! This is because the rule that traverses free variables allows for arbitrarily long traversals.
Fortunately, one does not need to enumerate all traversals: it's sufficient to enumerate at least one traversal per $\sim$-equivalence class of $\travstlc$.

\paragraph{Quotienting traversals}
The Game Semantics Correspondence explains why there can be infinitely many traversals: The game denotation of a term must allow Opponent's moves accounting for all possible context terms (modelled by the Opponent). In particular, suppose that $M$ takes a second-order function as argument. Then one can define an infinite sequence of term contexts that would pass as argument to $M$ a function that calls its argument exactly $k$ times for $k>0$ (\eg, $\lambda g . g ( g(\ldots (g z)))$ with $k$ applications of $g$).
...
This shows soundness and termination of the following algorithm:
\begin{algorithm}
\caption{Normalization by traversals}
\label{algo:stlc_normalization_by_traversals}
\begin{enumerate}
  \item Enumerate maximal \emph{branching} traversals of $M$ from Def.~\ref{dfn:branching_traversals};
...
In this section we show how the normalization procedure of Algorithm \ref{algo:stlc_normalization_by_traversals} gives the $\beta$-normal of an untyped lambda terms if it exists (without eta-long expansion).

\begin{theorem}[Normalized Paths Characterization for ULC]
For any untyped term $M$ with a normal form $\betanf{M}$ we have:
$$\Phi : \travulc(M)/{\sim} \ \cong\ \pathset(\betanf{M})$$
\end{theorem}
\begin{remark}
...
Observe that the game semantics correspondence conjecture for ULC implies this theorem since the same game-semantic argument used for STLC could be reused. Instead, we choose to prove it using a term-rewriting argument based on the \emph{leftmost linear reduction} strategy (See section \ref{sec:correctness}).
...
\begin{proposition}[Context-affine traversals suffice (modulo $\sim$)]
...
For terms in eta-long normal form, the on-the-fly eta-expansion traversal rules are never used, therefore branching traversals are bounded in the sense that at a given point, only finitely many nodes can be visited. So for STLC, in particular, the set of branching traversals is finite. This shows that the enumeration of alrgorithm \ref{algo:stlc_normalization_by_traversals} terminates.

In the next section we will explore how to further restrict the non-determinism of rule \rulenamet{IVar} to ensure that the enumeration also terminates for untyped terms with a normal form.
...
\begin{proposition}[Normalizing traversals are sufficient modulo $\sim$]
...
\item $t_1, t_2$ denotes traversal sub-sequences;
