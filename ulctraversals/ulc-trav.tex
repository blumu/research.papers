...
% set of extended nodes of the tree (both strucutural and ghost)
...
\begin{description}
    ...
    \item[- Link distance] the third component gives the distance between the occurence and its justifier;
...
\label{examp:ghost_materialization}
    Let $M = \lambda x. (\lambda y z.z) u$. The set of structural paths of the computation tree of $M$ is given by $\Sigma = \{\epsilon, 0, 00,000, 01,010\}$ (path to ghost nodes are ommitted, they are infinitely many of them).
...
Let's consider the tree structure at the trivial redex on Figure~\ref{fig:reducing_trivialredex}. It is of the form $(\lambda \overline{x} . T) A_1 \ldots A_r$ for $r\geq1$ where $(x_1, A_1)$ forms the leftmost trivial redex.
    ...
    \begin{figure}[htbp]
        ...
        \title{Reducing a (leftmost innermost) trivial redex. Case $r>1$ (top) and $r=1$ (bottom).}
    \end{figure}
    ...
    \end{equation}
    Observe also that $\phi$ commutes with the rebinding operation: because in the case of $\lambda\overeline{y}$, $\phi$ appends variable names to the left whereas the rebinding operation appends variables names to the right.
...
\end{theorem}
\begin{proof}
(i) The proof can be adpated from Danos-Regnier's proof of the same result for head-linear reduction~\cite{danos-head}.
