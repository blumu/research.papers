...
This characterization suggests a normalization method for STLC term based on enumerating the entire set of traversals. There is just one caveat: the set of STLC traversals may be infinite! This is because the non-determinism of the free variable rule can give rise to arbitrarily long traversals. Observe, however, that the bijection in the above proposition holds for the \emph{quotiented} traversal set. Thus enumerating all traversals is not necessary: it suffices instead to enumerate a $\sim$-complete subset. Algorithm \ref{algo:stlc_normalization_by_traversals} describes such a normalization procedure.
