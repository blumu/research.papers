...
We show by induction on $q-r$, the distance in the path from  $\lambda\overline{\xi_r}$ to  $\lambda\overline{\xi_q}$,
 that $lloc_M(\lambda\overline{\xi_r}) =\bot$ and that the lambda-list at $@_r$ is of size $\sum_{r\leq j\leq q} (|\lambda\overline{\xi_j}| - |@_j|)$.

\emph{Base case:}
We have $q=r$ (and $v=\epsilon$). The nodes $\lambda\overline{\xi_q}$ and $\lambda\overline{y_2}$ therefore share the same parent $@_q$: they are respectively its $0$th and $k_q$th child as represented in the left tree in Figure \ref{fig:strand_decomposition_proof_induction_base}.
Because $x$ is an external variable we necessarily have $lloc_M(\lambda\overline{\xi_r}) = lloc_M(\lambda\overline{\xi_q}) = \bot$.
Further the subterm at $\lambda\overline{\xi_q}$ is head normal therefore its head lambda-list is precisely of length $|\lambda\overline{\xi_q}|$ (the number of variables in the abstraction), and consequently the head lambda-list at $@_r$ is given by $|\lambda\overline{\xi_q}|-|@_q|$.
...
\emph{Induction case:}
We can now show that $lloc_M(\lambda\overline{y_2})=\bot$.
The subterm at $@_r$ is of the form
$$(\lambda\overline{\xi_r}. A_0) A_1 \ldots A_{k_{r}-1}\ (\lambda\overline{y_2}. \ldots) \ldots A_{|@_r|}$$

By assumption we have that $lloc_M(\lambda\overline{\eta}) =\bot$ therefore since $@_r$ occurs in the spine of $M$, by definition of $lloc_M$ we necessarily have $lloc_M(@_r) =\bot$, (and thus also $lloc_M(\lambda\overline{\xi_r}) = \bot$).

Let's calculate the length of the lambda list at $\lambda\overline{\xi_r}$.

Further by the induction hypothesis the lambda-list at $\lambda\overline{\xi_r}$ has length $|\lambda\overline{\xi_r}| $
\begin{align}
    |\lambda_l(\lambda\overline{\xi_r} . A_0)| &=
    |\lambda\overline{\xi_r}| + |\lambda_l(A_0)| \\
&= |\lambda\overline{\xi_r}| + |\lambda_l(@_{r+1})| \quad\mbox{($A_0$ is the subterm rooted at $@_{r+1}$)}\\
&= |\lambda\overline{\xi_r}| + \sum_{r+1 \leq j\leq q} (|\lambda\overline{\xi_j}| - |@_j|) \quad\mbox{(By induction hypothesis)}\\
&= |@_r| + \sum_{r \leq j\leq q} (|\lambda\overline{\xi_j}| - |@_j|) \\
&= |x| - k + k_r \qquad \mbox{(by Eq.\ref{eqn:ki_k} with $i=r$)}
\end{align}
But because $\lambda\overline{y_2}$ is a structural node by Property~\ref{prop:ghost_justifier_arity} we have $k>|x|$ therefore:
$$ |\lambda_l(\lambda\overline{\xi_r} . A_0)| < k_r $$

Consequently, in the sub-term $(\lambda\overline{\xi_r} . A_0) A_1 \ldots A_{k_r-1}$, there are more operands than the length of the lambda list of the operator. Thus by definition of the lambda list, we have $\lambda_l((\lambda\overline{\xi_r} . A_0) A_1 \ldots A_{k_r-1}) = \epsilon$.
Hence we necessarily have $lloc(\lambda\overline{y_2}) = \bot$, otherwise by definition of $lloc$ we would have
