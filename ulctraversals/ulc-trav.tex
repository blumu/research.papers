...
\title{Paranormal Traversals for the Lambda Calculus :-) [Draft] \\
 {\small Traversing and normalizing untyped lambda terms \\
  via head-linear reduction and on-the-fly eta-expansion.}}

\begin{document}
\maketitle
\begin{abstract}
We introduce a method to normalize untyped lambda terms (when the normal-form exist) by combining existing term-tree traversing techniques, inspired from Game Semantics, with judicious use of the eta-conversion rule of the lambda calculus.

Surprisingly, compared to existing term traversing technique for the simply-typed lambda calculus (STLC) \cite{BlumPhd}, very little change is required: (i) eta-long transformation is not required anymore: the original term is preserved intact (ii) we relax some of the structural restrictions from the original traversal rules of STLC; (iii) this relaxation induces the notion of imaginary nodes called ``ghost nodes''. They represent nodes that would exist in the abstract syntax tree of the term if some of its subterms were to be eta-expanded.
With these two modifications, a traversal that would normally get `stuck' using the original definition of traversal, can now proceed further. The normalization procedure then naturally follows, like in STLC, from the Path-Correspondence invariant of traversals.
\end{abstract}
...
The notion was then extended to the more general setting of simply-typed languages and in particular the simply-typed lambda calculus \cite{BlumPhd} where a new rule \rulenamet{IVar} was added to account for the presence of free variables. The relationship with Game Semantics is well understood in the typed setting: the set of traversals is in bijection with (the plays of) the interaction game denotation of a term, further the \emph{core projection} operation yields a bijection between the standard game denotation and its standard innocent game denotation (Theorem \ref{thm:correspondence}). This correspondence yields a method to evaluate beta-redexes of a simply-typed lambda term without performing the traditional $\beta$-reduction \cite{danos-head,BlumPhd,BlumGalop2008, Blum-LocalBeta2008}.

Berezun and Jones recently discovered a similar term-traversing technique for the \emph{untyped} lambda calculus (ULC) \cite{JonesBerezunLLL}. Starting from a definition of semantics for the untyped lambda calculus they derived a normalization method that, very much like the traversal of \cite{Ong2006, BlumPhd}, proceeds by traversing some tree representation of the term.
The Berezun-Jones traversals distinctively rely on two justification pointers: each `token' of the traversal can have one \emph{binding pointer} as well as one \emph{control pointer}. They also involve the use of a boolean parameter (the `flag') associated with every token of the traversal.
This contrasts with the game-semantic traversals of STLC which only require a single pointer per node, but, on the other hand, require eta-long expansion of the term prior to traversing it.

In this paper we show how the game-semantic traversals of STLC \cite{BlumPhd} naturally extend to ULC, and thus yield another method to normalize untyped lambda terms. In the absence of types, eta-long expansion is simply not an option: it would yield an infinite term. We thus opt instead for a more reasonable option: \emph{on-demand eta-expansion}. The idea stems from the following observation: attempting to apply the original STLC traversal rules on an untyped lambda term can lead to situations where the traversal gets ``stuck''. This happens when a sub-term in operator position has an insufficient number of operands to be able to resolve a parameter. For example, STLC traversals would get stuck when trying to resolved the ``$y$ argument'' of $f$ in term $(\lambda f.f)(\lambda y.y)$.

Consequently, a notable difference with the traversals for recursion scheme \cite{Ong2006} or the simply-typed lambda calculus \cite{BlumPhd}, is the absence of $\eta$-long transform of the term. This simplifies the presentation and bring it closer to Berezun-Jones' traversal where the traversed tree is just a direct representation of the original, unmodified, lambda term \cite{JonesBerezunLLL}. In contrast to Berezun-Jones, however, our traversal necessitate a single justification pointer per node occurrence.

This adaptation of the game-semantic traversals to the untyped setting has several new ingredients that we will detail in the remaining of the paper:
\begin{compactitem}
...
The normalization algorithm presented in this paper was implemented in the HOG tool \cite{Blum-HogTool}. In the last section of this paper I provide some examples of terms normalized with the algorithm.
...
{\bf TODO:}
(ii) connection with previous work by Daniil Berezun and Neil Jones on traversals compilation for ULC \cite{JonesBerezunLLL}.


\subsubsection{Game model of the untyped lambda calculus}
{\bf TODO:}
...
\todo{no eta-long expansion: betanf, not the eta-beta nf}

\subsection{Game semantics correspondence}
...
In \cite{BlumPhd} we showed that for every simply-typed term-in-context $\Gamma \entail M :T$ where $\Gamma$ denotes the typed-context of $M$ and $T$ denotes its type,
...
\begin{theorem}[Game Semantics and STLC Traversals (Theorem 4.96 in \cite{BlumPhd}]
...
The \defname{projection with respect to the root} of a traversal $t$, noted $t\filter {\theroot}$, also called the \defname{core} of the traversal, consists of the subsequence of nodes that are hereditarily justified by the initial occurrence of the root. By the correspondence theorem the traversal cores are precisely plays from the game denotation of the term.
...
E.g. Take the term $\lambda f . f (\lambda x. x)$. We have that $\lambda f \cdot f \cdot (\lambda x \cdot  x)^k$ is a traversal for all $k\geq0$. Viewed from a game semantic perspective, those traversals accounts for all the possible denotations of the function parameter $f$: for each $k$, there exists a term that applies its argument $k$ times: $\lambda y . y (y ( \ldots y)) $.
