...
\newcommand{\travsetaffine}{{\travset^a}}
...
In addition to those nodes already used in STLC, we introduce two ``imaginary'' nodes called \defname{ghost nodes}:
one ``ghost variable node'', denoted $\ghostvar$, and one ``ghost lambda nodes'', denoted $\ghostlmd$.
Those nodes will be used as placeholders in justified sequences to represent eta-expanded lambda nodes and variable nodes respectively.

We will write $\NodesLmd^\ghostlmd$ as a shortand for $\NodesLmd + \{\ghostlmd\}$
and $\NodesVar^\ghostvar$ as a shortand for $\NodesVar + \{\ghostvar\}$.
We will refer to nodes $\Nodes$ of the computation tree as \defname{structural nodes}, and nodes in $\{\ghostlmd, \ghostvar\}$ as the ghost nodes.
...
occurrence $m \in \Nodes_{\sf var}$ in $\oview{t\cdot n}$,
we have $t \cdot n \cdot \alpha \in \travset$ where $\alpha$ points to $m$ with label $k$.

\item \rulenamet{InputVar^\eta} If $n$ is a ghost variable then
for every ghost variable $\ghostvar$ occurring in $\oview{t\cdot n}$ and every label $k\geq 1$
we have $t \cdot n \cdot \ghostlmd \in\travset$, where $\ghostlmd$ points to $\ghostvar$ with label $k$.
\end{itemize}
...
\item $\alpha$ is either a structural lambda node in $\Nodes_\lambda$ or a ghost lambda node $\{\ghostlmd\}$
...
In this section we show how the same method works for untyped lambda terms using the ULC traversals of Tabel \ref{tab:trav_rules}.
...
account for all possible moves from the opponent O. This is need to get the isomoprhism with the game denotation of a term. For instance this is necessary to model contexts terms like the Kierstead terms $λf.f(λx.f(λy.y))$ and $λf.f(λx.f(λy.x))$. So for any lambda term $M$, its game denotation must contains plays with two occurrences of the same O-move representing the two separate occurrences of $f$.

For the sake of computing the beta normal form of a term, however, we only need to consider sufficiently many plays to cover all the path of the beta-normal form of the term. It is therefore not required to account for multiple occurrences of the same O-move in a given play: we can restrict plays to those denoting the interaction of the term $M$ with affine contexts (in which bound variables are consumed at most once.)

\begin{definition}[Context-affine traversals]
For any traversal $t$ we will consider the following set of triplets representing all the justification pointers in the O-view of $t$:
$$ Just(t) = \left\{ (\alpha, m, k) \in \NodesLmd^\ghostlmd \times \NodesVar^\ghostvar \times \nat\
...
with $\travsetaffine$ substituted for $\travset$ in every rule, and with rules \rulenamet{InputVar} and \rulenamet{InputVar^\eta} restricted as follows:

Let $t \cdot n \in \travsetaffine$ be a context-affine traversal where $n \in \NodesVar^\ghostvar$ is hereditarily justified by the root.

\begin{itemize}[]
\item \rulenamet{InputVar} Case where $n$ is a structural variable node.

For every occurrence of a structural variable nodes $m$ in $\oview{t\cdot n}$
and every $k$th child $\alpha \in \Nodes_\lambda$ of $m$
such that $(\alpha,m,k) \not\in Just(t)$
we have $t \cdot n \cdot \alpha \in \travsetaffine$, where $\alpha$ points to $m$ with label $k$.

\item \rulenamet{InputVar^\eta} Case where $n$ is a ghost variable.

For every occurrence $m$ of a ghost variable in $\oview{t\cdot n}$
and every $k\geq 1$ such that there is no $(\ghostlmd, m,k) \not\in Just(t)$
we have $t \cdot n \cdot \ghostlmd \in\travsetaffine$, where $\ghostlmd$ points to $m$ with label $k$.
\end{itemize}
\end{definition}

\begin{proposition}[Affine traversals are sufficient for normalization]
For all traversal $t\in\travset$ there exists  an affine traversal $u \in \travsetaffine$ such that
...
Let $t$ be traversal on which rule $\rulename{InputVar^\eta}$ can be applied. Then $t$ ends with a variable node hereditarily justified by the root, call it $n_1$. Let's consider the nodes between $n_1$ and its justifier:

$$ t = \Pstr[0.2cm]{ \cdots (aq){\alpha_k}\ n_k\ \alpha_{k-1}\ n_{k-1}\ \cdots\ \alpha_2\ n_2\ \alpha_1\ (n1-aq,25){n_1} } $$

for some $k>0$ where $\alpha_j$ are lambda nodes in $\NodesLmd^\ghostlmd$,
and $n_j$ are variable node in $\NodesVar^\ghostvar$ for all $1\leq j\leq k$.

The \defname{arity threshold} of $t$ is defined as:
\begin{align*}
\textsf{arth}(t) &= \max_{q=1..k} \left( |n_q| + \sum_{j=1..q-1} (|n_j| - |\alpha_j|) \right)
\end{align*}
\end{definition}
...
\begin{property}
Let $t$ be a traversal ending with a ghost variable $t^\omega = \ghostvar$ hereditarily justified by the root.
...
Then wee apply rule \rulenamet{Var^\eta} to visit a ghost lambda node justified by $n_q$ with label $i_q$.

\item If $\alpha_{q-1}$ is hereditarily justified by the root.
\todo{part missing!}
\end{itemize}

Since $i_{q-1} > |n_q|$ we can then apply the rule \rulenamet{Lam^\ghostlmd} to visit ghost variable node $\ghostvar$ justified by $\alpha_q$ and labelled by $i_q = i_{q-1} + |\alpha_{q-1}| -|n_{q-1}|$.
