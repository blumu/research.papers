...
\ghostvar_{k_{r}}
\end{align*}
where, for all $r\leq i \leq q$, ghost nodes
...
\end{align}
We introduce the abbreviation $\gamma_i = \sum_{i\leq j\leq q} (|\lambda\overline{\xi_j}| - |@_j|)$ so that $\gamma_{i} = \gamma_{i+1} + \lambda\overline{\xi_j}| - |@_j|$ and:
$$ k_i = k - |x| + |\lambda\overline{\xi}_{i}| + \gamma_{i+1}
$$

The subterm at $@_r$, represented on Figure\ref{fig:strand_decomposition_proof_induction}, is of the form
$$(\lambda\overline{\xi_r}. A_0) A_1 \ldots A_{k_{r}-1}\ (\lambda\overline{y_2}. \ldots) \ldots A_{|@_r|}$$

We show by a finite induction on $i$ ranging from $q$ down to $r+1$, that (i) $lloc_M(\lambda\overline{\xi_i}) =\bot$; (ii) the lambda-list at $@_i$ is of length $\gamma_i$.
...
\begin{tikzpicture}[baseline=(root.base),level distance=6ex,inner ysep=0.5mm,sibling distance=10mm]
        ...
        child {node{$@_1$}
            child{node{$\ldots$}
                child{node{$@_{r}$}
                    ...
                    child{node{\ldots}}
                    child{node{$\lambda\overline{y_2}$}
                                 edge from parent node[right]{$k_r$}
                    }
                    child{node{\ldots}}
    ...
    \end{tikzpicture}
\caption{Inductive case $q>r$}
\label{fig:strand_decomposition_proof_induction}
\end{subfigure}
\caption{Sub-tree at $\lambda\overline\eta$ for the base and induction case}
\end{figure}
