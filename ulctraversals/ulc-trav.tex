...
\item Let $M$ be an untyped term and $t$ be a traversal of $M$. We define the \defname{eta-expansion of $M$ with respect to $t$}, written $M^t$, as a term obtained by eta-expanding its subterms according to the rules used to traverse $M$. It is defined by induction on the traversal rules: By convention we define $M^\epsilon = M$. Suppose $t = u \cdot n$ for some node $n$. Let $m = \jp(t)^\omega$ denote the justifier of $n$ if it exists and $i$ denote its link label.  Consider the last rule used to traverse $t$:
\begin{equation}
M^{u \cdot n} =
\begin{cases}
    \rulename{Root} &  M^{u} \\
...
Let $M$ be an untyped term and $t$ be a traversal of $M$ then
...
\begin{proposition}
If $t$ is a traversal of $M$ then $t$ is also a traversal of $M^t$ and $t$ does not contain any ghost node.
\end{proposition}
\begin{proof}
By induction on the traversal rules, observing that in the variable rules, the on-the-fly eta-expansion of Definition \ref{def:onthefly_etaexpansion} increments the arity of the justifier node $m$ therefore no ghost variable needs to be introduced while traversing $M^t$.
\end{proof}
...
In fact, the STLC traversals rules of \cite{BlumPhd} are just a subset of the imaginary traversal rules of Table \ref{tab:trav_rules}:
...
For any simply-typed term in context $\Gamma \vdash M : T$, let $\etalf{M}$ denote the \emph{eta-long normal form}:
\begin{itemize}
\item[(i)] Traversals in $\travulc(\etalf{M})$ (where $\etalf{M}$ is considered as an untyped term) do not contain any ghost node.
\item[(ii)] $\travstlc(\Gamma \vdash M : T) = \travulc(\etalf{M})$.
\end{itemize}
\end{proposition}
\begin{proof}
(i) Because the term is eta-long expanded, the condition $i>|m|$ in the rules \rulenamet{Var} and \rulenamet{IVar} never holds while traversing the term. (ii) holds because the remaining traversals rules coincide for STLC and ULC.
\end{proof}
...
\begin{eqnarray*}
 ...
 \travulc^\coresymbol(M) &\cong& \sem{M} \enspace .
\end{eqnarray*}
...
In this section we show how traversals can be used to normalize lambda terms.
We first consider the STLC case and derive an normalization procedure for it.
Soundness follows immediately from the Game Semantics correspondence of \cite{BlumPhd}. We then spend the rest of the section showing how to extend this normalization procedure to ULC to calculate $\beta$-normal forms of untyped lambda terms when they exist.

Let's first introduce an equivalence relation on traversals:
\begin{definition}[Quotienting]
We define the \defname{core P-view} function $\Phi$ as the P-view function $\pview{\_}$ composed with the core projection $\coresymbol$:
\begin{align*}
\Phi : \travulc & \longrightarrow \justseqset \\
t & \longmapsto \pview{\core{t}}
\end{align*}

We defined $\sim$ as the equivalence relation over $\travulc$ induced by $\Phi$. That is, $t \sim u$ just if $\pview{\core{t}} = \pview{\core{u}}$. We write $\travulc/{\sim}$ for the set of equivalence classes of $\travulc$.
...
Behind this definition is the idea that core P-views capture what it strictly required from traversals to reconstruct the normal form of a term (Proposition \ref{prop:path_charact_stlc} and Theorem \ref{thm:path_charact_ulc}). The quotient helps eliminate `redundant' traversals that have the same core P-view: it is sufficient to exhibit at least one traversal of each $\sim$-equivalence class to cover the entire set of core P-views. In the remaining of this section we explore $\sim$-complete traversals subsets that can be effectively computed.

\subsection{Normalizing STLC by traversals}

The following characterization is a direct consquence of the Game-Semantic Correspondence Theorem for STLC \cite{BlumPhd}:
...
\begin{equation*}
\Phi : \travulc(M)/{\sim}\ \cong\ \pathset(\etabetalnf{M})
\end{equation*}
\end{proposition}
\begin{proof}
 By Theorem \ref{thm:gamesem_correspondence_stlc}, the set of traversals of the beta-normal form of $M$ is precisely the set of cores of traversals of $M$. The Path-View correspondence (Prop.~\ref{prop:pathview_ulc}) and Prop.~\ref{prop:ulc_and_stlc_trav_coincide} then show that the P-views of traversal cores of $M$ give the tree paths of the eta-long beta normal form.
\end{proof}

This characterization suggests a normalization method for STLC term based on enumeration of all traversals, with one caveat: the set of STLC traversals can be infinite! This is because the rule used to traverse free variables provide so much non-determinism that it gives rise to arbitrarily long traversals. Observe, however, that the bijection in the above proposition holds for the \emph{quotiented} traversal set. Thus one does not actually need to enumerate all traversals: it's sufficient to enumerate a $\sim$-complete subset. Algorithm \ref{algo:stlc_normalization_by_traversals} describes such a normalization procedure.

\begin{algorithm}[ht]
\caption{Normalization by traversals for STLC}
\label{algo:stlc_normalization_by_traversals}
Let $\Gamma \vdash M : T$ be a simply-typed term.
\begin{enumerate}
  \item Calculate $\etalf{M}$, the eta-long expansion of $M$;
  \item Enumerate maximal \emph{branching} traversals of $M$ from Def.~\ref{dfn:branching_traversals} over the computation tree of $\etalf{M}$;
  ...
  \item Apply the bijection from the Correspondence Theorem to get the corresponding path in the tree representation of the eta-long $\beta$-nf of $M$;
  \item Aggregate all the paths thus obtained to reconstruct the eta-long beta-normal form of $M$.
\end{enumerate}
\end{algorithm}

\paragraph{Soundness and termination}
Soundness of Algorithm \ref{algo:stlc_normalization_by_traversals} is shown by Proposition \ref{prop:path_charact_stlc} and from the fact
that the beta-normal form of a simply-typed term is uniquely determined by the paths of its tree representation. Termination is implied by Proposition \ref{prop:branching_traversal_normalizing} which shows that branching traversals are $\sim$-complete.
...
\subsection{Restricting traversal non-determinism}
...
The Game Semantics Correspondence explains why there can be infinitely many traversals: In the game denotation of a lambda term $M$, at every point in a play where it is Opponent's turn to play, all possible Opponent moves must be accounted for. More generally, the game denotation must allow Opponent's moves modeling the behaviour of \emph{all} contexts that can possibly interact with $M$. The traversals $t_k$ thus accounts for all the possible denotations of the function parameter $f$ of $M$: for each $k$, there exists a term that applies its argument $k$ times: $F_k = \lambda g . g (g ( \ldots (g z)))$ with $k$ applications of $g$, and there got to be plays in the game denotation of the term $M$ to accounts for those possible values of argument $f$.

From a traversal viewpoint, this fact is explained by the
way free variables are traversed. The rule \rulenamet{IVar} offers two non-deterministic choices:
\begin{itemize}
\item[(i)] the variable to pick in the O-view,
\item[(ii)] the child lambda node to pick amongst the children of variable picked in (i).
\end{itemize}
The first choice allows, among others, the repeated pattern described above where the same node $\lambda x$ in the O-view is picked again and again.

Fortunately, traversals accounting for all those contexts are redundant for normalization. Indeed, due to the absence of side-effects, calling the same argument multiple times always involves the same underlying computation in $M$. We formalize this intuition with the notion of \emph{branching traversals} (Definition \ref{dfn:branching_traversals}) which prevent such repetitive behaviour while still covering all $\sim$-equivalence classes (Proposition \ref{prop:branching_traversal_normalizing})
Branching traversals restrict the rule \rulenamet{IVar} to traverse only nodes leading to paths in the computation tree that are yet unexplored: when choosing the next lambda node to visit, it restricts choice (i) to the \emph{latest} variable node in the traversal, and choice (ii) to child lambda nodes of that variable node.

\paragraph{Bounding traversals non-determinism for ULC}
For simply-typed terms in eta-long form, there is a finite number of branching traversals which implies termination of the normalization procedure of algorithm \ref{algo:stlc_normalization_by_traversals}.
For untyped terms, however, branching traversals can still be infinite because \emph{on-the-fly} eta-expansion in the variable rules is unbounded ($i>0$). In the next section we will show how  this non-determinism can be bounded.
...
We now show that, for the purpose of normalizing a term, it is sufficient to consider traversals where we restrict the non-determinism in rule \rulenamet{IVar} such that the visited lambda node is necessarily justified be the immediately preceding input variable node. The intuition is that the non-determinism in those two rules represents branching points in a depth-first exploration of the beta-normal form of the term. Further, in the absence of side-effect, it is sufficient to explore each possible branch of the tree in separate traversals.
...
We define the set of \defname{branching traversals} $\travsetbr$ as the subset of $\travulc$ defined by induction with the rules of Table \ref{tab:trav_rules} (substituting $\travsetbr$ for $\travulc$ everywhere) with the input-variable rule replaced by:
...
For terms in eta-long normal form, the on-the-fly eta-expansion traversal rules are never used, therefore branching traversals are bounded in the sense that at a given point, only finitely many nodes can be visited. So for STLC, in particular, the set of branching traversals is finite, which implies that the enumeration of algorithm \ref{algo:stlc_normalization_by_traversals} terminates. In the next section we will explore how to further restrict the non-determinism of rule \rulenamet{IVar} to ensure that the enumeration also terminates for untyped terms with a normal form.
...
We can now generalize the normalization procedure of Algorithm~\ref{algo:stlc_normalization_by_traversals} to ULC.
Algorithm~\ref{algo:ulc_normalization_by_traversals} describe the normalization procedure that computes the $\beta$-normal of an untyped lambda term if it exists (without eta-long expansion).

\paragraph{Soundness} The following theorem shows soundess of the procedure:
\begin{theorem}[Normalized Paths Characterization for ULC]
\label{thm:path_charact_ulc}
For any untyped term $M$ having a normal form $\betanf{M}$ we have:
$$\Phi : \travsetnorm(M)/{\sim} \ \cong\ \pathset(\betanf{M})$$
\end{theorem}
Observe that Conjecture \ref{conj:ulc_corresp} implies this result by the same game-semantic argument used to show characterization for STLC. We will prefer instead to prove characterization using a term-rewriting argument based on the \emph{leftmost linear reduction} strategy. We sketch the argument below and refer the reader to section \ref{sec:correctness} for the proof.
...
Since $M$ is in beta-nf, the reduction sequence terminates by Theorem \ref{thm:soundness_leftmostlinearred} and yields the \emph{quasi normal form} (qnf).

The effect of leftmost linear reduction is essentially to inline the \emph{lloc} variable by its generalized argument (the argument of the generalized redex binding the \emph{lloc} variable).
...
By Proposition \ref{prop:qnf_nf}, the \emph{qnf} trivially reduces to its beta-nf. It's immediate to show that the invariant $\core{\travsetnorm(M)}$ is preseved by trivial beta-reduction.
\end{proof}

\paragraph{Termination} If $M$ has a normal form then its set of tree path is finite, therefore by the above theorem so is $\travsetnorm(M)/{\sim}$.
\todo{show that $\travsetnorm(M)$ contains only a finite number of traversals per $\sim$-equivalence class, therefore $\travsetnorm(M)$ is also finite and therefore the enumeration terminate.}
...
\begin{algorithm}[ht]
\caption{Normalization by traversals for ULC}
