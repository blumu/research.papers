...
It is \emph{well-defined}: $\travsetscn(N)$ is by definition the transitive closure of the initial strand by $\rightarrow_N$, so for any traversal $u$ of $\travsetscn(N)$, repeatedly applying (B2) for each step of its $\rightarrow_N$ derivation shows that $t = \phi(u) \in \travsetscn(M)$.
By commutativity of $\coresymbol$ and $\phi$ we have $\coresymbol(t)=\phi(\coresymbol(u)) \in \coresymbol(\travsetscn(M))$.
...
The linear substitution affects the term tree in three possible ways
 depending on the arity of the \emph{lloc} and the structure of the argument:
...
\end{description}
For each of case we show that there is an injection $\phi\colon\justseqset(N)\longrightarrow \justseqset(M)$ such that $M$ and $N$ are $\phi$-bisimilar.
...
%
    ...
    where $|x|>0$, and $A$ is an abstraction $\lambda\overline{y}. R$.
%
...
%
    ...
    The original variable name in $R$ are preserved, which does not cause capture of variables since the variable binding is uniquely determined by the enabling relation of the term tree.
...
Let $\travsetscn^p$ denote the prefix closure of $\travsetscn$.
We define $\phi$ as the extension of $\Phi$ to justified sequences of $\travsetscn^p(N)$ that
maps each occurrence to its image by $\Phi$;
preserves all justification pointers except for $@_x$ and $\lambda\overline{y}'$;
...
%
$\phi_1$ is the point-wise extension of $\Phi$ to $\travsetscn^p(N)$
...
term tree and, provided that the prefix at the $x$ is a valid traversal
of $M$, is obtained by repeatedly instantiating rules \rulenamet{Lmd} and \rulenamet{Var} until reaching node $\lambda\overline{y}$.
...
%%%%\begin{itemize}
%%%% \item
$\phi_1$, and $\phi$ are well-defined since by the P-view-path correspondence, $x$'s binder (resp.~$@_a$) necessarily occurs in the P-view at $x$ (resp.~$\lambda\overline{y}'$).

%%%% \item
The restriction of $\phi$ to $\coresymbol(\travsetscn(N))$
...
%%%% \item
The restriction of $\phi$ to $\coresymbol(\travsetscn(N))$, is injective.
...
%%%% \item
Since $\Phi$ preserves $\lambda$-node labels, it also preserves arities, hence $\coresymbol$ involves the same pending lambdas in $u$ or $\phi(u)$, hence $\coresymbol$ and $\phi$ commute.
%%%%\end{itemize}

To show that $M$ and $N$ are in $\phi$-bisimulation, we simultaneously show that the definition of $\phi$ is well-defined
(more specifically the argument lookup extension performed by $\phi_2$)
and that $(I)$, $(B1)$ and $(B2)$ hold by induction on the number of strands in the traversal traversals.

For case $(I)$, both traversal starts with the
rule $\rulename{Root}_\normalizing$, the proof argument is then the same as for $(B1)$ and $(B2)$, which shows that they extend into the $R_\phi$-related initial strands of $M$ and $N$.

For $(B1)$ and $(B2)$, given $u\in \travsetscn(N)$ and $t\in\travsetscn(M)$
such that $\phi$ is well defined for $u$ and $t = \phi (u)$
and $m, n$ denote respectively the last node in $t$ and $u$.
We prove both directions by induction on the length of the next strand
extending $t$ (resp.~$u$) and case analysis on the traversal rule used,
that every extension from $t$ to $t'$ (resp.~$u$ to $u'$) is a valid traversal.
%We assume that $t = t' t_1 \ldots t_
\begin{todobox}
