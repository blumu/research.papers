...
As mentioned in the introduction, the Correspondence Theorem \ref{thm:ulc_corresp} for STLC yields a method to normalize a simply-typed lambda term. This is formalized by the following characterization theorem.

\begin{theorem}[Normalized Paths Characterization for STLC]
\label{thm:path_charact_stlc}
For every simply-typed term-in-context $\Gamma\vdash M :T$, let $\betanf{M}$ denote the normal form of $M$. We have:
\begin{equation}
\travstlc(\Gamma\vdash M :T)/{\sim}\ \cong\ \pathset(\betanf{M}) \label{eqn:pviewcore_paths}
\end{equation}
where the isomorphism is given by the function $t\in\travstlc \mapsto \pview{\core{t}}$.
\end{theorem}
\begin{proof}
...
The above equality suggests a normalization procedure for STLC: enumerating all traversals and taking their core P-view yields all the paths in the beta-normal form which gives the normal form itself. There is just one caveat: the set of STLC traversals can be infinite! This is because the rule used to traverse free variables allows for arbitrarily long traversals.
...
The Game Semantics Correspondence explains the reason why there can be an infinite number of traversals. The game denotation of a term must allow Opponent's moves accounting for all possible context terms (modelled by the Opponent). In particular, suppose that $M$ is an order-$3$ term taking a second-order function $F$ as argument. Then one can define an infinite sequence of term contexts that would pass as argument to $M$ a function that calls its argument exactly $k$ times for $k>0$ (\eg, $\lambda g . g ( g(\ldots (g z)))$ with $k$ applications of $g$).

It turns out that traversals accounting for all those contexts are redundant in the sense that they all belong to the same $\sim$-equivalence class. The intuition being that, due to the absence of side-effects, calling the same argument multiple times will always involves the same underlying computation in $M$. Proposition \ref{prop:branching_traversal_normalizing} formalizes this intuition by showing that the subset of \emph{branching traversals}, which in particular prevents the repetitive behaviour described in the previous paragraph, covers all $\sim$-equivalence classes.

Because P-views are effectively computable, the Theorem \ref{thm:path_charact_stlc} yields a normalization procedure for STLC:
\begin{algorithm}
\caption{Normalization by traversals}
\label{algo:stlc_normalization_by_traversals}
\begin{enumerate}
  \item Enumerate maximal \emph{branching} traversals$^{(*)}$ of $M$ (Def \ref{dfn:branching_traversals});
...
In this section we aim to establish a similar normalization procedure untyped lambda terms with a normal form, by proving the counterpart of Equation \ref{eqn:pviewcore_paths} for the imaginary traversals of Table \ref{tab:trav_rules}.
...
\end{theorem}
\begin{remark}
This theorem has an infinitary counterpart for lambda terms with no normal form:
$$ \Union_{k\geq0} \pview{\core{\travulc^k(M)}} = \pathset(IT(M)) . $$
where $\travulc^k(M)$ denotes the set of traversals of length $k$ at most and $\pathset(IT(M))$ denotes the set of \emph{possibly infinite} maximal paths in $IT(M)$, the infinitary computation tree of the term obtained by reducing beta-redexes ad-infinitum (\ie, the B\"oem tree of $M$ represented as a computation tree). For a formal definition of infinitary computation trees see \cite{Blum17-HomogNotRestriction, Ong2006}.
\end{remark}

Like for STLC, this result would immediately follow from a proof of the game semantics correspondence conjecture. Instead, we will prove this result with a term-rewriting argument based on the \emph{leftmost linear reduction} strategy. See section \ref{sec:correctness} for the proof.
