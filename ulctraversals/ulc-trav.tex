...
This notion is well defined because a variable binder necessarily occurs in the path from it to the root. Further:
\begin{property}[Path characterization]
(i) An untyped term $M$ is uniquely determined by its set of justified paths $\pathset$.
(ii) Further $M$ is uniquely determined, up to $\alpha$-conversion, by the \emph{structure} (\ie, node types and pointers) of the sequences in $\pathset$.
\end{property}
\begin{proof}
(i) follows from the fact that computation trees are in one-to-one correspondence with the standard tree representation of a lambda term. (ii) is due to the fact that variables names are uniquely determined by the justification pointers and their associated label index.
\end{proof}
...
We define $\sim$ as the equivalence relation over $\travulc$ induced by $\pview{\core{\_}}$ up to relabelling. That is, $t \sim u$ just if $\pview{\core{t}} \equiv \pview{\core{u}}$ (same structure but not necessarily the same labels). We write $\travulc/{\sim}$ for the set of equivalence classes of $\travulc$.
...
\begin{equation*}
\pview{\core{\_}} : \travulc(\etalf{M})/{\sim}\ \stackrel{\cong}{\longrightarrow} \pathset(\etabetalnf{M})
\end{equation*}
\end{proposition}
\begin{proof}
...
For any untyped term $M$ having a normal form $N$ we have the following isomorphism
$$\pview{\core{\_}} : \travsetnorm(M)/{\sim} \ \stackrel{\cong}{\longrightarrow}\ \pathset(N)$$
\end{theorem}
\begin{proof}
...
Correctness of the normalization procedure is a consequence of the above Characterization theorem, the fact that branching traversals are $\sim$-complete (Prop.~\ref{prop:branching_traversal_simcomplete}) and the weaving Property \ref{prop:weaving}.

\todo{Detail soundness argument for new definition of $\coresymbol$!}
...
$$ \Union_{k\geq0} \pview{\core{\travulc^k(M)/{\sim})}} = \pathset(BT(M))$$
