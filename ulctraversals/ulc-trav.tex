...
and for any encoding $s = u \cdot (\overline{x}, k, l)\,  n_1 \ldots n_{j-1}$ for  $n_1, \ldots n_{j-1}\in \nameencoding$, $j\geq 1$, $k,l\geq 0$, and $u \in \nameencoding^*$ we have:
$$
\begin{array}{lll}
...
\begin{todobox}
    Rework in paragraph.
    Let $T\subset \nameencoding^* $ be a finite set of name-preserving encoding of justified sequences such that $T$ is isomorphic to justified paths $\pathset(M_2)$ of some other term $M_2$.
    Take $T = \{ t_0, \ldots t_n \}$

    $P^- = t_1 \vee t_n $
    $ P = \{ \readout[r] \| r \in P^- \}$

    We assume that we have a clash-avoiding renaming function
     $\alpha \colon \nameencoding^* \longrightarrow \nameencoding^*$
     which rename variable names in $\nameencoding^*$ so that a given variable name does not occur more than once.

     Let $X = \mathcal{P}(\nameencoding^*)$ the set of subsets of $\nameencoding^*$

     We define the unification function $\vee \colon X \longrightarrow X$
$\{ \} \vee \{ t \} = \{ t \}$
