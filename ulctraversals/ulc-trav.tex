...
The traversal rules are well defined, in particular for any sequence of nodes with pointers $t\in \travset$:
\begin{compactitem}
\item[(i)] $t$ is a valid justified sequence, with respect to enabling relation $\enables$ extended to ghost nodes, and $t$ verifies the alternation condition;
...
\item[(iii)] $\pview{t}$ is a (justified) path in the computation tree of $M$ extended with ghost nodes;
In particular, if $t$ does not contain ghost nodes then $\pview{t}$ is a structural path in the computation tree.
\end{compactitem}
\end{proposition}
\shortproof
Proved simultaneously by induction on the traversal rules.
Rule \rulenamet{Lam^{\sf var}}: By induction hypothesis (iii) the P-view is a path in the tree $M$ extended with ghost nodes. But the last node of $t$ is a structural node therefore, this path is necessarily a structural tree path that does not contain any ghost node. Consequently, if the last node is a bound variable node, its enabler is its binding lambda node which necessarily occurs exactly once in the P-view. If the variable is free then the enabler is the root itself: (i) holds. By definition, the justification pointer honors the enabling relation so (ii) holds. The node $n$ is by definition the unique child of $\alpha$ thus (iii) holds.
...
% For any $t\in \travset$, the sequences $t\filter\ExtNodes$ is a valid justified sequences verifying the alternation condition.
...
Let $M$ be an untyped term. If $n\in \Nodes$ is the root node of some subterm $N$ of $M$ then ${\sf ETA}(M, n)$ denote the term obtained by substituting in-place the subterm $N$ with $\lambda\theta. N \theta$ for some fresh variable $\theta$.
...
In the above definition, the subscript parameter $\overline{y}$ is called \defname{stack of pending lambdas}. As the sequence is read backwards, it accumulates variable names which then get prepended to the last external lambda nodes in the sequence. The \emph{core projection} is the sub-sequence obtained by removing internal nodes and prepending to each external lambda node's label, the stack of \emph{pending lambdas} between that point to the next external node.
...
where rules \rulenamet{Var} and \rulenamet{IVar} are restricted to their concrete sub-case--that is justification label $i$ must be strictly smaller than the arity $|m|$ of variable $m$.
\end{definition}
...
of Table~\ref{tab:trav_rules}
where \rulenamet{IVar} is restricted to its concrete subcase (enforcing the condition $i<|m|$).
\end{proposition}
\begin{proof}
...
%\item
How many more values of $k$ shall we look at? Intuitively, we should not eta-expand the node beyond the point where further traversals are guaranteed to visit ghost occurrences only. That upper-bound for $k$ is precisely given by the \emph{arity threshold} of $t_\epsilon$:
    \begin{align*}
     \arth(t_\epsilon)
     &= \max \{ |s^2| , %\\
                |s^1| + (|s^2| - |\lambda|) , %\\
