...
\subsection{Head-linear reduction}
We now introduce the head-linear reduction of Danos-Regnier using a slightly different presentation \cite{danos-head}.

In order to define head-linear reduction, one needs to consider specific \emph{occurrences} of variables and sub-terms in a given term. We will denote any redex $(\lambda x. M) A$ occurring in a larger term, by the pair $(\lambda x, A)$ where $\lambda x$ and $A$ denote respectively the specific \emph{occurrences} of $\lambda x$ and $A$ in the term forming the redex.
...
Danos-Regnier's head-linear reduction is a linear version of the head reduction.It is not complete for normalization: it only leads to the quasi-head normal form, not the normal form. We will now define the counterpart of the normal-order reduction strategy for the head-linear reduction.

\todo{We define the \defname{linear reduction}, written $\rightarrow_l$, as the reduction that linearly fires the left-most variable occurrence that is involved in some redex. (Compare this with the head-linear reduction that first locates the left-most variable occurrence and, if it is involved in a redex, linearly fires it.)
}
