...
\paragraph{Reading out a term from its justified paths}
Algorithm~\ref{algo:termtree_readout_from_justitied_paths} shows how to reconstruct the term tree from a set of justified sequences that is isomorphic to the justified paths of the term. In particular it shows how to assign consistent variable names to ghost variables. Special care is taken when assigning node labels to ensure that variable nodes with same binder and same link label have the same variable name.
...
\begin{itemize}[nosep]
    \item Directed paths $P \subseteq \nat^*$ are generated from the justified sequences in $\mathcal{P}$: the child index of a variable is $1$ and the child index of a lambda node is its justification label.
...
\begin{algorithm}[!ht]
\begin{algorithmic}
\caption{Reconstructing a term tree from a set of justified paths.}
\label{algo:termtree_readout_from_justitied_paths}
\REQUIRE{A set $\mathcal{P}$ of justified sequences verifying $\mathcal{P} \equiv \pathset(M)$ for some term $M$}
\ENSURE{Computation tree of $M$.}
...
\item Let $M$ be an untyped term and $t$ be a \emph{finite} traversal of $M$. The \defname{eta-expansion of $M$ with respect to $t$}, written $M^t$, is a term obtained by eta-expanding some of the subterms of $M$ according to the rules used to produce traversal $t$. We define $M^t$ inductively on the last rule used to traverse $t$. By convention we set $M^\epsilon= M$. Let $t = u \cdot n$ for some node $n$, $m$ denote the justifier of $n$ if it exists, and $i$ denote its link label, then $M^{u \cdot n}$ is
defined by case analysis on the last rule used:
...
\begin{array}{rlcrl}
    ...
    \rulename{Lam^\ghostlmd} & M^{u}  &       &                 & ETA(M^{u}, m) \qquad \hfill \mbox{if $i > |m|$.}
\end{array}
\end{itemize}
\end{definition}
...
\end{proposition}
\begin{proof}
(i) Because the term is eta-long expanded, the condition $i>|m|$ in the rules \rulenamet{Var} and \rulenamet{IVar} never holds while traversing the term.  This is shown by induction on the traversal rules, observing that the arity of an $@$ node is necessarily equal to the arity of its $0$th child lambda node and that the arity of a variable node with binding index $i$ is necessarily equal to the arity of the $i$th child lambda node of its binder's parent.
...
Let $M$ be an untyped term in eta-long form (\ie, with respect to some simple type that it inhabits). Then
\begin{enumerate*}
...
\ENSURE{Computation tree of the beta-normal form of $M$}
\begin{enumerate}[nosep]
