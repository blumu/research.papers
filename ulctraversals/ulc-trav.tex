...
When decoding a justified sequence, the variables from the `pending lambda' get appended to the list of variables bound by a lambda node.
Although it is empty in the initial encoding of a justified sequence, we use it later to define transformations that alter lambda node labels. This facilitates interpretation of a justified sequence over two distinct terms (\eg, before and after firing a redex).
...
$\cdots @ \cdot x \cdot \lambda\overline{x}^{[y_1 \ldots y_n]}$
...
Two justified sequences are considered \defname{equal} if, after appending
the respective `pending lambdas' to the variables bound by each lambda node occurrence, they have the same structure and same variable names. Formally, for two terms $M$ and $N$ we define the relation
...
The reflexive closure of $Eq_{M,N}^*$ defines an equivalence relation on
 the disjoint union $O_4(M) + O_4(N)$, and by extension $Eq_{M,N}^*$ defines an equivalence relation on $\justseqset(M) + \justseqset(N)$ where two sequences are in relation if their element-wise occurrences are.
...
We call a justified sequence \defname{canonical} just if the \emph{pending lambda} components are all empty. Observe that if we fix the term, a canonical justified sequence encoding uniquely determines its structure: discarding the label information yields the structure; reciprocally the labels can be uniquely reconstructed from the structure of the sequence and the term's enabling relation.

\begin{example}
    Alpha-conversion does not impact the structure of a sequence, by definition, so for instance we have $\Pstr[10pt]{ (l){\lambda x y} \cdot (n-l,25){x} } \equiv \Pstr[10pt]{ (l){\lambda z y} \cdot (n-l,25){z}}$. The number of variable bindings in a lambda node may also change without impacting its structure, as long as justification link labels are unaffected, so for instance we have $\Pstr[10pt]{ (l){\lambda x y z w t} \cdot (n-l,25){x} } \equiv \Pstr[10pt]{ (l){\lambda x} \cdot (n-l,25){x}}$ since both sequences have structure $(\lambda,0)\cdot({\sf Var},1)$.
\end{example}

We will use non-canonical justified sequences to analyze the effect of
$\beta$-reduction on a justified sequence, where the \emph{pending lambda} component helps define two useful operations:
\begin{enumerate}
