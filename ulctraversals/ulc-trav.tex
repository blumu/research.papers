...
for some function $\phi$ whose restriction to $\coresymbol(\travsetscn(N))$ is injective
...
It is \emph{well-defined}: $\travsetscn(N)$ is by definition the transitive closure of the initial strand by $\rightarrow_N$, so for any traversal $u$ of $\travsetscn(N)$, repeatedly applying (B2) for each step of its $\rightarrow_N$ derivation shows that $t \equiv \phi(u) \in \travsetscn(M)$.
Since by assumption $\phi$ is structure-preserving on $\travsetscn(M)$, it must commute with $\coresymbol$ and we have $\coresymbol(t)\equiv\phi(\coresymbol(u)) \in \coresymbol(\travsetscn(M))$.
