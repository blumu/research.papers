...
\begin{itemize}
\item  Let $t\in\travulc(M)$ denote a traversal with no ghost occurrences. We define $\coresymbol (t)$ by the following inductive definition, where the equality are implicitly pointer-preserving (so that nodes in $t$ have same justifiers as in $\coresymbol(t)$).
\begin{align*}
...
A property of the linear head reduction is that the reduct contains the original term itself, modulo alpha conversion and relabelling of the \emph{lloc} node. This induces an implicit map $\phi$ from nodes of the reduct $N$ to those of $M$: it maps nodes in $N$ from unmodified subterms to their corresponding nodes in $M$, and nodes from the substituted term to the corresponding nodes under the argument $A$ of $M$. By abuse of notation we write $\phi$ to denote its element-wise structure-preserving extension over justified sequences of $N$. (We recall that two justified sequences $t_1, t_2$ over two different terms are said to have the same structure just if their constituting nodes are of the same type, have the same link labels, and same pointer lengths.)

For any term $M$ we define $\travset^\dagger(M)$ as the subset of traversals of $\travsetnorm(M)$ ending with either a structural node or an external ghost (\ie, either a non-ghost node or a ghost node hereditarily justified by the root). Clearly, any traversal in $\travset$ can be extended into a traversal in $\travset^\dagger$. Indeed, let $t\in\travset$, if $t$ is not in
$\travset^\dagger$ then $t$ ends with an internal ghost node in which case $t$ can be extended with rule \rulenamet{var} and \rulenamet{Lmd} until reaching an external node, thus producing a traversal $t'$ in $\travset^\dagger$.

Hence, since by definition $\coresymbol$ removes all internal nodes from a traversal, proving the isomorphism (i) amounts to proving the corresponding isomorphism for the restricted subset $\travset^\dagger$:
\begin{equation}
\coresymbol(\travset^\dagger(M)) \cong \coresymbol(\travset^\dagger(N))
\ .\label{eqn:travesetnorm_dagger_eq}
\end{equation}
We show that function $\phi$ yields such an isomorphism. By definition, $\phi$ is structure-preserving. To show that it defines a bijection from $\coresymbol(\travset^\dagger(N))$ to $\coresymbol(\travset^\dagger(M))$ we use a bisimulation argument. Let $X$ denote the disjoint union $\travset^\dagger(M) + \travset^\dagger(N)$ and consider the following binary relation and transition over it:

\emph{Binary relation} For any traversals $t$ and $u$ in $X$ we write $t~R~u$, just if $t\in\travset^\dagger(M)$, $u\in\travset^\dagger(N)$, and  $\phi(\coresymbol_\alpha(u)) = \coresymbol_\alpha(t)$ for any sequence of variable names $\alpha$.

\emph{Transitions} For any traversals $t, t' \in \travset^\dagger(M)$ (resp.~$\travset^\dagger(N)$) we write $t \rightarrow_M t'$ (resp.~$t \rightarrow_N t'$) just if $t'$ is a minimal extension of $t$: that is $t'$ extends $t$ using rules from Table~\ref{tab:normalizing_trav_rules} and all the occurrences following $t$ but the last node are either ghost nodes or internal nodes. For $t, t' \in X$ we write $t \rightarrow_X t'$ if either $t \rightarrow_M t'$ or $t \rightarrow_N t'$ holds.

We show that $R$ defines a bisimulation over $X$, that is
\begin{itemize}
    \item (i) for all $t_1, t_2 \in \travset^\dagger(M), u_1 \in \travset^\dagger(N)$ with $t_1~R~u_1$ and $t_1 \rightarrow_M t_2$ there is $u_2 \in \travset^\dagger(N)$ such that $u_1 \rightarrow_N u_2$ and $t_2~R~u_2$.
    \item (ii) for all $t_1 \in \travset^\dagger(M), u_1, u_2 \in \travset^\dagger(N)$ with $t_1~R~u_1$ and $u_1 \rightarrow_N t_2$ there is $t_2 \in \travset^\dagger(M)$ such that $t_1 \rightarrow_N t_2$ and $t_2~R~u_2$.
\end{itemize}
...
;
\end{tikzpicture}
\caption{Reduction $M\rightarrow_l N$: Linearly firing the generalized redex $(\lambda x, A)$ for occurrence $x$ in $M$ (left) yields $N$ (right) for fresh variables $\overline{y_2}$. Case where node $x$ has operands and $A$ is a non-dummy abstraction $\lambda\overline{y} . A'$.}
\label{fig:firing_genredex_effect_on_tree_with_operands_and_lambda_argument}
\end{figure}

We proceed by induction. The empty and singleton traversals are trivially bisimilar. Inductive case: Let $u, u' \in \travset^\dagger(N)$, $t\in\travset^\dagger(M)$ such that $t \sim_X u$ and $u \rightarrow_N u'$. The result is immediate if $t$ does not contain the occurrence $x$; and the opposite direction is immediate for any traversal $u$ that do not contain  $\phi^{-1}(x)$. The interesting case occurs when a traversal reaches the \emph{lloc} variable $x$ in $M$ (resp.~$\phi^{-1}(x)$ in $N$).
...
Reciprocally: suppose $u \in \travset^\dagger(N)$ ends with $@_x$ and $t \sim_p u$. Then $t$ must end with $x$. Suppose $t \rightarrow_M t'$ then $t'$ is necessarily of form $t \cdot v\cdot \lambda\overline{y}$ where $v$ consists solely of internal ghost nodes. We can thus just take $u' = u \cdot \lambda\overline{y_2}$.
%%%%%%%%%
