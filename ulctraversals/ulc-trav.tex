...
\item[Concrete case] If $i \leq |m|$ then $\beta$ is a structural lambda node in $\NodesLmd$;
\item[Eta-expanded case] If $i > |m|$ then $\beta$ is a ghost lambda node $\ghostlmd$.
\end{description}
\end{itemize}
...
\begin{description}[align=right,leftmargin=10em]
\item[Concrete case] If $i\leq|m|$ then $\alpha$ is a structural lambda node in $\Nodes_\lambda$;
\item[Eta-expanded case] If $i>|m|$ then $\alpha$ is a ghost lambda node $\ghostlmd$.
\end{description}
\end{itemize}
...
We now generalize a known result from the theory of traversals for higher-order grammars~\cite{OngLics2006} and simply-typed lambda terms~\cite[Proposition 4.29]{BlumPhd} to the untyped setting. The game-semantic concept of P-view from Definition~\ref{def:views} corresponds to the concept of `tree path' in the following sense:
...
Daniil Berezun and Neil Jones were first to introduce a notion of traversals for the untyped lambda calculus~\cite{JonesBerezunLLL-PEPM17}. Their presentation has notable differences with ours. Firstly, they require two types of pointers: binding pointers and control pointers, instead of a single justification pointer for imaginary traversals.
