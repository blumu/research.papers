...
for every other ghost variable $\ghostvar$ occurring in $\oview{t\cdot n}$ and every label $k\geq 1$
...
By the Game-Traversal Correspondence Theorem \cite{BlumPhd}, the set of traversals of the beta-normal form of $M$ is precisely the set of projections, with respecte to the root, of the traversals of $M$. Furthermore, by the Path-P-view correspondence, the set of
...
It may be helpful to think of ghost nodes as the counterpart of imaginary numbers sometimes used in mathematics
to prove trigonometry identities (e.g. using De Moivre's Theorem): they are introduced intermediately to perform some computation or calculation but do not appear in the final result. Just like complex number $i$ is defined from the impossibility of taking the square root of $-1$, ghost nodes are defined from the impossibility of performing a beta-reduction on a lambda term due to insufficient number of binding variable names.
...
Intutively, after a sufficiently large number of eta-expansion, we can be guaranteed to only traverse ghost nodes that never materialize back to structural nodes. This section formalizes this intuition: we defined the \defname{arity threshold} as the maximum number of time we want to eta-expand a give subterm with the rule $\rulename{InputVar^\eta}$.
...
$$ t = \Pstr[0.2cm]{ \cdots (aq){\alpha_k}\ n_k\ \alpha_{k-1}\ n_{k-1}\ \cdots\ \alpha_2\ n_2\ \alpha_1\ (n1-aq){n_1} } $$
