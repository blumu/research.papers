...
This induces a method to evaluate simply-typed lambda-terms~\cite{BlumPhd,BlumGalop2008,Blum-HogTool,Ong-NormByTrav2015}, which we state in Algorithm~\ref{algo:ulc_normalization_by_traversals}, that was previously implemented in~\cite{Blum-HogTool}.

This paper generalizes the evaluation procedure to the untyped lambda calculus,
based on an extended notion of `projection with respect to the root'
that involves relabelling of the lambda nodes after filtering. A notable improvement is that it produces the traditional beta-normal form rather than the beta-\emph{eta-long} normal form. Another difference is that we prove soundness of the procedure through a connection with the notion of head-linear reduction rather than appealing to Game Semantics.
...
Traversals are essentially a recursive implementation of the head linear reduction strategy on lambda terms. A traversal starts, in a call-by-name-like fashion, with a depth-first search for \emph{head occurrence} of the \emph{hoc redex} as defined by Danos-Regnier, and implements linear substitution by ``jumping'' to the node in the tree representing the sub-term to be substituted for that head occurrence. It then continues its descent to look for the next head occurrence, as if that first head occurrence had been replaced by the redex argument. A property of head-linear reduction is that all the terms appearing in the reduction sequence consist of sub-terms of the original term. This property means that there is no need for the traditional notions of environments or closures to reduce beta-redexes. In traversals, such mechanisms are replaced by a system of pointers referencing subterms of the original term.

This paper formalizes the notion of \emph{leftmost linear reduction} strategy that recursively applies head linear reduction; just like how the \emph{normal reduction} from the lambda calculus proceeds by recursively applying head reduction. We show how the \emph{imaginary traversals} effectively implement leftmost linear reduction, which proves soundness of the traversal normalization procedure.

%%%%%%%%%%%%%%%%%
An imaginary traversal implements what Danos \etal call ``the gymnastics to find the argument of a subterm''~\cite{danos-head} via
the mechanism of `weaving' (Proposition~\ref{prop:weaving}),
where it traverses a succession of ghost occurrences until the node representing the argument to be fetched materializes.
A weaving consists of repeated instantiations of the lambda and variable
traversal rules where the link label `$|\alpha|+i-|m|$' from the definition
...
% `pending argument lookup' (Definition~\ref{def:spinaldescent_pendingarglookup})
%%%%%%%%%
...
Previous work~\cite{BlumPhd} showed that for typed languages such as the simply-typed lambda calculus, PCF or Idealized Algol, the theory of traversals~\cite{OngLics2006} correspond to Game Semantics in a very concrete way: traversals are in one to one correspondence with the plays from the game denotation of a term where all internal moves are revealed (Theorem~\ref{thm:gamesem_correspondence_stlc}). We conjecture a similar correspondence for the untyped lambda calculus: that there is an isomorphism between the cores of imaginary traversals from Table~\ref{tab:trav_rules} and the innocent \emph{effectively almost everywhere copycat (EAC)} game denotation of untyped lambda terms~\cite{KerThesis}; as well as an isomorphism between the imaginary traversals and the EAC game denotation where internal moves are not hidden during strategy composition. We suspect that a connection exists between the ``on-the-fly'' eta-expansion of imaginary traversals and the morphism $Fun : U \rightarrow (U \Rightarrow U)$ of Ker's game category, where $U$ denotes the maximal game arena of the EAC strategies.
%%%%%%%%%%%%%%%%%

\begin{todobox}
Traversal theory for other reduction strategies? Call by value.
\end{todobox}

Danos \etal showed the connection between game models of simply-typed lambda terms and \emph{head-linear reduction} and proposed \emph{Pointer Abstract Machine (PAM)} as an abstract machine implementation of this reduction~\cite{danosherbelinregnier1996}.
