...
\newcommand{\travsetscn}{\travset_{\sf s.c.n.}} % strand-complete normalizing traversals
...
of the lambda rule in Table~\ref{tab:trav_rules} effectively calculates the quantity $r-a+l$ that appear in step 2.(b) of the PAM transition from~\cite{danos-head}.
...
%
The connection between the two presentations lies in their respective correspondence with head-linear reduction: we show that imaginary traversals recursively implement head-linear reduction. Berezun and Jones claim that a similar correspondence hold for their notion of traversal.
%
Their traversals operate on the traditional term tree where each node represents a token from the lambda calculus syntax,
whereas imaginary traversals operate on a compressed term tree that merges consecutive lambdas nodes and consecutive applications nodes into single bulk lambda node or bulk application node. Neither notion of traversals require any prior syntactic transformation of the input term, unlike the original traversals from the typed setting.
%
...
%
Their `read-out' procedure to reconstruct the normal form (shared with the author in private correspondence) involves recursively applying rewriting rules to the traversal. In contrast, our read-out algorithm reconstructs the normal form from the tree-paths induced by the maximal traversals.
...
\paragraph{GoI} The relationship of traversals with proof-nets and the Geometry of Interaction (GoI) can be seen through the lenses of head-linear reduction, which they implement, or their correspondence with game semantics. In particular, the linear substitution performed by traversals corresponds to firing of an exponential branch of a proof-net~\cite{MASCARI1994111}. We also point to the work of Danos and Regnier who established the connection with abstract machines and GoI~\cite{danosherbelinregnier1996}.

\paragraph{Optimality} Traversal-based evaluation is not L\'evy-optimal~\cite{optimal-reduction-levy}.
Traversals do exhibit some sharing through their use of pointers, which avoids copying or rewriting subterms when evaluating redexes.
But there is no attempt to exploit such sharing to avoid redundant work: in the term $(\lambda x. y x x) A$, each occurrence of $x$ involves re-traversing the argument $A$.
If $A$ contains a redex then it will get traversed twice, without any attempt to exploit the shared redex.
%
On the other hand, just like head-linear reduction, traversals are  ``hyperlazy'' in nature: they only evaluate (linear) redexes one variable occurrence at a time and only when the value of that occurrence is needed. So beside the possible redundancy described above, no useless work is performed, as opposed to the traditional
way of evaluating redexes which involves substituting all variable occurrences at once, regardless of whether some of those occurrences are dead-code.
%
Traversals may not be too far from optimality:
we could conceivably augment traversals with some memoization mechanism
to detect shared families of redexes, in L\'evy-sense, to give rise to
a Levy-optimal notion of traversals, similarly to what is suggested by Danos and Regnier in~\cite{DANOS199979} to turn \emph{Jumping Abstract Machines} into optimal machines.
...
We write $\Nodes(M)$ for the set of nodes of $\ctree(M)$, or just $\Nodes$ if the term is clear from context; $\NodesVar$ for the set of variable nodes; $\NodesLmd$ for lambda nodes, and $\NodesApp$ for  application nodes. For any lambda node $\alpha\in\NodesLmd$ we write $\child(\alpha)$ to denote its unique child node (either a variable node or $@$).
\end{definition}
...
extending the parent-child node relationship and the enabling relation
associated with the term tree: for every (ghost) variable or @-node $n$ with parent (ghost) lambda-node $\alpha$, and for all $k>|n|$,
...
and $\alpha \enables_{|\alpha|+ k} \ghostvar$; where by convention ghost nodes have arity $0$, and the arity of structural nodes remains unchanged (\ie, given by their arity in the structural tree $\ctree(M)$).
%
This extended enabling relation induces a binding relationship between ghost variables and their (possibly ghost) lambda-binder, which lets us forgo the notion of variables names in ghost nodes: we can thus
...
% \end{cases}
% \end{array}$$
The case $\lambda\overline{x}$ where $\overline{x}=\epsilon$
covers both the ghost lambda $\ghostlmd$ and dummy lambda $\lambda$.
...
The set $\travulc(M)$ of \defname{traversals} of an untyped lambda term $M$, abbreviated $\travulc$ when the term is clear from context, is the set of justified sequences of nodes over the extensional tree of $M$, defined inductively by the derivation rules of Table~\ref{tab:trav_rules}:
 $t \in \travulc(M)$ if and only if the judgement $\istraversal t$
 is derivable.
%
...
\begin{itemize}[leftmargin=3em]
    \item[\rulenamet{Root}]
    $\istraversal \alpha$ if $\alpha$ is the root $\lambda$-node of the term tree $\ctree(M)$.

    \item[\rulenamet{App}] If $\istraversal t \cdot @$ then
    $\istraversal \Pstr[0.4cm]{t \cdot (at) @  \, (a-at,40:0) \alpha}$ where $\alpha\in\NodesLmd$ is the $0$th child $\lambda$-node of $@$.

    \item[\rulenamet{Lam}] If $\istraversal t \cdot \alpha$ for $\alpha\in\ExtendedNodesLmd$ then $\istraversal t \cdot \alpha ~ n$ where $n$ is $\alpha$'s unique child and if $n$ is
    \begin{compactitem}
        \item[\rulenamet{Lam^@}] an $@$-node then it has no justifier;
        \item[\rulenamet{Lam^{\sf var}}] in $\NodesVar$ then it points to the only occurrence in $\pview{t\cdot \alpha}$ of a node $m$ verifying $m \enables_k n$ for some $k\geq 0$, and has link label $k$;

        \item[\rulenamet{Lam^\ghostvar}] a ghost variable $\ghostvar$
        (and thus $\alpha$ is $\ghostlmd$) then
        it points to the predecessor $\alpha'$ of $\alpha$'s justifier
        and has label $|\alpha'| + i - |m|$,
        that is
        $\istraversal \Pstr[0.5cm]{ u \cdot(alphaprime){\alpha'}
    (m){m} \cdot v \cdot  (gl-m,30:i){\ghostlmd}\, (al-alphaprime,40:{|\alpha'| + i - |m|}){\ghostvar}}$
    %
        where $t \cdot \alpha = \Pstr[0.5cm]{ u \cdot \alpha'
        (m){m} \cdot v \cdot (gl-m,40:i){\ghostlmd}}$,
        $\alpha'  \in \ExtendedNodesLmd, m \in\ExtendedNodesVar \union \NodesApp$
    \end{compactitem}
\end{itemize}
...
\item[\rulenamet{Var}] If $\istraversal \Pstr[0.5cm]{t\cdot(m){m}
 \, (alphaprime){\alpha'}
\cdot v \cdot (n-alphaprime,50:i){n}}$ for $m \in\ExtendedNodesVar \union \NodesApp$, $\alpha' \in \ExtendedNodesLmd\inter\InternalNodes$, $n \in \ExtendedNodesVar\inter\InternalNodes$, $i>0$ then
  $\istraversal \Pstr[0.3cm]{ t  \cdot
(m){m}\, (lx){\alpha'}  \cdot v \cdot (x-lx,30:i){n}
    \, (letai-m,45:i){\alpha}}$
    where $\alpha$ denotes the $i$th child ($\lambda$-node) of $m$.
\end{itemize}
...
for every $i$th child $\alpha$ of any node $m \in \ExtendedNodesVar$ occurring in $\oview{t\cdot n}$, $i\geq1$, we have
$\istraversal t \cdot n ~ \alpha$ where $\alpha$ points to $m$ with label $i$.
\end{itemize}
...
%
For \rulenamet{Lam^\ghostvar}, the induction hypothesis (ii) shows that $i>|m|$, which guarantees that $|\alpha'|+i-|m|>0$ is a valid justification label (i) and that (ii) holds for the traversed ghost node $\ghostvar$.
Finally (iii) holds because the node preceding $\ghostvar$ in the P-view is precisely $\alpha'$, its parent node.
%
...
$J^t = (\lambda u . u(x (\lambda \theta_1. u (\lambda \theta_2.\theta_1\,\theta_2))) (\lambda v . v\,y)$
for some fresh variable names $\theta_1, \theta_2$.
\end{example}
...
$\Pstr[0.7cm]{
 ...
 \, (n10-n1){\lambda v}
 \, (n11-n10){v}
 \, (n12-n9){\underline{\lambda\theta_2}}
 \, (n13-n8){\underline{\theta_1}}
 \, (n14-n13){\underline{\lambda}}
 \, (n15-n12){\underline{\theta_2}}
 \, (n16-n11){\lambda}
 \, (n17-n0,30){y} }$
 of $\ctree(J^t)$.
\end{example}
...
A traversal is \defname{strand-complete}
...
%
A finite \emph{maximal} traversal is necessarily strand-complete for if it were not ending with an external variable we could extend it with either \rulenamet{App}, \rulenamet{Lmd} or \rulenamet{Var}.
%
...
The \defname{concrete traversals} $\travset_\concrete$
are the imaginary traversals that contain no ghost occurrence.
They are defined by the rules of Table~\ref{tab:trav_rules} without \rulenamet{Lam^\ghostvar} and such that \rulenamet{Var} and \rulenamet{IVar} are restricted to their concrete sub-case, where the next node traversed must be structural (\ie, $i\leq|m|$).
\end{definition}
...
With such consideration, it suffices to look for a subset covering every possible core P-view.
...
The set of \defname{essential traversals} $\travsetes$ is the subset of $\travulc$ defined by the rules of Table~\ref{tab:trav_rules} where the justifier node in the input-variable rule \rulenamet{IVar} is restricted to be the preceding occurrence in the traversal. Written as a judgement derivation, the rule becomes:
\infrule[$\rulefont{IVar_\essential}$]
     {\istraversal t \cdot n
      \andalso n \in\ExternalNodes\inter\ExtendedNodesVar
      \andalso \alpha \in\ExtendedNodesLmd
      \andalso n \enables_i\alpha
      \andalso i \geq 1
     }
     {\istraversal  \Pstr[0.5cm]{t \cdot (n){n} \, (l-n,25:i){\alpha}}}

The set of \defname{concrete and essential traversals} $\travsetcones$ refers to $\travsetes \inter \travsetcon$ which is obtained by imposing the above restriction and additionally restricting the rule to its concrete sub-case (Definition~\ref{def:concrete_traversals}):
...
%
(ii) In essential traversals, non-initial occurrences of external lambda nodes must immediately follow their justifier by rule $\rulenamet{IVar}_\essential$, hence after removing internal nodes the P-view coincide with the identify function.
\endshortproof
\proofatend
%\begin{proof}
...
But with the goal of finding a normal form, traversing ghost nodes only helps if it eventually terminates and leads to some structural node of the final normal term tree.
...
The set of essential traversals is infinite due to rule $\rulename{IVar}$ where the justification label of the traversed node is unbounded. In this section, we show that we can bound it by a quantity called the \emph{dynamic arity}, effectively limiting the number of eta-expansions
 performed by rule \rulenamet{IVar}. Section \ref{sec:correctness_ulc_normalization} shows that this suffices to characterize the normal form of a lambda term when it exists.
...
We can therefore use rule \rulenamet{Var}
...
We then use rule \rulenamet{Lam^\ghostvar} to traverse the next ghost variable justified by $\alpha_q$ and labelled by $i_{q+1} = i_q + |\alpha_{q}| -|n_{q}|$.
...
Since $i_q > |n_q|$, by rule \rulenamet{Var} the next node is necessarily a ghost lambda node justified by $n_q$ with label $i_q$. With rule \rulenamet{Lam^\ghostvar} the following node is a ghost variable node $\ghostvar$ justified by $\alpha_q$ and labelled by $i_{q+1} = i_q + |\alpha_{q}| -|n_{q}|$.
\end{itemize}
...
in the system of rules of Definition~\ref{dfn:essential_traversals}
so that the argument index $i$ cannot be greater than the dynamic arity of the traversal. Written as a judgement derivation rule:
 \infrule[$\rulefont{IVar_\normalizing}$]
      ...
      {\Pstr[0.5cm]{\istraversal t \cdot (n){n} \, (l-n,25:i){\alpha}}.}
\iflongversion
...
\begin{table}[htbp]
    ...
    \caption{Normalizing traversals $\travsetnorm(M)$ of an untyped $\lambda$-term $M$, presented using judgement rules. They are a strict subset of imaginary traversals $\travulc(M)$ from Table \ref{tab:trav_rules}. The two systems only differ in rule  $\rulefont{IVar}$
...
t_1 = \Pstr[4ex]{
    ...
    \vtspace (n13-n8){{\ghostvar^{1}}}
    \vtspace (n14-n13){{\ghostlmd^{1}}}
...
@_1 \cdot\lambda\overline{h_1} \cdots @_q\cdot \lambda\overline{h_q}$ for some $q\geq 1$. The subterm at $@_r$,  represented on Figure~\ref{fig:strand_decomposition_proof_induction}, is of the form
$$(\lambda\overline{h_r}. A_0) A_1 \ldots A_{k_{r}-1}\ (\lambda\overline{y_2}. \ldots)A_{k_{r}+1}\ \ldots A_{|@_r|} .$$
...
{@_1} \cdot \lambda\overline{h_1} \cdot
\cdots
(atr){@_r} \lambda\overline{h_r} \cdot
\cdots
{@_q} \cdot\lambda\overline{h_q} \cdot
...
$\ghostvar_{k_i}$ and ghost lambda $\ghostlmd_{k_i}$ point respectively to $\lambda\overline{h_i}$ and $@_i$ with label $k_i\geq1$ defined by:
\begin{align*}
  k_i &= k - |x| + |\lambda\overline{h}_q| + \sum_{i\leq j < q} (|\lambda\overline{h}_{j}| - |@_{j+1}|)
%\\
%  k_i &= k + |\lambda\overline{h}_q| - |x| +
%  \sum_{j=i..q-1} |\lambda\overline{h}_{j}|
%  - \sum_{j=i..q-1} |@_{j+1}|
%\\
%  k_i &= k + |\lambda\overline{h}_q| - |x| +
%  \sum_{j=i..q-1} |\lambda\overline{h}_{j}|
...
%\\
%  k_i &= k + |\lambda\overline{h}_q| - |x| +
%  |\lambda\overline{h}_{i}| +
%  \sum_{j=i+1..q-1} (|\lambda\overline{h}_{j}| - |@_{j}|) - |@_{q}|
\\
   &= k - |x| +
  |\lambda\overline{h}_{i}| +
  \sum_{i< j\leq q} (|\lambda\overline{h}_j| - |@_j|)
\end{align*}
...
To prove that $lloc(\lambda\overline{y_2}) =\bot$ we first show that for all $i$ ranging from $r$ to $q$ we have
 $k_i>|\lambda_l(\lambda\overline{h_i})|$. We prove the result by a finite induction proceeding bottom-up from the lower tree node
 $\lambda\overline{h_q}$ up to $\lambda\overline{h_r}$.

\emph{Base case:} We have $i=q$. The subterm
rooted at $\lambda\overline{h_q}$ is in head normal form
$\lambda\overline{h_q}. x \cdots$ therefore its head lambda-list consists precisely of the lambda abstractions $\lambda\overline{h_q}$, and thus
$|\lambda_s(\lambda\overline{h_q})| = |\lambda\overline{h_q}|$.
We have $k_q = k -|x| + |\lambda\overline{h_q}|$ and by Equation~\ref{eqn:k_greater_than_x} we have $k>|x|$, hence $k_q >|\lambda\overline{h_q}|$.


\emph{Induction case:} Let $r+1\leq i \leq q$. We suppose that the result holds for $i$ we show that it must hold for $i-1$. By definition, the head lambda-list at $\lambda\overline{h_i}$ is precisely given by the variables in the abstraction concatenated with the head lambda-list at $@_{i+1}$, thus $|\lambda_l(\lambda\overline{h_i})| =
|\lambda\overline{h_i}| + |\lambda_l(@_{i+1})|$. The lambda-list at $@_i$, is by definition, obtained by popping $|@_i|$ lambdas from the lambda-list at $\lambda\overline{h_i}$; or is the empty list if the arity is greater than the number of pending lambdas. In terms of lengths this means:
\begin{align*}
    |\lambda_l(@_i)| &= \max(0, |\lambda_l(\lambda\overline{h_i})| - |@_i|) \\
     &= \max\left( 0, |\lambda_l(@_{i+1})| + |\lambda\overline{h_i}| - |@_i| \right) & \mbox{for $r\leq i \leq q-1$.}
\end{align*}

Furthermore by definition of $k_{i-1}$ we have:
\begin{equation}
k_{i-1} = k_i + |\lambda\overline{h_{i-1}}| - |@_i| \label{eqn:ki_minus_one}
\end{equation}
Hence,
\begin{align*}
    |\lambda_l(\lambda\overline{h_{i-1}})| - k_{i-1}
    &= |\lambda\overline{h_{i-1}}| +|\lambda_l(@_i)| - k_{i-1} &\mbox{(Def~of $\lambda_l$)}\\
    &= |\lambda\overline{h_{i-1}}| + \max\left( 0, |\lambda_l(\lambda\overline{h_i})| - |@_i| \right) - k_{i-1} &\mbox{(Def~of $\lambda_l$)}\\
    &= \max\left( |\lambda\overline{h_{i-1}}| - k_{i-1},    |\lambda\overline{h_{i-1}}| + |\lambda_l(\lambda\overline{h_i})| - |@_i| - k_{i-1} \right) \\
    &= \max\left(  |@_i|- k_i, |\lambda_l(\lambda\overline{h_i})| - k_i \right) &\mbox{(Eqn~\ref{eqn:ki_minus_one})}
\end{align*}
By the induction hypothesis we have $|\lambda_l(\lambda\overline{h_i})| < k_i$, and by Equation~\ref{eqn:qnfdecomp_kj} we have $ |@_i|< k_i$, therefore the maximum of the two quantities in the last equation is negative which shows
the desired result $|\lambda_l(\lambda\overline{h_{i-1}})| < k_{i-1}$.
...
\begin{tikzpicture}[baseline=(root.base),level distance=6.5ex,inner ysep=0.5mm,sibling distance=15mm]
    ...
    child[dashed] {node{$\lambda\overline{w_1}$}
        child[solid] {node{$@_1$}
            child{node{$\ldots$}
                child{node{$@_{r}$}
                    child{node{$\lambda\overline{h_r}$}
                        child[dashed]{node{$@_{q-1}$}
                            child[solid]{node{$\lambda\overline{h_{q-1}}$}
                                child[solid]{node{$@_q$}
...
By assumption we have that $lloc_M(\lambda\overline{w_1}) =\bot$ so since $@_r$ occurs in the spine of $M$, by definition of $lloc_M$ we must have $lloc_M(@_r) =\bot$, and thus also $lloc_M(\lambda\overline{h_r}) = \bot$. We have just shown that $|\lambda_l(\lambda\overline{h_r})| < k_r$, consequently, the sub-term $(\lambda\overline{h_r} . A_0) A_1 \ldots A_{k_r-1}$ has more operands than pending lambdas in the operator's lambda list. By definition of lambda list this implies that $\lambda_l((\lambda\overline{h_r} . A_0) A_1 \ldots A_{k_r-1})$ is empty. Hence, we necessarily have $lloc(\lambda\overline{y_2}) = \bot$, otherwise by definition of $lloc$ we would have
 $lloc((\lambda\overline{h_r} . A_0) A_1 \ldots A_{k_r-1} (\lambda\overline{y_2}. \ldots)) = lloc(\lambda\overline{y_2}) \ne \bot$, which subsequently implies $lloc(\lambda\overline{w_1})\ne\bot$, contradicting the assumption.
...
@_1 \cdot\lambda\overline{h_1} \cdots @_q\cdot \lambda\overline{h_q}$ for some $q\geq 1$. The subterm at $@_r$, is of the form
$(\lambda\overline{h_r}. A_0) A_1 \ldots A_{k_{r}-1}\ (\lambda\overline{y_2}. \ldots)A_{k_{r}+1}\ \ldots A_{|@_r|}$.
...
$
   ...
   \cdot {@_1}
   \cdot \lambda\overline{h_1}
   \cdots %(atr)
        {@_r}
   \cdot \lambda\overline{h_r}
   \cdots {@_q}
   \cdot\lambda\overline{h_q}
   \cdot %(x)
        {\underline{x_1}}
   \cdot %(gl-x,25:k)
        {\underline\ghostlmd}
...
where ghost variables point to $\lambda\overline{h_i}$ for
$i \in \{ r..q \}$ with label $k_i\geq1$,
ghost lambdas point to $@_i$ for $i \in \{ r-1..q \}$ with label $k_i\geq1$;
where $k_q = k - |x| + |\lambda\overline{h}_q|$ and $ k_{i-1} = k_i + |\lambda\overline{h_{i-1}}| - |@_i|$, $k_i\geq1$.

We show by a finite induction on $i \in \{r, q\}$ proceeding bottom-up from the lower tree node $\lambda\overline{h_q}$ up to $\lambda\overline{h_r}$
that $k_i>|\lambda_l(\lambda\overline{h_i})|$. Base case follows from $k>|x|$ since $\lambda\overline{y_2}$ is structural.
 %induction
Suppose the result holds for $i\in\{r+1..q\}$.
The lambda-list at $@_i$ is obtained by popping $|@_i|$ lambdas from the lambda-list at $\lambda\overline{h_i}$ and $\lambda_l(\lambda\overline{h_i})$ is by definition $\overline{h_i}$ concatenated with the lambda-list of $@_{i+1}$, hence $|\lambda_l(@_i)| = \max( 0, |\lambda_l(@_{i+1})| + |\lambda\overline{h_i}| - |@_i| )$.
We thus have $|\lambda_l(\lambda\overline{h_{i-1}})| - k_{i-1}
    = |\lambda\overline{h_{i-1}}| +|\lambda_l(@_i)| - k_{i-1}
    = \max (  |@_i|- k_i, |\lambda_l(\lambda\overline{h_i})| - k_i )$.
By the induction hypothesis we have $|\lambda_l(\lambda\overline{h_i})| < k_i$, and since nodes in $v$ are all ghosts we have $|@_i|< k_i$ thus the `$\max$' is negative which shows $|\lambda_l(\lambda\overline{h_{i-1}})| < k_{i-1}$.
Since in particular $|\lambda_l(\lambda\overline{h_r})| < k_r$, the application $(\lambda\overline{h_r} . A_0) A_1 \ldots A_{k_r-1}$ has more operands than pending lambdas in the operator, thus its lambda list is empty. By assumption the strand is of type $S$ thus $lloc(\lambda\overline{w_1})=\bot$ and since there is no lloc ``left of'' $\lambda\overline{y_2}$, there cannot possibly be a lloc under $\lambda\overline{y_2}$. Hence $lloc(\lambda\overline{y_2}) =\bot$.
...
\path (L) edge [bend right,->>]      node[right,xshift=-2em,text width=4cm]{Spinal descent to \emph{lloc} \rulenamet{App},\rulenamet{Lmd^{\sf var}},\rulenamet{Lmd^@}} (V)
      (V) edge [bend right]  node[left,text width=3cm] {Concrete argument $\rulename{IVar}_\normalizing$} (L)
          edge [bend left]   node[right, text width=3cm] {Imaginary argument $\rulename{IVar}_\normalizing$} (G)
...
%
To simplify the definition of bisimulation, we now assume that $\travsetnorm$ also contain the empty justified sequence $\epsilon$, which was excluded in our presentation of traversal rules, so that $\travsetnorm(M)$
...
\node (root){}
        child[dashed]{node{$@_{a}$}
            child[solid]{node{$\lambda\overline{h}_a$}
...
\fi
%
Let $@_j$ and $\lambda\overline{h_j}$ for $1\leq j\leq a$, $a\geq 1$
...
So that $\lambda\overline{h_1}$ is $x$'s binder and $@_1$ its parent.
Let $i$ denote $x$'s binding index in $\lambda\overline{h_1}$.
%
After reduction,
...
\begin{eqnarray*}
    u &=& \Pstr[10pt]{ \ldots (aa){@_a} \cdot (la){\bulklambda{h_a}} \cdot \ldots (a1){@_1} \cdot (l1){\bulklambda{h_1}} \cdot @ \ldots \bulklambda{z} \cdot (ax){@_x} \cdot (ly-ax,20:0){\bulklambda{y}'} }
    \\
    \phi(u) &=& \Pstr[10pt]{ \ldots (aa){@_a} \cdot (la){\bulklambda{h_a}} \cdot \ldots (a1){@_1} \cdot (l1){\bulklambda{h_1}} \cdot @ \ldots \bulklambda{z} \cdot (x-l1,30:i) x \cdot \ghostlmd \cdot \ghostvar \ldots \ghostlmd \cdot \ghostvar \cdot (ly-a,20:i_a){\bulklambda{y}} }
\end{eqnarray*}
\else
So a traversal of the shape
$u = \Pstr[10pt]{ \ldots @_a (la){\bulklambda{h_a}}
  \ldots (a1){@_1} (l1){\bulklambda{h_1}} \, @ \ldots \bulklambda{z} \, (ax){@_x} \, (ly-ax,20:0){\bulklambda{y}'} }$ of $N$
maps to
$\phi(u) = \Pstr[10pt]{ \ldots @_a \, (la){\bulklambda{h_a}}
\ldots (a1){@_1} \, (l1){\bulklambda{h_1}} \, @ \ldots \bulklambda{z} \, (x-l1,25:i) x \, \ghostlmd \, \ghostvar \, \ghostlmd \, \ghostvar \, (ly-a,22:i_a){\bulklambda{y}}}$.
\fi
...
%
    ...
    from $@_a$ to $\lambda\overline{h_1}$ verifying $|@_a| >  i_a$,
        where $i_a = i + \sum_{1\leq j< a} (|\lambda\overline{h_j}| - |@_j|)$.
...
A traversals of $M$ of the form $\ldots \lambda\overline{z} \cdot x \cdot v \cdot \lambda\overline{y} \ldots$, where $v$ is an argument lookup, can be bisimulated by traversals in $N$ of the form $\ldots \lambda\overline{z}\overline{y}' \ldots$ The arity of $x$ is not preserved but it does not alter the dynamic arity because $|\lambda{\overline{z} \overline{y}'}| =
...
We have seen that imaginary traversals implements leftmost linear reduction,
%in the untyped lambda calculus
a non-strict, ``hyper-lazy''~\cite{danosherbelinregnier1996}, reduction strategy that substitutes variables one at a time. This bring the question of whether alternative notions of traversals are definable for other evaluation strategies such as call-by-name and call-by-value~\cite{plotkin-75}. A call-by-name version of head-linear reduction, that does not reduce inside the body of an unapplied abstraction, would easily be accommodated in the traversal setting. A strict evaluation strategy like call-by-value, on the other hand, might require substantial changes. As for traditional reduction strategies, such as the normal order of lambda calculus,
%it is not immediately obvious how to implement simultaneous substitution with traversals since, by nature, they traverse the term tree one node at a time.
perhaps this can be modeled by non-deterministically spawning new traversals for every free occurrence of the variable to be substituted.
