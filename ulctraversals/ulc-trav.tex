...
denoted $\exttree(M)$ over the \defname{extended nodes} $\ExtendedNodes = \Nodes + \ghostvar + \ghostlmd$, where $+$ denotes disjoint set union. We use the shorthands $\ExtendedNodesLmd$ for $\NodesLmd + \ghostlmd$ and $\ExtendedNodesVar$ for $\NodesVar + \ghostvar$.
...
Two subsets $J_1\subseteq \justseqset(M_1)$ and $J_2\subseteq\justseqset(M_2)$ are \defname{isomorphic}, written $J_1\structisomorphic J_2$, if  there exists a structure-preserving bijection between $J_1$ and $J_2$; that is a bijection $\phi :J_1\longrightarrow J_2$ such that $j\equiv\phi(j)$ for all $j\in J$.
...
\begin{itemize}[leftmargin=3em]
    \item[\rulenamet{Root}]
    $\istraversal \alpha$ where $\alpha$ is the root $\lambda$-node of the term tree $\ctree(M)$.
    ...
    \begin{compactitem}
        ...
        \item[\rulenamet{Lam^{\sf var}}] a structural variable in $\NodesVar$ then it points to the only occurrence in $\pview{t\cdot \alpha}$ of a node $m$ verifying $m \enables_k n$ for some $k\geq 0$, and has link label $k$;
...
\path (L) edge [bend right,->>]      node[right,xshift=-2em,text width=4cm]{Spinal descent to \emph{lloc} \rulenamet{App},\rulenamet{Lmd^{\sf var}},\rulenamet{Lmd^@}} (V)
      ...
      (G) edge [->>]                 node[below,text width=4cm] {Pending argument look-up \rulenamet{Lmd^\theta}, $\rulename{Var}$} (L)
;
\end{tikzpicture}
...
\fi
%%%%%%%
\begin{tikzpicture}[baseline=(root.base),level distance=3ex,inner ysep=0.5mm,sibling distance=7mm]
\node (root){}
...
nodes statically determined by the term tree and obtained by repeatedly instantiating rules \rulenamet{Lmd} and \rulenamet{Var} until reaching node $\lambda\overline{y}$.
%
\iflongversion
...
$u = \Pstr[10pt]{ \ldots @_a (la){\bulklambda{h_a}}
  \ldots (a1){@_1} (l1){\bulklambda{h_1}} \, @ \ldots \bulklambda{z} \, (ax){@_x} \, (ly-ax,20:0){\bulklambda{y}'} } \in \travsetnorm(N)$
is
...
$\Pstr[10pt]{\ldots
    (l){\lambda\overline{w}}  (a)@  (lx-a,25:0){\lambda\overline{x}} \ldots (gl-a,25:i)\ghostlmd  (gv-l,33:{|\eta|-1+i})\ghostvar }
$ of $M$, $i\geq1$.
