...
Since $s$ is a sub-stack of a reachable configuration, by Remark \ref{rem:linkorder_cpdaprime} we have $j=1$ if $\lambda\overline\eta$ is prime and $j = n-\ord \lambda\overline\eta +1$ otherwise. Furthermore, since
$top_2\,s$ is safe and $\lambda\overline\eta$ belongs to the order-decomposition, the height component necessarily equals $1$.
\end{proof}
...
Clearly $CPDA(G)$ and $CPDA'(G)$, have the same set of $\rightarrow$-reachable configurations
so Proposition \ref{prop:reachconf_safe} clearly holds for $CPDA(G)$.
The simulating PDA from \ref{simulating_pda} is obtained from
from $CPDA(G)$ by replacing every $collapse$ operation by $pop_{n-\ord{top_1(s)}+1}$;
