...
\todo{A proof attempt was presented in a previous version of this note. Since then Broadbent has obtained a working proof based on the same idea but using a different notion of stack-safety \cite{Broadbent2009}. This note fixes the hole in my original attempt by introducing yet another definition of stack safety inspired by Broadbent's one.}
...
since $\lambda\overline\eta_b$ is the last node in $\orddec_l(c)$, by definition of $l$-safety we have that $t$ is $l$-safe. Finally Lemma \ref{lem:push1pop1_preserves_safety} shows that $c'$ is safe.
