...
since $\lambda\overline\eta_b$ is the last node in $\orddec_l(c)$, by definition of $l$-safety we have that $t$ is $l$-safe. Finally the lambda node $E_i(top_1)$ pushed by the last operation has precisely order $l =\ord(x)$ therefore
\begin{align*}
\orddec_0 (c') &= \orddec_l (c') \cdot E_i(top_1(t))^1 \\
&= \orddec_l (t) \cdot E_i(top_1(t))^1 & \mbox{by Lemma \ref{lem:push1pop1_orderdecompo}(i).}
\end{align*}

Thus since $t$ is $l$-safe we have that all the lambda nodes in $\orddec_0 (c')$ have a link of height $1$.
To obtain that $c'$ is safe it remains to show that $collapse(c'_{\prefixof{E_i(top_1(t))}}) = collapse\, c'$ is safe.
Since we have $i\geq 1$ the node $top_1(c') = E_i(top_1(t))$ is not a prime lambda node, thus by Lemma \ref{lem:safecollapsesimulation} we have:
\begin{align*}
...
top $(n-l)$-stack. Consequently, the operation $pop_{n-l+1}$ effectively restores the stack 
