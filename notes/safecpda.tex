...
%$$sig(s) = \left\{
%           \begin{array}{ll}
%             \orddec_{\ord(top_1\,s)}(s), & \hbox{if $top_1\,s \in \INodes_@ \union \INodes_{var}$;} \\
%             \orddec_{-1}(s) = \orddec_{\ord(top_1\,s)}(pop_1\,s) \cdot (top_1\,s), & \hbox{if $top_1\,s \in \INodes_\lambda$.}
%           \end{array}
%         \right.$$
%\end{definition}
%
...
By (\ref{eqn_safe_prefix}), $\orddec_l(s)$ is a prefix of $\orddec(s)$.
...
(\ref{eqn_ordec_l}),  $\orddec_l(s_{\prefixof \lambda\overline\eta_j}^{\langle n-l+1\rangle})
